...
class BaseFlexibleFund(ABC):
    ...
    def get(self, start, end):
        ...
        portfolio_strategies = [col for col in weights.columns if col.startswith('portfolio.')]
        alpha_strategies = [col for col in weights.columns if col.startswith('alpha.')]
        flexible_fund_strategies = [col for col in weights.columns if col.startswith('flexible_fund.')]
        ...
        for flexible_fund_strategy in flexible_fund_strategies:
            flexible_fund = flexible_fund_loader.get_flexible_fund(flexible_fund_strategy)
        ...
        for strategy in strategy_dict:
            strategy_dict[strategy] = strategy_dict[strategy].reindex(all_indices).ffill()
        ...
        pf = sum(
            strategy_dict[strategy] * weights[strategy].values[:, None] for strategy in strategy_dict
        )
    ...
    def weight(self, start, end) -> Optional[pd.DataFrame]:
        ...
        strategy_list = self.portfolios
        sub_strategies = [self.portfolio_loader(start, end).get_portfolio(portfolio) for portfolio in self.portfolios]
        if hasattr(self, 'alphas') and self.alphas:
        ...
        if hasattr(self, 'flexible_funds') and self.flexible_funds:
            ...
            sub_strategies.extend([
                self.flexible_fund_loader(start, end).get_flexible_fund(flexible_fund)
