...
void DependencyView::build_graph(
  ...
  QRectF sceneRect = scene_->itemsBoundingRect();

  // Add a small margin (10% on each side)
  qreal margin = 0.1;
  QRectF expandedRect = sceneRect.adjusted(
      -sceneRect.width() * margin, -sceneRect.height() * margin,
      sceneRect.width() * margin, sceneRect.height() * margin);

  // Set the scene rect to this expanded rect
  scene_->setSceneRect(expandedRect);

  // Fit the view to show this rectangle
