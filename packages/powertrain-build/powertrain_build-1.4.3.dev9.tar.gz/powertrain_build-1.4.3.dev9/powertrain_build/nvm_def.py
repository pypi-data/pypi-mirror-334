...
class ZCNVMDef(NVMDef):
    ...
    def _append_nvm_rte_function_calls(self):
        ...
        main_function.append(error_status)
        if nvsw_init_instructions:
            init_function_lines.append("  // NVSWCOMPONENTS init calls are scheduled by the arxml.\n")
        main_function.extend(main_function_lines)

        if type2_warning:
            self.warning(
                'NVSWCOMPONENT write calls are not generated automatically for type 2, \
                they have to be written manually and added to source code.'
            )

        output = init_function_lines + footer + ["\n"] + main_function + footer
