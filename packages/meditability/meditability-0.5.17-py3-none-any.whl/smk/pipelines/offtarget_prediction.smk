...
rule set_gscan_indices:
    ...
    params:
        gscan_indices_dir=lambda wildcards: os.path.join("{meditdb_path}/gscan_indices".format(
            meditdb_path=config["meditdb_path"])),
        gs_index_prefix=lambda wildcards: os.path.join("{meditdb_path}/gscan_indices/{offtarget_genomes}.consensus.fa.index".format(
...
rule casoff_run_ref:
    input:
        casoff_input="{root_dir}/{mode}/jobs/{run_name}/guide_prediction-{reference_id}/offtarget_prediction/input_files/{query_index}_{editing_tool}_guidescan_input.csv",
        gs_index=lambda wildcards: os.path.join(f"{config['meditdb_path']}/gscan_indices/{wildcards.reference_id}.consensus.fa.index.gs")
    output:
    ...
    params:
        ...
        gscan_indices_path=lambda wildcards: os.path.join(f"{config['meditdb_path']}/gscan_indices"),
...
rule casoff_run_extended:
    ...
    params:
        ...
        # == GuideScan Indices Path
        gscan_indices_path=lambda wildcards: os.path.join("{meditdb_path}/gscan_indices".format(meditdb_path=config["meditdb_path"])),
        # == GuideScan Parameters
