...
rule serialize_chromosomes:
	input:
		assembly_path=lambda wildcards: os.path.join(f"{config['fasta_root_path']}/{wildcards.sequence_id}.fa.gz")
	output:
...
rule predict_guides:
	input:
		...
		serialized_chr_manifest = "{root_dir}/tmp/{sequence_id}_chr_manifest.csv",
		assembly_dir_path = lambda wildcards: os.path.join("{fasta_root_path}".format(
...
rule filter_vcf:
	input:
		assembly_path=lambda wildcards: os.path.join("{fasta_root_path}/{sequence_id}.filtered.fa.gz".format(
			fasta_root_path=config["fasta_root_path"], sequence_id=wildcards.sequence_id)),
		source_vcf=lambda wildcards: os.path.join("{meditdb_path}/{mode}/source_vcfs/{vcf_filename}.vcf.gz".format(
