import os
from src.vertibit_osi_image_generator.shared_aux.file_scanner_shared import read_text_file, scan_txt_file_for_key, scan_directory, scan_for_main_modules, clean_module_path
...
def generate_flask_start_command(entry_file):
    start_command = f'["/app/venv/bin/python","-m","gunicorn", "-w" ,"4", "-b", "0.0.0.0:8000", "{
        entry_file}:app"]'
...
def generate_fastapi_start_command(entry_file):
    start_command = f'["/app/venv/bin/python","-m","uvicorn" ,"{
...
def generate_standard_python_start_command(entry_file):
    start_command = f'["/app/venv/bin/python","{entry_file}"]'
