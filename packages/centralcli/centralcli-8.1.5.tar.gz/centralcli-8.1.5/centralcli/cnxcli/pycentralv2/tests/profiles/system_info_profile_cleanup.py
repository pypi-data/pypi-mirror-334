...
def main():
    ...
    result = sysinfo_global.delete()

    if not result:
            print("error in deleting Library system information profile")
            exit()
    ...
    for site, sysinfo in sites.items():
        ...
        result = tmp_sysinfo.delete()

        if not result:
            print("error in deleting Site system information profile " +
                  f"{tmp_site.name}")
            exit()
