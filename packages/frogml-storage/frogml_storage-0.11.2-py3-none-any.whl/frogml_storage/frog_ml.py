...
from frogml_storage._artifactory_api import ArtifactoryApi, StartTransactionResponse
from frogml_storage._log_config import logger
from frogml_storage._storage_utils import calculate_sha2
from frogml_storage._utils import (
...
from frogml_storage.dataset_manifest import DatasetManifest
...
from frogml_storage.storage import Storage
from frogml_storage.utils._input_checks_utility import (
    is_not_none,
...
class FrogMLStorage(Storage):
        ...
        try:
            ...
            if version is None:
                version = response.files_upload_path.replace(
                    f"{entity_type_info.folder_name}/{namespace_and_name}/tmp/", ""
                ).split("/")[0]
                logger.debug(
            ...
            if entity_type_info == FrogMLEntityTypeInfo.MODEL:
                assert isinstance(model_type, SerializationMetadata)
    ...
    def __milliseconds_to_iso_instant(milliseconds: str) -> str:
        instant = datetime.utcfromtimestamp(int(milliseconds) / 1000.0).replace(
            tzinfo=timezone.utc
        )
