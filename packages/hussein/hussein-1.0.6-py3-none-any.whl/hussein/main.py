...
Version 1.0.6 | Type 'hussein -h' for help\033[0m
"""
...
class HusseinTool:
    ...
    def get_current_version(self):
        """Get the current version of the package from setup.py or __init__.py"""
        # Attempt to read version from setup.py
        try:
            setup_path = os.path.join(os.path.dirname(__file__), "..", "setup.py")
            setup_path = os.path.abspath(setup_path)  # Convert to absolute path
            ...
            print(f"Error reading version from setup.py: {e}")

        # Fallback: attempt to read version from __init__.py
        try:
