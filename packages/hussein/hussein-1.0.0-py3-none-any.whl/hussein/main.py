...
Version 1.1.0 | Type 'hussein -h' for help\033[0m
"""
...
class HusseinTool:
    ...
    def run(self):
        ...
        try:
            ...
            if args.command in ["scan", "subdomains"]:
    ...
    def scan_network(self, target, port_range, timeout=0.5, verbose=False, output_file=None):
        """Scan a target for open ports with improved feedback"""
        print(f"\n\033[1;34m[*] Scanning target: {target}\033[0m")
        print(f"\033[1;34m[*] Port range: {port_range}\033[0m")
        
        try:
            # Parse port range with better error handling
            if "-" in port_range:
            ...
            print(f"\n\033[1;34m[*] Starting scan on {ip} ({end_port - start_port + 1} ports)...\033[0m")
            ...
            for port in range(start_port, end_port + 1):
                ...
                if result == 0:
                    ...
                    banner = self._get_banner(ip, port) if verbose else ""
                    print(f"\033[1;32m{port:<10}{'open':<12}{service:<20}{banner:<18}\033[0m")
                elif verbose:
                    # If verbose, show closed/filtered ports too
            ...
            if output_file and open_ports:
                result_content = f"# HUSSEIN Port Scan Results\n"
                result_content += f"# Target: {target} ({ip})\n"
                result_content += f"# Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                result_content += f"# Port range: {port_range}\n"
                result_content += f"# Open ports: {len(open_ports)}\n\n"
                result_content += "PORT\tSTATE\tSERVICE\n"
                result_content += "----\t-----\t-------\n"
