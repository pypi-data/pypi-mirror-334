...
class SensorResource(Resource):
    ...
    def to_dict(self):
        return {"resource": {"sensorName": self.sensor_name, **super().to_dict()}}


@dataclass
class ImageResource(SensorResource):
    start_shutter_timestamp_ns: Optional[int]
    end_shutter_timestamp_ns: Optional[int]
    ...
    def __post_init__(self):
        ...
        if self.start_shutter_timestamp_ns >= self.end_shutter_timestamp_ns:
...
class Frame:
    ...
    pointclouds: Optional[List[SensorResource]]
    ego_vehicle_pose: Optional[EgoVehiclePose]
...
class SceneRequest:
    ...
    external_id: str
    frames: List[Frame]
    sensor_specification: Optional[SensorSpecification]
    calibration_id: Optional[str]
    metadata: Optional[dict]
    imudata_resource: Optional[Resource]
    should_motion_compensate: Optional[bool]
    postpone_external_resource_import: Optional[bool]
    ...
    def is_indexed(self):
        return self.frames[0].images[0].external_resource_uri is not None
    ...
    def to_dict(self):
        ...
        if self.postpone_external_resource_import:
