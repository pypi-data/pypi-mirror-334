...
def direct_download_files(  # noqa: D103
    ...
    try:
        with progress_bar:
            if NGC_CLI_EXPERIMENTAL_DOWNLOAD:
                print("Using requests library to download")
...
def request_model_file_url(session, url, api_client, auth_org, auth_team: str):  # noqa: D103
    _retry = 1
    while _retry <= NGC_CLI_DOWNLOAD_RETRIES:
        try:
...
def request_s3_download(  # noqa: D103
    session: requests.Session, url: str, full_fp: str, size: int, progress: ThreadTransferProgress
):
    _retry = 1
...
def download_one_file(file_params):  # noqa: D103
    filename, size, url, download_dir, api_client, auth_org, auth_team, progress, ngc_session, s3_session = file_params
    ...
    request_s3_download(s3_session, presigned_url, outfile, size, progress=progress)
