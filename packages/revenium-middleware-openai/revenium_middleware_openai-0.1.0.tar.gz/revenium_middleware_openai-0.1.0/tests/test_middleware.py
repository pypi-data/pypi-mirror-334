...
from revenium_middleware_openai.metering import shutdown_event, StopReason
...
class TestMiddleware:
    ...
    @patch("revenium_middleware_openai.middleware.run_async_in_thread")
    @patch("revenium_middleware_openai.middleware.client")
    def test_create_wrapper_basic(self, mock_run_async, mock_client, reset_state, mock_openai_response, test_kwargs, caplog):
        """Test the basic functionality of create_wrapper."""
        # Set up mocks
        mock_wrapped = MagicMock(return_value=mock_openai_response)
    ...
    @patch("revenium_middleware_openai.middleware.run_async_in_thread")
    @patch("revenium_middleware_openai.middleware.client")
    def test_create_wrapper_no_metadata(self, mock_run_async, mock_client, reset_state, mock_openai_response):
    ...
    @patch("revenium_middleware_openai.middleware.run_async_in_thread")
    @patch("revenium_middleware_openai.middleware.client")
    def test_create_wrapper_during_shutdown(self, mock_run_async, mock_client, reset_state, mock_openai_response, test_kwargs, caplog):
    ...
    @patch("revenium_middleware_openai.middleware.run_async_in_thread")
    @patch("revenium_middleware_openai.middleware.client")
    def test_create_wrapper_metering_exception(self, mock_run_async, mock_client, reset_state, mock_openai_response, test_kwargs, caplog):
    ...
    def test_create_wrapper_no_choices(self, mock_run_async, mock_client, reset_state, 
                                      mock_openai_response, test_kwargs):
    ...
    @freeze_time("2023-01-01T12:00:00Z")
    @wrapt.decorator(enabled=False)
    @patch("revenium_middleware_openai.middleware.datetime")
    @patch("revenium_middleware_openai.middleware.run_async_in_thread")
    @patch("revenium_middleware_openai.middleware.client")
    def test_create_wrapper_request_duration(self, mock_run_async, mock_client, mock_datetime, 
                                            reset_state, mock_openai_response, test_kwargs):
