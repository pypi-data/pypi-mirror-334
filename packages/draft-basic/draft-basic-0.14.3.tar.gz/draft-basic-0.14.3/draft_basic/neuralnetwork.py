...
class MLP:
    ...
    def train(self, x_train, y_train, x_valid='NULL', y_valid='NULL', lr=0.01, epochs=100, batch_size=1, verbose_frequence=1, methods=False):
            ...
            for method in methods:
                ...
                if method == 'validation_set_early_stop':
                    if not isinstance(methods['validation_set_early_stop'][0], int):
                        ...
                        if not methods['validation_set_early_stop'][1] is False:
                            raise ValueError(f"目前只支持保存提前终止后训练的最后一代模型的参数，验证集损失最小的模型暂不支持")
                        else:
