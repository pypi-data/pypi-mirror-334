...
try:
    import gssapi
    if getattr(gssapi, '__title__', None) == 'python-gssapi':
...
def GSSAuth(auth_method, gss_deleg_creds=True):
    ...
    :return: Either an `._SSH_GSSAPI` (Unix) object or an `_SSH_SSPI` (Windows) object
    :rtype: object
    ...
    :note: Check for the available API and return either an `._SSH_GSSAPI`
           (MIT GSSAPI using gssapi package) object or an `._SSH_SSPI` (MS SSPI) object.
    """
    if _API == "MIT":
        raise ImportError("unsupported gssapi package installed"
                          ' (need "gssapi" instead of "python-gssapi")')
    elif _API == "MIT-NEW":
