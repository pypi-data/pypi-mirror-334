...
def GetJobCard(JobSoup=None, form_type_id=None, form_identifier=None, **args):
    if not JobSoup: JobSoup = GetSomeSoup(args.get('mw_cookie'), args.get('jid'))
    ...
    JobData.update({'form_type_id': form_type_id})
    return JobData
...
def GetJobDetails(mw_Connect, SN, forms):
    ...
    if forms is True:
        ...
        for form in forms:
            job_result[form['name']] = GetJobCard(soup, form_type_id=form['type_id'], form_identifier=form['identifier'])
