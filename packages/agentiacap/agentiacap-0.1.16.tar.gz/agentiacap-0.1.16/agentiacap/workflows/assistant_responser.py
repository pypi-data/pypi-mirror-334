...
Si la factura está contabilizada en SAP con una fecha de pago, genera el siguiente correo:
Asunto: Estado de facturas - (Sociedad) Cuerpo:

Estimado Proveedor,

Hemos verificado en el sistema y le informamos:
La(s) factura(s) N° (Nro. de Factura(s)) se encuentra(n) contabilizada(s) con Nro. de OP (número de documento de compensación) abonada(s) en la fecha (fecha de compensación).

Recuerde que puede descargar los comprobantes mediante Extranet de Proveedores.

Recordatorio: 
RD en SAP es FCE - Factura de Crédito Electrónica.
RE en SAP es FC - Factura Común.
...
Si la factura está contabilizada pero no tiene una orden de pago generada, y la fecha de vencimiento no ha pasado, genera el siguiente correo:
Asunto: Estado de facturas - (Sociedad) Cuerpo:

Estimado Proveedor,

Hemos verificado en el sistema y le informamos:
La(s) factura(s) N° (Nro. de Factura(s)) se encuentra(n) contabilizada(s) y con fecha de vencimiento (fecha de vencimiento). Actualmente no se ha generado una orden de pago. Puede hacer el seguimiento/control de la misma mediante el Portal de Extranet.
...
Si la factura está contabilizada pero no tiene una orden de pago generada y la fecha de vencimiento ya pasó, genera el siguiente correo:
...
# Factura contabilizada vencida sin OP generada, esperando confirmación del CBU
caso_4 = """Factura contabilizada vencida sin OP generada, esperando confirmación del CBU:

Si la factura está contabilizada y vencida, y se espera confirmación del CBU del proveedor, genera el siguiente correo:
Asunto: Facturas contabilizadas vencidas - (Sociedad) Cuerpo:
...
def responder_mail(datos:list):
    prompt = f"""Eres un asistente experimentado y dedicado a responder emails. Tu tarea consiste en analizar una lista de datos de facturas,
    agruparlas según el caso al que correspondan y generar un mail conjunto que respete el formato de mail de cada caso con todas las facturas.
