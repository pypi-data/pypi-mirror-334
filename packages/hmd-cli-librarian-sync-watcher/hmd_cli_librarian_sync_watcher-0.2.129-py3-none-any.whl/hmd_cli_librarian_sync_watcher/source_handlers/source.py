    ...
    def upsert_file(
        ...
        if file:
            ...
            if (
                ...
                #     self._librarian_sync_client.get_file_upload_by_file_id(
                #         file.identifier
                #     )
                # )
                ...
                # ]:
                #     logger.info(
                #         f"{self.name}: Canceling existing FileUpload for file {str(source_path)}"
                #     )
                #     existing_file_upload.upload_status = "canceled"
                #     self._librarian_sync_client.upsert_file_upload(existing_file_upload)

                # logger.info(
                #     f"{self.name}: Creating new FileUpload for updated file {str(source_path)}"
                # )
                # new_file_upload = FileUpload(upload_status="pending")
                # new_file_upload = self._librarian_sync_client.upsert_file_upload_hmd_lang_librarian_sync(
                #     new_file_upload
                # )
                # self._librarian_sync_client.upsert_file_to_upload_hmd_lang_librarian_sync(
                #     FileToUpload(
                #         ref_from=file.identifier, ref_to=new_file_upload.identifier
                #     )
                # )
                ...
                if file.librarians_synced[self.librarian] != file.modified:
                    ...
                    #     )
                    #     existing_file_upload.upload_status = "canceled"
                    #     self._librarian_sync_client.upsert_file_upload(
                    #         existing_file_upload
                    #     )
                    # new_file_upload = FileUpload(upload_status="pending")
                    # new_file_upload = self._librarian_sync_client.upsert_file_upload_hmd_lang_librarian_sync(
