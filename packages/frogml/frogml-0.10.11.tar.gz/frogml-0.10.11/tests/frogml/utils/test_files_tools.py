...
def test_zip_model_success(tmp_path: Path):
    ...
    # When
    zip_file = _zip_model(code_dir_path=dir_path, target_dir=tmp_path)
...
def test_zip_model_error_on_zipping(tmp_path: Path):
    # Given
    dir_path: str = "path/does/not/exist"

    # When + Then
    try:
        _zip_model(code_dir_path=dir_path, target_dir=tmp_path)
        assert False
    except Exception as e:
        assert str(e).__contains__("Unable to zip model")
