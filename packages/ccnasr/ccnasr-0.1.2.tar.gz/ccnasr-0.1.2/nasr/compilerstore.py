...
class BuiltCompilerInfo:
    """Information about a compiler built by nasr.
    ...
    Attributes:
        ...
        prefix (Path):
            ...
            be subdirectory of the nasr compiler store
        commit (Commit):
...
class CompilerStore:
    """A database of compilers built by nasr.

    Appart from the built compilers, the database also stores
    the set of commits that nasr faild to build.
    """
    ...
    def add_failed_to_build_compiler(
        ...
        """Record that nasr failed to build a compiler.
        Args:
            project (CompilerProject):
    ...
    def has_previously_failed_to_build(
        ...
        """Check if nasr has previously failed to build a compiler.
        Args:
            project (CompilerProject):
                The compiler project (GCC or LLVM)
            commit (Commit):
        ...
        Returns:
            bool:
                True if nasr has previously failed to build the compiler.
        """
    ...
    def failed_to_build_compilers(self) -> list[tuple[CompilerProject, Commit]]:
        """Get the list of compilers that nasr has previously failed to build.
        Returns:
            list[tuple[CompilerProject, Commit]]:
                A list of (compiler, commit) pairs that nasr has previously failed to build.
        """
        return [
            (p, Commit(c))
    ...
    def remove_from_previously_failed_to_build(
        ...
        """Remove a commit from the list of that nasr has previously failed to build.
        Args:
            project (CompilerProject):
                The compiler project (GCC or LLVM)
            commit (Commit):
    ...
    def clear_previously_failed_to_build(self) -> None:
        """Delete all the commits from the list of that nasr has previously failed to build."""
        with self.con:
