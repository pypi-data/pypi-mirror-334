...
class Grid:
    ...
    def get_attribute_values_by_coords(grid_data, grid_xx, grid_yy, coords):
        if np.any(np.isnan(coords)):
            ...
            x = coords[0]
            y = coords[1]
            dx = (np.max(grid_xx) - np.min(grid_xx)) / grid_xx.shape[1]
            dy = (np.max(grid_yy) - np.min(grid_yy)) / grid_yy.shape[0]
            x_idx = int((x - np.min(grid_xx)) // dx) + 1
            y_idx = int((y - np.min(grid_yy)) // dy) + 1
            return grid_data[y_idx * x_idx, :]
