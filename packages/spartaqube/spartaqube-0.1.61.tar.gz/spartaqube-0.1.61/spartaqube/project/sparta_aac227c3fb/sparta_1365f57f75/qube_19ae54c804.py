...
from project.sparta_1df4957b8d.sparta_f6456b32f2.qube_4ed691eaf9 import sparta_2623027b72
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_8f4ae177c6(json_data,user_obj):
...
def sparta_6625f8d84f(message,typeCase=0,companyName=_E):
	...
	if E.count()>0:
		...
		A.send()
def sparta_24b5354b3d(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_2623027b72(model_to_dict(D));del A[G];A[_F]=naturalday(parser.parse(str(A[_F])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_2623027b72(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_d5c684ccd0(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_2623027b72(model_to_dict(L));A[_F]=naturalday(parser.parse(str(A[_F])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_7acd9c4b32(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_6625f8d84f(F,A.type_ticket,_E)
...
def sparta_a58a3c352b(json_data,user_obj):
	...
	return{_A:1}
def sparta_c83451f2c9(json_data):
	...
	return{_A:1}
def sparta_403dbf2780(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}