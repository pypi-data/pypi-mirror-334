...
def sparta_01b9e9686c():return str(uuid.uuid4())
def sparta_f06b776694(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_fafd7a71f4(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
...
def sparta_7b3a230bfe(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_fafd7a71f4(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_49534f0809(B)
	...
	return{_A:1}
def sparta_553f0678c4(json_data,userObj):
	...
	return{_A:1}
def sparta_2935f1d06d(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_fafd7a71f4(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_ff88a233ce(json_data,userObj):A=json_data[_B];B=qube_46369769b7.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_3d4ddb517e(json_data,userObj):
	A=json_data
	try:
...
def sparta_f07953f9c1(json_data,userObj):
	...
	return{_A:1}
def sparta_48d12220dd(json_data,userObj):
	...
	return{_A:1}
def sparta_0c56324bd7(json_data,userObj):
...
def sparta_6b542b334e(json_data,userObj):
	...
	return{_A:-1}
def sparta_1579ffc29d(json_data,userObj):
