...
def sparta_a78ce73af4(user_obj):
	...
	return B
def sparta_1231e4e7dd():B=sparta_978905e1c8();A=os.path.join(B,'sq_venv');os.makedirs(A,exist_ok=_C);return A
def sparta_1576f09d00(env_name):
	A=env_name;B=sparta_1231e4e7dd()
	if sys.platform=='win32':C=os.path.join(B,A,'Scripts','pip.exe')
	else:C=os.path.join(B,A,'bin','pip')
	return C
def sparta_289fb1b0ba(json_data,user_obj):A=sparta_1231e4e7dd();B=[B for B in os.listdir(A)if os.path.isdir(os.path.join(A,B))];return{_A:1,'available_venvs':B}
def sparta_bf0dc52660(json_data,user_obj):
	B=sparta_1231e4e7dd();A=json_data[_D];C=os.path.join(B,A)
	try:
		subprocess.run([sys.executable,'-m','venv',C],check=_C);D=['cloudpickle']
		for E in D:
			F=sparta_1576f09d00(A);G=f"pip install {E}";H=G.replace('pip',F);I=subprocess.Popen(H,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_C)
...
def sparta_fd59c0b64d(json_data,user_obj):
	...
	L=qube_f776bbbf13.sparta_6176601ad1(C,B);return{_A:1}
def sparta_f9c55bc867(json_data,user_obj):
	B=user_obj;E=json_data[_H];C=Dashboard.objects.filter(dashboard_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.dashboard_id;F=sparta_a78ce73af4(B)
	...
	return{_A:1}
def sparta_a0689b24f2(json_data,user_obj):
	C=user_obj;A=json_data;D=A[_H];E=A[_J]
	if str(D)!=_F:sparta_f9c55bc867(A,C)
	if str(E)!=_F:sparta_873ec736e7(A,C)
	F=sparta_1231e4e7dd();G=A[_D];H=os.path.join(F,G)
...
def sparta_de1e0c89c7(json_data,user_obj):
	B=json_data[_D];C=sparta_1576f09d00(B);A=[]
	try:
...
def sparta_1eed663a5c(env_name,project_path):
	A=os.path.join(project_path,'requirements.txt');B=sparta_1576f09d00(env_name)
	try:
		with open(A,'w')as C:subprocess.run([B,'freeze'],stdout=C)
		return{_A:1}
	except Exception as D:return{_A:-1,_E:str(D)}
def sparta_770ba527a1(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.developer_id;G=sparta_a78ce73af4(C)
	...
	L=qube_f776bbbf13.sparta_6176601ad1(B,C);return{_A:1}
def sparta_873ec736e7(json_data,user_obj):
	B=user_obj;E=json_data[_J];C=Developer.objects.filter(developer_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.developer_id;F=sparta_a78ce73af4(B)
	...
	return{_A:1}
def sparta_74aa5d4d0a(json_data,user_obj):
...
def sparta_05a937c035(json_data,user_obj):
	...
	return{_A:1}
def sparta_fa5b3ef3af(json_data,user_obj):
	C=json_data;B=user_obj;from project.sparta_aac227c3fb.sparta_27abf0b36c import qube_f776bbbf13 as J;F=C[_L];D=Kernel.objects.filter(kernel_manager_uuid__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.kernel_manager_uuid;G=sparta_a78ce73af4(B)
	...
	M=J.sparta_6176601ad1(C,B);return{_A:1}
def sparta_9653bc59f3(json_data,user_obj):
	B=user_obj;E=json_data[_L];C=Kernel.objects.filter(kernel_manager_uuid__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.kernel_manager_uuid;F=sparta_a78ce73af4(B)
	...
	return{_A:1}
def sparta_92276b8d62(json_data,user_obj):
	B=user_obj;A=json_data;logger.debug(_O);logger.debug(A);D=A[_H];E=A[_J];F=A[_K];G=A[_L]
	if str(D)!=_F:return sparta_528299541b(A,B)
	if str(E)!=_F:return sparta_89b5bf8a6e(A,B)
	if str(F)!=_F:return sparta_23627c0e28(A,B)
	if str(G)!=_F:return sparta_b5d9347463(A,B)
	H=A[_D];C=A[_G]
	if A[_P]:C=os.path.join(C,_M)
	return sparta_1eed663a5c(H,C)
def sparta_528299541b(json_data,user_obj):
	C=user_obj;B=json_data;F=B[_H];J=B[_D];K=B[_G];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_a78ce73af4(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:return sparta_1eed663a5c(J,K)
	return{_A:1}
def sparta_89b5bf8a6e(json_data,user_obj):
	...
	if E.count()==1:
		A=E[E.count()-1];G=A.developer_id;H=sparta_a78ce73af4(C)
		...
		if I:return sparta_1eed663a5c(K,D)
	return{_A:1}
def sparta_23627c0e28(json_data,user_obj):
	...
	if D.count()==1:
		...
		if H:return sparta_1eed663a5c(J,K)
	return{_A:1}
def sparta_b5d9347463(json_data,user_obj):
	A=json_data;from project.sparta_aac227c3fb.sparta_27abf0b36c import qube_f776bbbf13 as B;C=A[_L];D=A[_D];E=A[_G];F=B.sparta_4f7a5a6d1f(user_obj,C)
	if F is _I:return sparta_1eed663a5c(D,E)
	return{_A:1}
def sparta_d8db4c5274(json_data,user_obj):
	B=json_data;logger.debug(_O);logger.debug(B);E=B[_D];C=os.path.join(sparta_1231e4e7dd(),E);logger.debug('venv_path');logger.debug(C);A=sparta_9a9cd2cf6a(B[_G])
