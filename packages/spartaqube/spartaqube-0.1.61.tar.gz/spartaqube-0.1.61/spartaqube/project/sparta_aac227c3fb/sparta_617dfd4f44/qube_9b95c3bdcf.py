...
from project.sparta_aac227c3fb.sparta_30f5cbfc95 import qube_a01da2006c as qube_a01da2006c
def sparta_af8e5abfb7(json_data,userObj):
...
def sparta_bc0522e3fe(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_02cd768c6f.sparta_c02f9aa26f(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_02cd768c6f.sparta_c02f9aa26f(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_97f5b2bef4(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_02cd768c6f.sparta_c02f9aa26f(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_02cd768c6f.sparta_c02f9aa26f(A);C=[A.userGroup for A in H]
...
def sparta_35c4eed2e6(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_02cd768c6f.sparta_c02f9aa26f(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_02cd768c6f.sparta_c02f9aa26f(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_02cd768c6f.sparta_c02f9aa26f(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_14ce3067d0(json_data,user_obj):
