...
from project.sparta_1df4957b8d.sparta_f6456b32f2.qube_4ed691eaf9 import sparta_6f00ce0a1c,sparta_bcc9b0b100,sparta_fbab38c7f1
from project.sparta_aac227c3fb.sparta_24405b5ac7.qube_651f37dbb3 import sparta_921332448f
from project.logger_config import logger
def sparta_04dd2ecf2e(json_data,user_obj):
...
def sparta_d7d6b80942(message,typeCase=0,companyName=_G):
	...
	if C.count()>0:
		...
		A.send()
def sparta_d4675320c1(json_data,userObj):
	...
	return B
def sparta_d08c389c24(json_data,userObj):
	...
	return A
def sparta_d785cde231(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_921332448f(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_138ddfcac4():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_2c944389b0(json_data):
	A=json_data;C=A[_F];E=A[_H];B=A[_I];F=A[_R]
	if not sparta_fbab38c7f1(E):return{_A:-1,_B:_J}
...
def sparta_6409a3a39d(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_H];G=sparta_6f00ce0a1c(E)
	if G[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_bcc9b0b100(F):return{_A:-1,_B:_J}
...
def sparta_cd0fd5ab3b(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_H];H=A[_U];E=A[_I];I=A[_R];J=sparta_6f00ce0a1c(F)
	if J[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_bcc9b0b100(G):return{_A:-1,_B:_J}
