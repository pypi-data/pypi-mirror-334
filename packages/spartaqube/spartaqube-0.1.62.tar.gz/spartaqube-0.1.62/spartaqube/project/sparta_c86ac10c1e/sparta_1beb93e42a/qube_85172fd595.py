...
def sparta_8c74ef8900(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_6d69a7bc1b(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_a70a5fd95a.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_c47ba47c16(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();logger.debug('realtime_output 1');logger.debug(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);logger.debug('realtime_output 2');logger.debug(B)
def sparta_d5a8b4b528(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_6d69a7bc1b(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_ea5bc68e95(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_6d69a7bc1b(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_306ba1d886(repo,user_obj):
...
def sparta_75d33793de(webSocket,json_data,user_obj):
...
def sparta_9540833344(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_5e9eb10115(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_8c74ef8900(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_306ba1d886(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_306ba1d886(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_c47ba47c16(K);os.chdir(L)
	return{_A:1}
def sparta_495a0aa5d2(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_8c74ef8900(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_306ba1d886(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_82cc04bef0(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_8c74ef8900(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_306ba1d886(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_c47ba47c16(E);os.chdir(F)
	return{_A:1}
@sparta_d5a8b4b528
def sparta_40a226de85(json_data,user_obj):return{_A:1}
def sparta_9d70f7edaa(json_data,user_obj):
	B=json_data[_C];C=sparta_8c74ef8900(B)
	if C:A=git.Repo(B);sparta_306ba1d886(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_976ada91e6(json_data,user_obj):
	B=json_data[_C];C=sparta_8c74ef8900(B)
	if C:A=git.Repo(B);sparta_306ba1d886(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_68dd4040c6(json_data,user_obj):
	A=json_data[_C];C=sparta_8c74ef8900(A)
	if C:
		B=git.Repo(A);sparta_306ba1d886(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_9767d34c3e(json_data,user_obj):A=json_data[_C];B=sparta_8c74ef8900(A);return{_A:1,_L:B}
def sparta_173134caa9(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_8c74ef8900(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_edd03466ed(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_8c74ef8900(C)
	if I:
		B=git.Repo(C);sparta_306ba1d886(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_9bcde34944(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_8c74ef8900(C)
	if E:A=git.Repo(C);sparta_306ba1d886(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_d5a8b4b528
def sparta_0dafa2cd1e(json_data,user_obj):return{_A:1}
def sparta_777b9e54d3(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_8c74ef8900(C)
	if E:
		B=git.Repo(C);sparta_306ba1d886(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_9c19c9fa6c(json_data,user_obj):return{_A:1}
def sparta_b416dbb8ea(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_8c74ef8900(E)
	if F:
		A=git.Repo(E);sparta_306ba1d886(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_4b85f96c0d(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_8c74ef8900(B)
	if E:
		C=git.Repo(B);sparta_306ba1d886(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_4b74692ee1(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_8c74ef8900(C)
	if F:
		A=git.Repo(C);sparta_306ba1d886(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_7d4f57b30c(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_8c74ef8900(A)
	if F:
		E=git.Repo(A);sparta_306ba1d886(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_8a307ec48e(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_8c74ef8900(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_306ba1d886(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}