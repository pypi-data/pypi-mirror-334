...
def sparta_26b96afe1f(path):A=Path(path).resolve();return str(A).replace('\\','\\\\')
def sparta_ab3c7c5810():
	B='virtual_env_name';C=hasattr(sys,'real_prefix')or hasattr(sys,'base_prefix')and sys.base_prefix!=sys.prefix
	if C:A=sys.prefix;D=os.path.basename(A);return{_A:True,B:D,_B:sparta_26b96afe1f(A)}
	else:return{_A:False,B:None,_B:None}
def sparta_588a9978d8(bat_file_path,shortcut_path,icon_path):
...
def sparta_7f7126a67d(shell_script_path,shortcut_name,icon_path):
...
def sparta_fdfb107479(shortcut_name,shell_script_path,icon_path):
...
def sparta_a964401b8e(json_data,user_obj):
	X='SpartaQube Launcher';W='SpartaQube.sh';V='SpartaQube.lnk';U='favicon512.ico';T='Icon';S='images';R='assets';L='errorMsg';F='res';M=qube_6e3dd19911.sparta_e9053d7a5f(json_data,user_obj)
	if M[F]==1:
		Y=M['token'];B=sparta_58b54fa947()['api'];A=sparta_26b96afe1f(os.path.join(B,'spartaqube_exec.py'))
		if os.path.exists(A):
			try:os.remove(A)
...
"""
		...
		G=sparta_26b96afe1f(os.path.join(B,'spartaqube_launcher.py'))
		if os.path.exists(G):
			try:os.remove(G)
			except Exception as D:return{F:-1,L:str(D)}
		J=sparta_ab3c7c5810();logger.debug('env_info');logger.debug(J)
		if J[_A]:E=J[_B];a=sparta_26b96afe1f(os.path.join(f"{E}",'bin','python')if os.name!='nt'else os.path.join(f"{E}",'Scripts','python.exe'));N=sparta_26b96afe1f(os.path.join(f"{E}",'bin/activate')if os.name!='nt'else os.path.join(E,'Scripts/activate.bat'));O=f'''
import os
import sys
import subprocess
import platform
...
'''
		...
		P=sparta_58b54fa947()['spartaqube_path']
		if conf_settings.IS_DEV:H=sparta_26b96afe1f(os.path.join(P,'static',R,S,T,U))
		else:H=sparta_26b96afe1f(os.path.join(P,'staticfiles',R,S,T,U))
		Q=platform.system()
		if Q==_C:
			C=sparta_26b96afe1f(os.path.join(B,'SpartaQube.bat'));K=sparta_26b96afe1f(os.path.join(B,V))
			if os.path.exists(K):
				try:os.remove(K)
				except Exception as D:return{F:-1,L:str(D)}
			sparta_588a9978d8(C,K,H);b=os.path.join(os.path.expanduser('~'),'Desktop');c=os.path.join(b,V);sparta_588a9978d8(C,c,H)
		elif Q=='Darwin':C=sparta_26b96afe1f(os.path.join(B,W));sparta_7f7126a67d(shell_script_path=C,shortcut_name=X,icon_path=H)
		else:C=sparta_26b96afe1f(os.path.join(B,W));sparta_fdfb107479(shell_script_path=C,shortcut_name=X,icon_path='/path/to/your/icon.png')
	return{F:1}