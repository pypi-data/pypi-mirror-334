...
def sparta_c59ce69502(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_8f38ae2875(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_3f4f684e58(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_8f38ae2875(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_bbee1fe47a(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_8f38ae2875(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_8f38ae2875(currentPath,typeexplorer=None):
...
def sparta_e8bca96a29(currentPath):
