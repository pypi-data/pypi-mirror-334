...
from project.sparta_c86ac10c1e.sparta_45f849ee34.qube_9cbbe19431 import convert_to_dataframe,convert_dataframe_to_json,sparta_26b96afe1f
...
def sparta_e01a6ec1c4(user_obj):from project.sparta_c86ac10c1e.sparta_ec46e9a9b4.qube_6e3dd19911 import sparta_9de5364c58 as A;return A(user_obj)
def sparta_35acb13806(user_obj):
	A=qube_9ad248f4ee.sparta_ca0a23ce8e(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_ab52fc3957(json_data,user_obj):
	D=user_obj;E=sparta_35acb13806(D)
...
def sparta_f52e60dd1b():return{_A:1,'available_engines':qube_d09eab44e0.sparta_f52e60dd1b()}
def sparta_0192e74096(json_data,user_obj):
	...
	return{_A:1}
def sparta_48ac96ede3(json_data):
...
def sparta_59b51cacc7(json_data):
...
def sparta_660bea1299(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_35acb13806(C)
...
def sparta_af2bc290d0(json_data,user_obj):
	...
	return{_A:1}
def sparta_cddc00d2b9(package_name):
...
def sparta_f4ad0b3a86(json_data,user_obj):
	B=_D;C=[];D=json_data['pip_cmds']
	for E in D:
		A=sparta_cddc00d2b9(E)
...
def sparta_614c8a35d6(obj):
	A=obj
	if isinstance(A,dict):
		B=DotDict()
		for(C,D)in A.items():B[C]=sparta_614c8a35d6(D)
		return B
	elif isinstance(A,list):return[sparta_614c8a35d6(A)for A in A]
	else:return A
def sparta_ade2bd8882(json_data,user_obj):
	B=json_data;D=B.get(_AK,_D)
	if D:A=sparta_614c8a35d6(B)
	else:
		E=B[_H];A=sparta_32379c29a3(E,user_obj)
...
def sparta_86be5d4c3a(json_data,user_obj):
	A=json_data;G=A[_H];E=A[_O];H=int(A.get(_p,'0'))==1;B=[];C=sparta_32379c29a3(G,user_obj)
...
def sparta_baab5d32fc(json_data,db_connector_obj):
	B=db_connector_obj
...
def sparta_8973ce3eb0(json_data,user_obj):
	A=json_data;print(_AL);print(A);H=A.get(_AK,_D);I=A[_H];C=A.get(_O,_B);F=int(A.get(_p,'0'))==1;G=int(A.get('previewTopMod','0'))==0
	if H:D=sparta_614c8a35d6(A)
	else:
		D=sparta_32379c29a3(I,user_obj)
		if D is _B:return{_A:-1,_E:_g}
	sparta_baab5d32fc(A,D);B=Connector(db_engine=D.db_engine);B.init_with_model(D)
	if F is not _B:
		if F:
			J=A[_A0]
...
def sparta_ee5b3612bb(json_data,user_obj):
	A=json_data;F=A[_H];D=A.get(_O,'');G=int(A.get(_p,'0'))==1;K=A.get(_N,_B);B=sparta_32379c29a3(F,user_obj)
	if B is _B:return{_A:-1,_E:_g}
	sparta_baab5d32fc(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if G:
		H=A[_A0]
...
def sparta_2f925ea1e4(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_5aef38c0cd(json_data,user_obj):
	A=json_data;D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_32379c29a3(D,user_obj)
	if B is _B:return{_A:-1,_E:_g}
	sparta_baab5d32fc(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_567dcb5ece(json_data,user_obj):
	A=json_data;M=A[_M];D=_B
	if M:D=A[_s];D=qube_bacc7c2bb0.sparta_92e26cc746(D)
...
def sparta_48786d0103(json_data,user_obj):
	...
	if H.count()>0:
		...
		if I.count()>0:
			...
			if L.is_admin or L.has_write_rights:
				P=B[_M];C=_B
				if P:C=B[_s];C=qube_bacc7c2bb0.sparta_92e26cc746(C)
				D=_B;E=B.get(_AM,_B)
				if E is not _B:
					E=E.split(',')[1];R=base64.b64decode(E)
					try:
						S=os.path.dirname(__file__);T=os.path.dirname(os.path.dirname(os.path.dirname(S)));M=os.path.join(T,'static',_AN,'widget');os.makedirs(M,exist_ok=_C)
	...
	return{_A:1}
def sparta_07c2574d68(json_data,user_obj):0
def sparta_6c2373cfbe(json_data,user_obj):
	D=user_obj;F=sparta_35acb13806(D)
...
def sparta_8e9d0f9040(json_data,user_obj):
	...
	if I.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				J=Q[0];e=J.user;f=sparta_e01a6ec1c4(e);R=J.user;K=[];A=J.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if L:K.append(B[b])
							...
							if _I in B:
								h=B[_H];S=sparta_32379c29a3(h,R);T=[]
...
def sparta_f366dba42c(json_data,user_obj):
	...
	return{_A:1}
def sparta_76c9394c6f(user_obj,widget_id):
...
def sparta_6004b68753(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_35acb13806(K)
...
def sparta_db814365d5(json_data,user_obj):
	...
	return{_A:1}
def sparta_45bf929069(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_bacc7c2bb0.sparta_f94244b9ff(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_270d2b65c2(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_K:A}
					...
					try:
						if qube_bacc7c2bb0.sparta_f94244b9ff(A.widget_password_e)==F:return{_A:1,_K:A}
	...
	return{_A:-1}
def sparta_ced2f3469d(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_35acb13806(C)
	...
	return{_A:1,_i:_D}
def sparta_835f92cb3f(plot_db_chart_obj):
	...
	return C
def sparta_485313e227(json_data,user_obj):
	B=user_obj;D=sparta_35acb13806(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_M];C=_B
				if N:C=B[_s];C=qube_bacc7c2bb0.sparta_92e26cc746(C)
...
def sparta_1e1891bd98(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_M];C=_B
				if K:C=B[_s];C=qube_bacc7c2bb0.sparta_92e26cc746(C)
	...
	return{_A:1}
def sparta_fd0aaf1e3c(json_data,user_obj):
	...
	return{_A:1}
def sparta_11ee8e4938(user_obj):
	B=user_obj;C=sparta_35acb13806(B)
	...
	return E
def sparta_871b803299(json_data,user_obj):
	try:A=sparta_ced2f3469d(json_data[_AX],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_b91003f306(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_ced2f3469d(A[_AX],B);D=C[_i]
	if D:E=C[_K];A[_G]=E.plot_chart_id;F=sparta_8e9d0f9040(A,B);return{_A:1,_F:[A[_F]for A in F[_AA]]}
	return{_A:-1}
def sparta_a1e52edece(json_data,user_obj):
	F='Error quantstats';C=user_obj;A=json_data;from project.sparta_c86ac10c1e.sparta_d3a4fd1e04 import qube_7137dfbbd6 as D;E=A['service']
	if E=='quantstats':
		try:return D.sparta_3521863478(A,C)
		except Exception as B:logger.debug(F);logger.debug(traceback.format_exc());logger.debug(F);return{_A:-1,_E:str(B)}
	elif E=='matplotlib':
		try:return D.sparta_6c0f25bf25(A,C)
		except Exception as B:logger.debug('Error matpltlib');logger.debug(traceback.format_exc());return{_A:-1,_E:str(B)}
	return{_A:1}
def sparta_f8d26bb891(json_data,user_obj):
...
def sparta_d8607f3bbc(json_data,user_obj):
