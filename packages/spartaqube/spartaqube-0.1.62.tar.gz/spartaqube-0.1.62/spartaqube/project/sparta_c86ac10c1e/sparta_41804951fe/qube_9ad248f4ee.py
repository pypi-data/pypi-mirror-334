...
def sparta_bfd980e017(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_253882140c(G,F,A,B);H={_A:1,_C:C};return H
def sparta_253882140c(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_4c6853dc9f(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_ca0a23ce8e(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_31205aa8dc(json_data,user_obj):A=sparta_4c6853dc9f(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_cc770fbc83(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_3890415ae8(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_253882140c(G,A,I,C)
	J={_A:1};return J
def sparta_2ce58d8cb2(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_2b8b95108b(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_b0cb7e4e36(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_b4665fe5ce(json_data,user_obj):
