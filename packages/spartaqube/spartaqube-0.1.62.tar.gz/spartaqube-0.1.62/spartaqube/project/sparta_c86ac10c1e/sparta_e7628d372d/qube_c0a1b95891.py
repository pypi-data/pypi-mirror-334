...
def sparta_35acb13806(user_obj):
	...
	return B
def sparta_50ffbcd461(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_58b54fa947()[_a],_K,_b)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_252bdc5808(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_H];A=sparta_26b96afe1f(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_35acb13806(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_50ffbcd461(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_3c28ec9a88(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_85172fd595.sparta_9540833344(A,user_obj)
def sparta_60d1d7ab3f(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_35acb13806(E)
...
def sparta_61296916c1(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_35acb13806(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_1f88e55cb9(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_d0197a260b(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_ebe42e1e52(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_Y];E=_D
	if G:E=A[_j];E=qube_bacc7c2bb0.sparta_92e26cc746(E)
	O=A[_Z];P=A[_I];Q=A[_Q];D=A[_H];D=sparta_26b96afe1f(D);R=A[_k];S=A[_l];G=A[_Y];T=A[_m];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_I]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_58b54fa947()[_o];M=os.path.join(D,_p,_q,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_702312bb4b());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_ebe42e1e52(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_35acb13806(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_58b54fa947()[_o];S=os.path.join(b,_p,_q,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_8930dfc93f(json_data,user_obj):
	...
	return{_B:1}
def sparta_4e45348926(json_data,user_obj):
	...
	return{_B:1}
def sparta_1f88e55cb9(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_bacc7c2bb0.sparta_f94244b9ff(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_3fdb57b0c4(json_data,user_obj):A=sparta_26b96afe1f(json_data[_H]);return sparta_9b06f9378f(A)
def sparta_1714878221(json_data,user_obj):A=sparta_26b96afe1f(json_data[_H]);return sparta_46acdf785c(A)
def sparta_05d07d728a(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_26b96afe1f(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec_DEPREC(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			H=qube_120f717b56.sparta_9f41bde041(A);I=H['cells']
			for B in I:
...
async def notebook_permission_code_exec(json_data):
	...
	return''
def sparta_5af7641cd5(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_62fe0e53f3(project_path,python_executable=_u):
...
def sparta_c96140540d():
	...
	return B
def sparta_cd7b6175ae(json_data,user_obj):
	D=sparta_26b96afe1f(json_data[_H]);A=D;B=os.path.join(sparta_58b54fa947()[_a],_K,_b);C=os.path.join(A,_O)
...
def sparta_3e70d1cf40(json_data,user_obj):
	A=sparta_26b96afe1f(json_data[_H]);A=os.path.join(A,_O);G,C,D=sparta_62fe0e53f3(A);B=_A;E=1;F=''
	if D:
...
def sparta_efefb04f6c(project_path,python_executable=_u):
...
def sparta_8d7c09e607(json_data,user_obj):
	A=sparta_26b96afe1f(json_data[_H]);A=os.path.join(A,_O);B,C=sparta_efefb04f6c(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_y:C,_E:E}
def sparta_47545a2326(json_data,user_obj):return{_B:1}
def sparta_492418f889(json_data,user_obj):return{_B:1}