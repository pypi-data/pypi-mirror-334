...
def sparta_cbf65cb9b2():return str(uuid.uuid4())
def sparta_d5735e6266(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_4fd8abb93f(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_9ff9d23bae(path):
	with open(path,'a'):os.utime(path,None)
def sparta_5d1b2268fb(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_4fd8abb93f(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_9ff9d23bae(B)
	...
	return{_A:1}
def sparta_1b5632e509(json_data,userObj):
	...
	return{_A:1}
def sparta_13e25989c6(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_4fd8abb93f(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_81bad4cc52(json_data,userObj):A=json_data[_B];B=qube_edd1fdc02e.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_81add413d9(json_data,userObj):
	A=json_data
	try:
...
def sparta_50e0fa1c4d(json_data,userObj):
	...
	return{_A:1}
def sparta_bbd67a5598(json_data,userObj):
	...
	return{_A:1}
def sparta_7071e0f8ec(json_data,userObj):
...
def sparta_5a663d2d74(json_data,userObj):
	...
	return{_A:-1}
def sparta_9b3528fcef(json_data,userObj):
