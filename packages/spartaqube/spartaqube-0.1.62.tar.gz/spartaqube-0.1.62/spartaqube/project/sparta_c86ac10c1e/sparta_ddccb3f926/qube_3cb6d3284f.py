...
from project.sparta_c86ac10c1e.sparta_e8d7ed4bc7 import qube_0727da2bca as qube_0727da2bca
def sparta_75f52ee08e(json_data,userObj):
...
def sparta_c11536040d(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_9ad248f4ee.sparta_ca0a23ce8e(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_9ad248f4ee.sparta_ca0a23ce8e(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_46fec7f645(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_9ad248f4ee.sparta_ca0a23ce8e(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_9ad248f4ee.sparta_ca0a23ce8e(A);C=[A.userGroup for A in H]
...
def sparta_ce95f2c482(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_9ad248f4ee.sparta_ca0a23ce8e(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_9ad248f4ee.sparta_ca0a23ce8e(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_9ad248f4ee.sparta_ca0a23ce8e(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_e77e99a5c5(json_data,user_obj):
