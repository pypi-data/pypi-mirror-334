...
from project.sparta_c86ac10c1e.sparta_45f849ee34.qube_953c9d598c import sparta_96781f896a,sparta_3048137f79
from project.logger_config import logger
def sparta_8d231d92e2():keygen_fernet='spartaqube-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_8cd491d9b5():keygen_fernet='spartaqube-internal-decoder-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_3f8900e223(f,str_to_encrypt):data_to_encrypt=str_to_encrypt.encode(_B);token=f.encrypt(data_to_encrypt).decode(_B);token=base64.b64encode(token.encode(_B)).decode(_B);return token
def sparta_9c0d1fbe3b(api_token_id):
	if api_token_id=='public':
		try:return User.objects.filter(email='public@spartaqube.com').all()[0]
		except:return
	try:
		f_private=Fernet(sparta_8cd491d9b5().encode(_B));api_key=f_private.decrypt(base64.b64decode(api_token_id)).decode(_B).split('@')[1];user_profile_set=UserProfile.objects.filter(api_key=api_key,is_banned=_D).all()
...
def sparta_9de5364c58(user_obj):
...
def sparta_d6497fc596(user_obj,domain_name):api_key=sparta_9de5364c58(user_obj);random_nb=str(randint(0,1000));data_to_encrypt=f"apikey@{api_key}@{random_nb}";f_private=Fernet(sparta_8cd491d9b5().encode(_B));private_encryption=sparta_3f8900e223(f_private,data_to_encrypt);data_to_encrypt=f"apikey@{domain_name}@{private_encryption}";f_public=Fernet(sparta_8d231d92e2().encode(_B));public_encryption=sparta_3f8900e223(f_public,data_to_encrypt);return public_encryption
def sparta_e9053d7a5f(json_data,user_obj):api_key=sparta_9de5364c58(user_obj);domain_name=json_data['domain'];public_encryption=sparta_d6497fc596(user_obj,domain_name);return{_A:1,'token':public_encryption}
def sparta_6ae420be97(json_data,user_obj):userprofile_obj=UserProfile.objects.get(user=user_obj);api_key=str(uuid.uuid4());userprofile_obj.api_key=api_key;userprofile_obj.save();return{_A:1}
def sparta_c63f7ea336():plot_types=sparta_96781f896a();plot_types=sorted(plot_types,key=lambda x:x['Library'].lower(),reverse=_D);return{_A:1,'plot_types':plot_types}
def sparta_435861288a(json_data):logger.debug('DEBUG get_plot_options json_data');logger.debug(json_data);plot_type=json_data['plot_type'];plot_input_options_dict=sparta_3048137f79(plot_type);plot_input_options_dict[_A]=1;return plot_input_options_dict
def sparta_c3ce8a33a1(code):
	tree=ast.parse(code)
...
def sparta_587e5a5160(json_data,user_obj):
	A='errorMsg';user_code_example=json_data['userCode'];resp=_E;error_msg=''
	try:
		logger.debug('EXECUTE API EXAMPLE DEBUG DEBUG DEBUG');api_key=sparta_9de5364c58(user_obj);core_api_path=sparta_58b54fa947()['project/core/api'];ini_code='import os, sys\n';ini_code+=f'sys.path.insert(0, r"{str(core_api_path)}")\n';ini_code+='from spartaqube import Spartaqube as Spartaqube\n';ini_code+=f"Spartaqube('{api_key}')\n";user_code_example=ini_code+'\n'+user_code_example;exec(user_code_example,globals());last_expression_str=sparta_c3ce8a33a1(user_code_example)
...
def sparta_8d6938c0aa(json_data,user_obj):
	...
	return{_A:-1}
def sparta_76c9394c6f(json_data,user_obj):widget_id=json_data['widgetId'];return qube_af72c7c492.sparta_76c9394c6f(user_obj,widget_id)
def sparta_03a40eae6d(json_data,user_obj):
	api_service=json_data['api_service']
	if api_service=='get_status':output=sparta_9e55c63f9a()
	elif api_service=='get_status_ws':return sparta_b523b72eec()
	elif api_service=='get_connectors':return sparta_b906fbb23c(json_data,user_obj)
	elif api_service=='get_connector_tables':return sparta_c31afe8354(json_data,user_obj)
	elif api_service=='get_data_from_connector':return sparta_89dbf939cb(json_data,user_obj)
	elif api_service=='get_widgets':output=sparta_9368cb373a(user_obj)
	elif api_service=='has_widget_id':return sparta_4428597f61(json_data,user_obj)
	elif api_service=='get_widget_data':return sparta_bc45643284(json_data,user_obj)
	elif api_service=='get_plot_types':return sparta_96781f896a()
	return{_A:1,_F:output}
def sparta_9e55c63f9a():return 1
def sparta_b906fbb23c(json_data,user_obj):
	A='db_connectors';keys_to_retain=['connector_id',_C,'db_engine'];res_dict=qube_af72c7c492.sparta_ab52fc3957(json_data,user_obj)
	if res_dict[_A]==1:res_dict[A]=[{k:d[k]for k in keys_to_retain if k in d}for d in res_dict[A]]
	return res_dict
def sparta_c31afe8354(json_data,user_obj):res_dict=qube_af72c7c492.sparta_ade2bd8882(json_data,user_obj);return res_dict
def sparta_89dbf939cb(json_data,user_obj):res_dict=qube_af72c7c492.sparta_8973ce3eb0(json_data,user_obj);return res_dict
def sparta_9368cb373a(user_obj):return qube_af72c7c492.sparta_11ee8e4938(user_obj)
def sparta_4428597f61(json_data,user_obj):return qube_af72c7c492.sparta_871b803299(json_data,user_obj)
def sparta_bc45643284(json_data,user_obj):return qube_af72c7c492.sparta_b91003f306(json_data,user_obj)
def sparta_467dbc77f5(json_data,user_obj):date_now=datetime.now().astimezone(UTC);session_id=str(uuid.uuid4());pickled_data=json_data['data'];NewPlotApiVariables.objects.create(user=user_obj,session_id=session_id,pickled_variables=pickled_data,date_created=date_now,last_update=date_now);return{_A:1,'session_id':session_id}
def sparta_28cf7ce685():return sparta_96781f896a()
def sparta_378f15be31():cache.clear();return{_A:1}
def sparta_b523b72eec():
	global is_wss_valid;is_wss_valid=_D
	try:
		api_path=sparta_58b54fa947()['api']
...
def sparta_15d7dc1fe0(json_data,user_obj):
	...
	if not user_obj.is_anonymous:
		api_get_widgets=sparta_9368cb373a(user_obj)
...
def sparta_48b142384b(notebook_id):
...
def sparta_826709597e(dashboard_id):
