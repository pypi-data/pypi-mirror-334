...
class PipInstallWS(WebsocketConsumer):
	...
	def receive(C,text_data):
		...
		if len(F)>0:
			G=json.loads(F);H=G['pipInstallCmd'].strip();L=G['env_name'];M=qube_f82fa51e45.sparta_a82ad50ccc(L);N=H.replace('pip',M)
			if not sparta_b84a163d88(H):A={E:-1,'errorMsg':'Invalid syntax'};B=json.dumps(A);C.send(text_data=B);return
