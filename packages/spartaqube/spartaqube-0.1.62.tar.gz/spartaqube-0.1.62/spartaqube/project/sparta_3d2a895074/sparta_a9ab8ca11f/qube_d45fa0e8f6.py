...
from project.sparta_3d2a895074.sparta_28819454f8 import qube_05f3410cc3 as qube_05f3410cc3
from project.sparta_c86ac10c1e.sparta_45f849ee34 import qube_9cbbe19431 as qube_9cbbe19431
from project.sparta_c86ac10c1e.sparta_aa64b2d163.qube_c0f12c0a99 import sparta_9dbe1c11d3
from project.sparta_c86ac10c1e.sparta_aa64b2d163.qube_0c0e513404 import sparta_fb51107614
...
from project.sparta_c86ac10c1e.sparta_ec46e9a9b4.qube_6e3dd19911 import sparta_d6497fc596,sparta_9de5364c58,get_api_key_async
class NotebookWS(AsyncWebsocketConsumer):
	...
	async def notebook_permission_code_exec(A,json_data):from project.sparta_c86ac10c1e.sparta_e7628d372d import qube_c0a1b95891 as B;return await coreNotebook.notebook_permission_code_exec(json_data)
		...
		if len(t)>0:
			...
			if E=='init-socket'or E=='reconnect-kernel'or E=='reconnect-kernel-run-all':
				...
				try:
					...
					AP=json.loads(A['execCodeInput']);x=N.join([A[Y]for A in AP]);U=W(x,A);U+=N+L;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_9dbe1c11d3(x);y=A['guiInputVarName'];AQ=A['guiOutputVarName'];AR=A[AH];n=[y,AQ,AR];X=[]
					for S in n:
				...
				try:
					z=M;d=M
					if m in A:e=A[m];A0=sparta_fb51107614(e);z=A0['assign_state_variable'];d=A0['assign_code']
					AS=json.loads(A['execCodeOutput']);A1=N.join([A[Y]for A in AS]);U=d+N;U+=z+N;U+=A1;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_9dbe1c11d3(A1)
					try:V.append(A[m][l])
				...
				try:
					...
					f+=N+g;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(f);await B.sender_kernel_obj.send_zmq_request({D:P,J:f});V=sparta_9dbe1c11d3(g)
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});await B.send(text_data=C);return
