...
async def step_impl(context):
    ...
    keys = context.semantic.typeMapKeys()
    for k in keys:
     assert k == 'JointState'

@then('check that what we get out is what we expect')
def step_impl(context):
    eitherMap: Either = context.semantic.getInsertInstance(type1,name1)
    result = eitherMap.either(lambda e: f'Error: getInsertInstance: {e}', lambda x: x)
    if eitherMap.is_left():
       msg('result of getInsertInstance '+type1+'  '+name1+' is_left()')
...
def step_impl(context):
  assert context.semantic.typeMapKeys() != None

@when('we serialize the insert')
def step_impl(context):
    flatMap: Either = context.semantic.getFlatTypeMap()
    context.flatMap = flatMap.either(lambda e: f'Error: getFlatTypeMap: {e}', lambda x: x)
...
async def step_impl(context):
   ...
   '''
   path_either : Either = await context.pathMap.pathForSegment( shoulder_pan_name)
   ...
   if path_either.is_right() and path is not None:
      ...
      redMsg(f'asserting False as pathForSegment: {path}')
      assert False
...
@async_run_until_complete
async def step_impl(context):
   await context.pathMap.setPathForSegment( shoulder_pan_name)
...
@async_run_until_complete
async def step_impl(context):
      ref_either = await context.pathMap.refForSegment(shoulder_pan_name)
      ref = ref_either.either(lambda e: f'refForSegment Error: shoulder_pan_name: {e}', lambda x: x)
...
async def step_impl(context):
    ...
    ref_either = await context.pathMap.refForSegment( shoulder_pan_name)
    parent = ref_either.either(lambda e: f'refForSegment Error: shoulder_pan_name: {e}', lambda x: x)
    greenMsg(f'parent {parent}')
    ...
    eitherMap: Either = context.semantic.getInsertInstance(type1, shoulder_pan_name)
    result = eitherMap.either(lambda e: f'Error: getInsertInstance: {e}', lambda x: x)
    if eitherMap.is_left():
       msg('result of getInsertInstance '+type1+'  '+shoulder_pan_name+' is_left()')
...
async def step_impl(context):
  ...
  eitherMap: Either = context.semantic.getInsertInstance(type1, shoulder_pan_name)
  instance_from_insert = eitherMap.either(lambda e: f'Error: getInsertInstance: {e}', lambda x: x)
...
def step_impl(context):
  ...
  eitherClearedMap: Either = context.semantic.getInsertInstance(type1, shoulder_pan_name)
...
async def step_impl(context):
   ...
   path_either : Either = await context.pathMap.pathForSegment( 'base_link')
   ...
   if path_either.is_right() and path is not None:
      ...
      eitherMap: Either = context.semantic.getInsertInstance('ToolCenterPointState', 'base_link')
      result = eitherMap.either(lambda e: f'Error: getInsertInstance: {e}', lambda x: x)
      if eitherMap.is_left():
          assert False
          redMsg(f'asserting False as pathForSegment: {path}')
...
async def step_impl(context):
   ...
   ref_either : Either = await context.pathMap.refForSegment('base_link')
...
async def step_impl(context):
   ...
   if ref_either.is_right() and ref is not None:
      ...
      async for index in async_iter:
         greenMsg(f'narration index {index}')
