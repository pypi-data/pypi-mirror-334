...
class TextFormatter:
    ...
    group_3d_by_layer: bool = field(default=True, metadata={'description': "对3D数组分层展示"})

    def array_1d_to_lines(self, array_1d: List[Union[float, int]]) -> List[str]:
        """
        ...
        return self.__to_lines(result, max_len)

    def array_2d_to_lines(self, array_2d: List[List[Union[float, int]]]) -> List[str]:
        """
    ...
    def array_3d_to_lines(self, array_3d: List[List[List[Union[float, int]]]]) -> List[str]:
        """
        ...
        array_1d_sum = []
        #分层展示
        if self.group_3d_by_layer:
            num_layers = np.array(array_3d).shape[0]  # 层数
            for i in range(num_layers):
                array_1d_sum.extend(self.array_2d_to_lines(array_3d[i]))
        #所有数据合并一个块后展示
        else:
    ...
    def arrays_to_lines(self, arrays: List[Array]) -> List[str]:
        ...
        blocks = []
        for array in arrays:
            items = [array.array_name, array.unit_name]
            items.extend(array.data)
            blocks.append(items)

        return self.__to_blocks(blocks)

    def str_array_to_lines(self, values: List[Any]) -> List[str]:
        return self.__to_lines(values, self.pad_length)
