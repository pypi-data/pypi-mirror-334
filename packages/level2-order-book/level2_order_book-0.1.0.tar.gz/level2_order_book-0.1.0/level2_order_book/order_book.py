...
class OrderBook:
    ...
    def add_order(self, order):
        ...
        if order.side == Direction.BUY:
            ...
            logger.info(f"添加买单: {order}, 当前该价位聚合量: {self.buy_levels[order.price]}")
        elif order.side == Direction.SELL:
            self.sell_levels[order.price] = self.sell_levels.get(order.price, 0) + order.volume
            logger.info(f"添加卖单: {order}, 当前该价位聚合量: {self.sell_levels[order.price]}")
        else:
    ...
    def cancel_order(self, order):
        ...
        if order.side == Direction.BUY:
            if order.price in self.buy_levels:
                ...
                logger.info(f"撤销买单: {order}, 撤单后该价位剩余量: {self.buy_levels.get(order.price, 0)}")
            ...
            if order.price in self.sell_levels:
                ...
                logger.info(f"撤销卖单: {order}, 撤单后该价位剩余量: {self.sell_levels.get(order.price, 0)}")
    ...
    def reduce_order(self, order_id, volume):
        ...
        logger.info(f"订单 {order_id} 扣减数量 {reduce_qty}")
        return reduce_qty
    ...
    def process_trade(self, trade):
        ...
        if ttype == TrdType.UNKNOWN:
            ...
            if buy_order_no is not None and sell_order_no is not None:
                ...
                logger.warning("成交报文要求同时包含买方与卖方订单号")
        else:
