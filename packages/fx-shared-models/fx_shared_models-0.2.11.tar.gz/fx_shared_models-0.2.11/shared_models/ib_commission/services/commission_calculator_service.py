...
class CommissionCalculatorService:
    ...
    def _calculate_rule_based_distribution(cls, deal_ticket, client_mapping, 
        ...
        for ib_id, agreement in ib_agreements.items():
            ...
            result = cls._find_applicable_rules(
                ...
                symbol='*',
                order_type='*',  # This is not used for filtering but kept for API compatibility
                customer=client_mapping.customer,
                account=client_mapping.account,
                **rule_filter
            )
    ...
    def _find_applicable_rules(cls, ib_id, mt5_account_id, symbol, order_type, customer=None, account=None, **kwargs):
        ...
        Args:
            ...
            order_type: The order type (not used for filtering as the model doesn't have this field)
        ...
        if mt5_account_id:
            # Try to find account-specific mapping first
            client_mapping = ClientIBMapping.objects.filter(
                mt5_login=mt5_account_id,
                customer=customer
            ).first()
            logger.info(f"client_mapping after checking mt5_account_id: {client_mapping}")
        # PRIORITY 2: If no account-specific mapping, try to find customer mapping
        if not client_mapping and customer:
            client_mapping = ClientIBMapping.objects.filter(
                customer=customer
            ).first()
            logger.info(f"Client mapping after checking customer: {client_mapping}")
        ...
        logger.info(f"Account-specific agreements: {account_agreements}")
        ...
        logger.info(f"Client mapping agreement ID: {client_mapping.agreement.id}")
        ...
        if client_mapping and client_mapping.agreement_id:
            ...
            print(f"rules_query: {rules_query}")
            # Apply additional filters if provided
            if kwargs:
                rules_query = rules_query.filter(**kwargs)
