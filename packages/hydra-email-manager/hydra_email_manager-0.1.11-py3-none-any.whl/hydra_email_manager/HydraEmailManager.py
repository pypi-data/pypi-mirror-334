...
class HydraEmailManager:
    ...
    def baixar_emails(self, user_email, folder_id, is_read=None, file_format="eml", subject_filter=None, from_filter=None, order_by=None, limit=None):
        if self.headers:
            ...
            if from_filter:
                from_filter_escaped = urllib.parse.quote(from_filter)
                filters.append(f"from/emailAddress/address eq '{from_filter_escaped}'")
            if filters:
