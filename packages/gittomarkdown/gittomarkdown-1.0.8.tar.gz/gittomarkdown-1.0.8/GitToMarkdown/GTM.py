...
class GitToMark:
    ...
    config_path = pkg_resources.resource_filename('GitToMarkdown',"GitToMarkdown/conf.cfg")
    ...
    def __init__(self, urls: List["str"]|str,ssh=False) -> None:
        ...
        self.ssh=ssh
        if ssh:
            if not self.config["Default"]["Shh_key_path"]:
                raise SSH_key_not_set
            x=subprocess.run(f'ssh -i "{self.config["Default"]["Shh_key_path"]}" -T git@github.com',text=False)
            if x.returncode!=1:
        ...
        with ThreadPoolExecutor(int(self.config["Default"]["Threads"])) as DownloadPool:
    ...
    def config(self,threads=10,ssh_path=""):
        self.config["Default"]["THREADS"]=str(threads)
        self.config["Default"]["Shh_key_path"]=ssh_path
    @property
    def store(self):
        Zipper(self.UID)
    ...
    @property
    def generate(self):
        with ThreadPoolExecutor(int(self.config["Default"]["THREADS"])) as pool:
            i = 0
