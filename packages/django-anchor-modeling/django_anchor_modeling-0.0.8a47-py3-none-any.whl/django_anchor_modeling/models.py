...
class TransactionBackedModel(models.Model):
    ...
    def delete(self, *args, transaction: Union[int, "Transaction"] = None, **kwargs):
        ...
        # Store the TransactionBackedModels to be deleted separately, excluding self
        to_delete = {}
        for model, instances in collector.data.items():
            if is_transaction_backed_model(model):
                to_delete[model] = [obj for obj in instances if obj != self]

        # First delete all related TransactionBackedModels using their delete() method
        for model, instances in to_delete.items():
            for obj in instances:
                obj.delete(transaction=transaction_before_delete)

        # Now delete self normally, which will cascade delete any remaining non-TransactionBackedModels
