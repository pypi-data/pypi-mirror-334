...
class ValidationLayer:
	...
	async def handle_post_config_request(G,data_path,query_dict,request_body):
		V="' must contain one element.";U="Input 'list' node '";T="' not a 'list' node.";S="Input node '";H=data_path;D=request_body;B=query_dict
		...
		try:W=G.dm.parse_resource_id(H)
		except yangson.exceptions.NonexistentSchemaNode as A:raise NonexistentSchemaNode('Unrecognized schema path for parent node: '+H)from A
		try:I=G.inst.goto(W)
		except yangson.exceptions.NonexistentInstance as A:raise ParentNodeNotFound(_O+H+_F)from A
		C=next(iter(D))
		if':'not in C:raise InvalidInputDocument(_P)
		X,F=C.split(':');M=I.schema_node;J=M.get_child(F,X)
		...
		if isinstance(J,yangson.schemanode.SequenceNode):
			if isinstance(J,yangson.schemanode.ListNode):
				Y=J.keys[0];Z=Y[0]
				if not isinstance(D[C],list):raise InvalidInputDocument(S+F+T)
				if len(D[C])!=1:raise InvalidInputDocument(U+F+V)
				N=D[C][0];Q=N[Z]
			...
			if H==_A:E=C;K=_A+E+'='+quote(Q,safe='')
			else:E=F;K=H+_A+E+'='+quote(Q,safe='')
			try:a=G.dm.parse_resource_id(K)
			...
			for O in B:
				...
				if O==_B:
					try:b=G.dm.parse_resource_id(B[_B])
					...
					if H!=B[_B].rsplit(_A,1)[0]:raise InvalidQueryParameter(_I+B[_B]+') is not a sibling of the target node ('+K+').')
			try:L=I[E]
			...
			if len(L.value)==0:
				try:c=L.update([N],raw=_D)
				except yangson.exceptions.RawMemberError as A:raise UnrecognizedInputNode('Incompatable node data. '+str(A))from A
				P=c.up()
			else:d=L[-1];P=d.insert_after(N,raw=_D).up()
		else:
			if len(B)>0 and len(B)==1 and'sleep'not in B:raise UnrecognizedQueryParameter(_W)
			if H==_A:E=C
			else:E=F
			if E in I:raise NodeAlreadyExists('Node "'+E+'" already exists.')
			try:
				if M.ns is _E:P=I.put_member(C,D[C],raw=_D).up()
				else:P=I.put_member(F,D[C],raw=_D).up()
