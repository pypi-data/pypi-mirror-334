...
_B='Accept'
_A=None
import re,json,base64,datetime,textwrap,importlib.resources as importlib_resources,xml.etree.ElementTree as ET
...
async def insert_notification_log_record(nvh,notif_payload):
	H='notification';C=notif_payload;B=nvh;D=datetime.datetime.now(datetime.UTC).replace(microsecond=0);F=D.strftime(_R);D=datetime.datetime.fromisoformat(F.replace('Z','+00:00'));E='/yangcore:notification-log';I={'yangcore:notification-log-record':{_G:D,H:C}};await B.dal.handle_post_opstate_request(E,I)
	...
	G=J['yangcore:relay-notification-log-record-callout'];E=_S+G;A=await B.dal.handle_get_config_request(E,{});A=A[_T][0];assert G==A[_C];K=A[_H][_U];L=A[_H][_V];P=next(iter(C));M={'notification-log-record':{_G:F,H:C}};N=_A;O=await B.plugins[K][_W][L](M,N);assert O is _A
