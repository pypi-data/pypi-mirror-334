...
_i='operation-failed'
_h='data-exists'
_g='missing-attribute'
_f='Unable to parse "input" JSON document: '
_e='malformed-message'
_d='text/plain'
_c='yangcore:'
_b='reference-statistics'
_a='": '
_Z='cert-data'
_Y='\\g<1>'
_X='.*plugins/plugin=([^/]*).*'
...
class NativeViewHandler(RouteHandler):
	...
	def __init__(A,_dal,_loop):
		...
		for K in A.dal.ref_stat_collectors:
			assert K.endswith(_b);L,T=K.rsplit(_B,1)
			if T.startswith(_c):A.register_create_callback(L,_handle_ref_stat_parent_created_yc)
	...
	async def _check_auth(B,request,data_path):
		P='No authorization required for fresh installs.';O='/yangcore:users/user';L='access-denied';K='failure';J='success';G='comment';E='outcome';D=request;assert data_path is not _A;A={};A['timestamp']=datetime.datetime.now(datetime.UTC).replace(microsecond=0);A['source-ip']=D.remote
		...
		if M is _A:
			...
			A[E]=K;A[G]='No authorization specified in the HTTP header.';await utils.insert_audit_log_record(B.dal,B.plugins,A);C=web.Response(status=401);F=utils.gen_rc_errors(_C,L);C.text=json.dumps(F,indent=2);return C
		...
		try:S=await B.dal.handle_get_config_request(R,{})
			...
			H=await B.dal.num_elements_in_list(O)
			if H==0:A[E]=J;A[G]=P;await utils.insert_audit_log_record(B.dal,B.plugins,A);return web.Response(status=200)
			A[E]=K;A[G]='Unknown user: '+I;await utils.insert_audit_log_record(B.dal,B.plugins,A);C=web.Response(status=401);F=utils.gen_rc_errors(_C,L);C.text=json.dumps(F,indent=2);return C
		N=S['yangcore:password'];assert N.startswith('$5$')
		if not sha256_crypt.verify(Q,N):A[E]=K;A[G]='Password mismatch for user '+I;await utils.insert_audit_log_record(B.dal,B.plugins,A);C=web.Response(status=401);F=utils.gen_rc_errors(_C,L);C.text=json.dumps(F,indent=2);return C
	...
	async def handle_get_restconf_root(D,request):
		...
		assert isinstance(B,str);C=B;assert C!=_d;F=utils.Encoding[C.rsplit('+',1)[1].upper()];A=web.Response(status=200);A.content_type=C
	...
	async def handle_get_opstate_request_lower_half(E,data_path,query_dict):
		...
		async with E.fifolock(Read),contextlib.nullcontext():
			...
			except dal.NodeNotFound as C:A=web.Response(status=404);D=utils.gen_rc_errors(_C,_I,error_message=str(C));A.text=json.dumps(D,indent=2);return A,_A
			except NotImplementedError as C:assert _L;A=web.Response(status=501);D=utils.gen_rc_errors(_D,_M,error_message=str(C));A.text=json.dumps(D,indent=2j);return A,_A
	...
	async def handle_get_config_request_lower_half(E,data_path,query_dict):
		...
		async with E.fifolock(Read),contextlib.nullcontext():
			if not os.environ.get(_G):
				...
				except val.InvalidDataPath as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A,_A
				except val.NonexistentSchemaNode as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A,_A
				except val.NodeNotFound as B:A=web.Response(status=404);C=utils.gen_rc_errors(_C,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A,_A
			if os.environ.get(_R)and _H in D:await asyncio.sleep(int(D[_H]))
			try:G=await E.dal.handle_get_config_request(F,D)
			except dal.NodeNotFound as B:A=web.Response(status=404);C=utils.gen_rc_errors(_C,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A,_A
	...
	async def handle_post_config_request(A,request):
		...
		try:H=await B.json()
		except json.decoder.JSONDecodeError as I:C=web.Response(status=400);J=utils.gen_rc_errors(_C,_e,error_message=_f+str(I));C.text=json.dumps(J,indent=2);return C
	...
	async def handle_post_config_request_lower_half(D,data_path,query_dict,request_body):
		...
		async with D.fifolock(Write),BinaryTypePatcher():
			if not os.environ.get(_G):
				...
				except(val.InvalidInputDocument,val.UnrecognizedQueryParameter,val.InvalidQueryParameter)as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.MissingQueryParameter as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_g,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.NonexistentSchemaNode as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.ValidationFailed as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.ParentNodeNotFound as B:A=web.Response(status=404);C=utils.gen_rc_errors(_C,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.UnrecognizedInputNode as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except NotImplementedError as B:A=web.Response(status=501);C=utils.gen_rc_errors(_D,_M,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.NodeAlreadyExists as B:A=web.Response(status=409);C=utils.gen_rc_errors(_D,_h,error_message=str(B));A.text=json.dumps(C,indent=2);return A
			...
			except(dal.CreateCallbackFailed,dal.CreateOrChangeCallbackFailed,dal.ChangeCallbackFailed)as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_i,error_message=str(B));A.text=json.dumps(C,indent=2);return A
			except(PluginNotFound,PluginSyntaxError,FunctionNotFound,FunctionNotCallable)as B:A=web.Response(status=501);C=utils.gen_rc_errors(_D,_M,error_message=str(B));A.text=json.dumps(C,indent=2);return A
	...
	async def handle_put_config_request_lower_half(D,data_path,query_dict,request_body):
		...
		async with D.fifolock(Write),BinaryTypePatcher():
			if not os.environ.get(_G):
				...
				except val.InvalidDataPath as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.ParentNodeNotFound as B:A=web.Response(status=404);C=utils.gen_rc_errors(_C,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.UnrecognizedInputNode as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_I,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except(val.NonexistentSchemaNode,val.ValidationFailed)as B:A=web.Response(status=400);C=utils.gen_rc_errors(_D,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except(val.InvalidInputDocument,val.UnrecognizedQueryParameter)as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_E,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.MissingQueryParameter as B:A=web.Response(status=400);C=utils.gen_rc_errors(_C,_g,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except val.NodeAlreadyExists as B:A=web.Response(status=409);C=utils.gen_rc_errors(_D,_h,error_message=str(B));A.text=json.dumps(C,indent=2);return A
				except NotImplementedError as B:A=web.Response(status=501);C=utils.gen_rc_errors(_D,_M,error_message=str(B));A.text=json.dumps(C,indent=2);return A
			if os.environ.get(_R)and _H in E:await asyncio.sleep(int(E[_H]))
	...
	async def handle_delete_config_request_lower_half(A,data_path):
		...
		async with A.fifolock(Write),BinaryTypePatcher():
			if not os.environ.get(_G):
				...
				except val.NonexistentSchemaNode as C:B=web.Response(status=400);D=utils.gen_rc_errors(_D,_E,error_message=str(C));B.text=json.dumps(D,indent=2);return B
				except val.NodeNotFound as C:B=web.Response(status=404);D=utils.gen_rc_errors(_C,_I,error_message=str(C));B.text=json.dumps(D,indent=2);return B
				except val.ValidationFailed as C:B=web.Response(status=400);D=utils.gen_rc_errors(_D,_E,error_message=str(C));B.text=json.dumps(D,indent=2);return B
	...
	async def handle_action_request(A,request):raise NotImplementedError(_W)
	async def handle_rpc_request(A,request):raise NotImplementedError(_W)
...
async def _handle_transport_delete(watched_node_path,nvh):raise NotImplementedError(_W)
...
async def _handle_plugin_deleted(watched_node_path,nvh):A=re.sub(_X,_Y,watched_node_path);C=re.sub('\\..*','',__name__);B=C+_j+A;D=sys.modules[B];del sys.modules[B];del D;del nvh.plugins[A]
async def _handle_function_created(watched_node_path,jsob,jsob_data_path,nvh):
	assert watched_node_path is not _A;B=re.sub(_X,_Y,jsob_data_path);A=jsob[_U][_P]
...
async def _handle_function_deleted(watched_node_path,nvh):A=watched_node_path;B=re.sub(_X,_Y,A);C=A.rsplit('=',1)[1];del nvh.plugins[B][_J][C]
...
async def _handle_ref_stat_parent_created_yc(watched_node_path,jsob,jsob_data_path,nvh):await _handle_ref_stat_parent_created(watched_node_path,jsob,jsob_data_path,nvh,_c)
...
async def _handle_asymmetric_key_cert_created_or_changed(watched_node_path,jsob,jsob_data_path,nvh):
	I='End entity certificates must not encode superfluous certificates. ';C=nvh;B=watched_node_path;assert jsob_data_path is not _A;J=jsob[_N][_Z];K=base64.b64decode(J);L,P=decode_der(K,asn1Spec=rfc5652.ContentInfo());M=utils.degenerate_cms_obj_to_ders(L);A=[]
	...
	while len(A)!=0:
		...
		if len(F)==0:raise dal.CreateOrChangeCallbackFailed(I+'Found certificates unconnected to chain from the "leaf" '+'certificate while looking for "'+str(D.subject)+_a+B.rsplit(_B,1)[0])
...
async def _handle_verify_asymmetric_key_and_certs_post_sweep(watched_node_path,conn,opaque):
	...
	for H in G:
		O=H['jsob'][_N][_Z];P=base64.b64decode(O);Q,V=decode_der(P,asn1Spec=rfc5652.ContentInfo());R=utils.degenerate_cms_obj_to_ders(Q)
		for S in R:
...
async def _handle_trust_anchor_cert_created_or_changed(watched_node_path,jsob,jsob_data_path,nvh):
	B=watched_node_path;assert jsob_data_path is not _A;assert nvh is not _A;G=jsob[_N][_Z];H=base64.b64decode(G)
	...
	while len(A)!=0:
		...
		if len(E)==0:raise dal.CreateOrChangeCallbackFailed('Trust anchor certificates must not encode superfluous certificates.'+'Discovered additional certificates while looking for the issuer of "'+str(C.subject)+_a+B.rsplit(_B,1)[0])
		if len(E)>1:raise dal.CreateOrChangeCallbackFailed('Trust anchor certificates must encode a single chain of '+'certificates.  Found '+str(len(E))+' certificates issued by "'+str(C.subject)+_a+B.rsplit(_B,1)[0])
		C=E[0];A.remove(C)
