...
class DataAccessLayer:
	...
	async def _handle_get_data_request(E,data_path,schema_path,content_type,query_dict):
		...
		return H
	async def _recursively_attempt_to_get_data_from_subtable(E,subtable_name,parent_id,jsob_schema_path,jsob_data_path,jsob_iter,content_type,query_dict,conn):
		...
		if C in E.config_true_obu_seq_nodes:assert _X not in J;J[_X]=_J
		S=E._find_rows_in_table_having_parent_id(C,parent_id,J,conn);K=S.fetchall()
		if len(K)==0:return
		B=jsob_iter
		if N==_B:O=C[1:]
		else:O=C.replace(N+_B,'')
		D=re.sub(_W,'',C);G=re.sub(D+'$','',O)
		if G!='':
			G=G[:-1];T=G.split(_B)
			for L in T:
				try:B=B[L]
				except KeyError:assert ContentType.CONFIG_FALSE in I;B[L]={};B=B[L]
		U=E.schema_path_to_real_table_name[C];P=E.metadata.tables[U]
		if any(C.startswith(A)for A in E.config_false_prefixes):
			B[D]=[]
			for F in K:
				H={}
				for A in P.c:
					if A.name not in(_O,_H,_d,_e):
						if isinstance(A.type,sa.sql.sqltypes.DateTime):H[A.name]=F[A.name].strftime('%Y-%m-%dT%H:%M:%SZ')
						elif isinstance(A.type,(sa.sql.sqltypes.JSON,sa.sql.sqltypes.PickleType)):
							if F[A.name]is not _A and not(isinstance(F[A.name],list)and len(F[A.name])==0):H[A.name]=F[A.name]
			...
			for M in K:
				...
				if D not in B:B[D]=[]
				B[D].append(R.pop(D));V=E._get_list_of_direct_subtables_for_schema_path(C)
				for W in V:await E._recursively_attempt_to_get_data_from_subtable(W,M[_O],C,jsob_data_path+_D+M[P.c[4].name],B[D][Q],I,{},conn)
				Q+=1
	async def handle_post_config_request(A,data_path,query_dict,request_body,create_callbacks,change_callbacks,opaque):
		...
		with A.engine.begin()as C:
			...
			if F is _A:raise ParentNodeNotFound(_f+B+_U)
	...
	async def _handle_post_config_request(B,parent_dbpath,query_dict,request_body,create_callbacks,change_callbacks,opaque,conn):
		...
		if O==_B:C=E;L=_B+C
		else:C=re.sub(_o,'',E);L=O+_B+C
		M=re.sub(_F,'',L);H=B._get_table_name_for_schema_path(M);assert H is not _A
		if H==A.table_name:
			if C in A.node_ptr:
				if not isinstance(A.node_ptr[C],list):raise NodeAlreadyExists(_Y+C+_Z)
				assert len(F[E])==1
				if F[E][0]in A.node_ptr[C]:raise NodeAlreadyExists(_Y+F[E][0]+_Z)
			...
			else:A.node_ptr[C]=F.pop(E);c=await B._recursively_post_subtable_data(A.record_id,L,A.node_ptr[C],A.jsob,A.jsob_data_path,U,V,K)
			B._update_jsob_for_record_id_in_table(A.table_name,A.record_id,A.jsob,K)
		else:
			assert M in B.table_keys
			if C not in A.node_ptr:A.node_ptr[C]=[];B._update_jsob_for_record_id_in_table(A.table_name,A.record_id,A.jsob,K)
			...
			except sa.exc.IntegrityError as d:raise NodeAlreadyExists(_Y+C+_Z)from d
			L+=_D+str(Q);c=await B._recursively_post_subtable_data(Z.inserted_primary_key[0],L,W[C],W,L,U,V,K);S=B.schema_path_to_real_table_name[H];N=B.metadata.tables[S];K.execute(N.update().where(N.c.record_id==Z.inserted_primary_key[0]).values(jsob=W))
			if M in B.config_true_obu_seq_nodes:
				...
				S=B.schema_path_to_real_table_name[H];N=B.metadata.tables[S];e=B.table_keys[H];f=N.update().where(and_(N.c.parent_id==A.record_id,getattr(N.c,e)==bindparam(_S))).values(obu_idx=bindparam(_T));Z=K.execute(f,[{_S:A[0][1],_T:A[1][1]}for A in I[D:len(I)]])
			else:0
		g=re.sub(_F,'',O)
		if g in a:
		...
		if isinstance(A,dict):
			...
			if B in C.table_keys:
				...
				while A:
					...
					except sa.exc.IntegrityError as Y:raise NodeAlreadyExists(_Y+K+'" with value "'+G[K]+_Z)from Y
	...
	async def _handle_put_config_request(B,data_path,query_dict,request_body,create_callbacks,change_callbacks,delete_callbacks,opaque,conn):
		...
		if A is _A:
			...
			if A is _A:raise ParentNodeNotFound(_f+G+_U)
	...
	async def handle_put_opstate_request(D,data_path,request_body):
		...
		with D.engine.begin()as E:
			...
			if A is _A:raise ParentNodeNotFound(_f+C+_U)
	...
	def _get_jsob_for_record_id_in_table(B,table_name,record_id,conn):
		...
		for E in A.c:
			if E.name not in(_O,_H,_d,_e):J[I][E.name]=H[E.name]
		return J
	...
	def _init_from_existing_db(A,url,cacert_param,cert_param,key_param):
		...
		if not(H.startswith('sqlite:///')or H.startswith(_a)or H.startswith(_r)):raise SyntaxError('The database url contains an unrecognized dialect.')
		D={}
		if I is not _A:
			D[_M]={};D[_M]['ca']=I;D[_M]['mode']=6
			if J is not _A:D[_M]['cert']=J
			if K is not _A:D[_M][_h]=K
		A.engine=sa.create_engine(H,connect_args=D);A.db_schema=_A;A.table_keys={};A.config_false_prefixes={};A.config_true_obu_seq_nodes={};A.schema_path_to_real_table_name={};A.leafrefs={};A.referers={};A.ref_stat_collectors={}
		try:
			if A.engine.url.database==_i or not db_utils.database_exists(A.engine.url,connect_args=D):A.engine=_A;raise NotImplementedError
		...
		else:A.db_schema=A.engine.url.database;A.schema_path_to_real_table_name[_B]=A.db_schema.join(_b);A.schema_path_to_real_table_name[_G]=A.db_schema+_b;O=A.engine.execute(_s);L=O.fetchall();P=[L[A][0]for A in range(len(L))];assert A.db_schema in P
		...
		for Q in A.metadata.tables.values():
			for E in Q.c:
				...
				if A.engine.dialect.name==_a and isinstance(E.type,sa.dialects.mysql.types.LONGTEXT):E.type=sa.JSON()
	...
	def _init_new_db_with_factory_default(A):
		b='import-only-module';a='module';Z='local-address';V='endpoint';N='datastore';M='local-bind';K='ietf-yang-library:yang-library';J='schema';I='module-set'
		if os.environ.get('YANGCORE_TEST_DAL'):
			B=_B;C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
			with A.engine.begin()as E:F=E.execute(D.insert().values(name=_l,jsob={}))
			A.global_root_id=F.inserted_primary_key[0];return
		c=A._load_yang_dir_file('factory-default.json');d=A._load_yang_dir_file('yang-library-nbi.json');e={'yangcore:audit-log':{'audit-log-record':[]},'yangcore:notification-log':{'notification-log-record':[]}};H=c|d|e;O=H['ietf-restconf-server:restconf-server']['listen']['endpoints'];W={V:O[V].pop(0)};O=W[V]['http-over-tcp']['tcp-server-parameters'];P=[]
		for f in O[M]:P.append({M:f})
		O[M]=[];assert len(P)==2;L=[]
		while len(H[K][I]):g=H[K][I].pop(0);L.append({I:g})
		Q=[]
		while len(H[K][J]):h=H[K][J].pop(0);Q.append({J:h})
		X=[]
		while len(H[K][N]):i=H[K][N].pop(0);X.append({N:i})
		if(S:=os.environ.get('YANGCORE_INIT_PORT')):
			try:T=int(S)
			except ValueError as j:raise ValueError('Invalid "YANGCORE_INIT_PORT" value ('+S+').')from j
			if T<=0 or T>2**16-1:raise ValueError('The "YANGCORE_INIT_PORT" value ('+S+') is out of range [1..65535].')
			for R in P:R[M]['local-port']=T
		k=H;B=_B;C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		with A.engine.begin()as E:F=E.execute(D.insert().values(name=_l,jsob=k))
		assert F.inserted_primary_key[0]==F.lastrowid;A.global_root_id=F.inserted_primary_key[0];B='/ietf-restconf-server:restconf-server/listen/endpoints/endpoint';C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		with A.engine.begin()as E:U=E.execute(D.insert().values(parent_id=A.global_root_id,name='default startup endpoint',jsob=W))
		assert U.inserted_primary_key[0]==U.lastrowid;l=U.inserted_primary_key[0]
		for R in P:
			G={};G[_H]=l;G[Z]=R[M][Z];G[_K]=R;B='/ietf-restconf-server:restconf-server/listen/endpoints/'+'endpoint/http-over-tcp/tcp-server-parameters/local-bind';C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
			with A.engine.begin()as E:F=E.execute(D.insert().values(**G))
		assert len(L)==1;B='/ietf-yang-library:yang-library/module-set';C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		with A.engine.begin()as E:G={_H:A.global_root_id,_I:L[0][I][_I],a:L[0][I][a],b:L[0][I][b]};F=E.execute(D.insert().values(**G));assert F.inserted_primary_key[0]==F.lastrowid;m=F.inserted_primary_key[0]
		assert len(Q)==1;B='/ietf-yang-library:yang-library/schema';C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		with A.engine.begin()as E:G={_H:A.global_root_id,_I:Q[0][J][_I],I:Q[0][J][I]};F=E.execute(D.insert().values(**G))
		...
		with A.engine.begin()as E:
			for Y in X:G={_H:A.global_root_id,_I:Y[N][_I],J:Y[N][J]};F=E.execute(D.insert().values(**G))
	def _create_new_db(A,url,yl_obj,cacert_param=_A,cert_param=_A,key_param=_A):
		...
		if A.engine.url.database!=_i:
			if A.engine.dialect.name==_a:db_utils.create_database(A.engine.url,encoding='utf8mb4',connect_args=B)
			else:db_utils.create_database(A.engine.url,connect_args=B)
		A.db_schema=_A
		if A.engine.dialect.name in(_a,_r):
		...
		if A.engine.dialect.name==_j:A.schema_path_to_real_table_name[_B]=_G;A.schema_path_to_real_table_name[_G]=_G
		else:A.schema_path_to_real_table_name[_B]=A.db_schema+_b;A.schema_path_to_real_table_name[_G]=A.db_schema+_b
	...
	def _recursive_gen_tables(D,node,parent_table_name):
		...
		if issubclass(type(C),yangson.schemanode.ListNode):
			B=[];B.append(sa.Column(_O,sa.Integer,primary_key=_E));O=D.schema_path_to_real_table_name[G];B.append(sa.Column(_H,sa.Integer,sa.ForeignKey(O+'.record_id'),index=_E,nullable=_C));B.append(sa.Column(_d,sa.String(250),index=_E,nullable=_E))
			class P(enum.Enum):UNIVERSAL=0;OPERATIONAL=1;RUNNING=2;STARTUP=3;SYSTEM=4;INTENDED=5;FACTORY=6
			B.append(sa.Column(_e,sa.Enum(P),index=_E,nullable=_E))
			if C.config is _E:
				...
				for A in C.children:
					if issubclass(type(A),yangson.schemanode.LeafNode):
						if isinstance(A.type,yangson.datatype.StringType):
							if str(A.type)=='date-and-time(string)':B.append(sa.Column(A.name,sa.DateTime,index=_E,nullable=A.mandatory is _C or A.when is not _A))
						...
						for I in A.children:
							...
							for Q in I.children:
								if not isinstance(Q,yangson.schemanode.LeafNode):H=_C;break
