...
class MORTMEncoderLayer(TransformerEncoderLayer):
    def __init__(self, d_model, dim_ff, num_head, dropout, batch_first, bias, layer_norm_eps):
        ...
        self.self_attn = MultiHeadAttentionRPR(d_model, num_head, dropout).to(dtype=torch.bfloat16)
...
class MORTMDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, dim_ff, num_head, dropout, batch_first, bias, layer_norm_eps):
        ...
        self.cross_attention = MultiheadAttention(d_model, num_head, dropout, batch_first=False).to(dtype=torch.bfloat16)
        #self.self_attention: FlashSelfAttentionM =FlashSelfAttentionM(d_model, num_head, dropout)
        self.self_attention = MultiHeadAttentionRPR(d_model, num_head, dropout).to(dtype=torch.bfloat16)
