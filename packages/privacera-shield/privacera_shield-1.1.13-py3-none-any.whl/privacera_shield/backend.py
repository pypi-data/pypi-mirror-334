...
class ShieldRestHttpClient:
    ...
    def handle_unsuccessful_response(self, response, error_message_template: ErrorMessage):
        """
        Handle unsuccessful response from the shield server.

        Args:
            response (Response): The response object from the shield server.
            error_message_template (ErrorMessage): The error message to be displayed.

        Raises:
            PAIGException: If the response status code is 400 or 404 or 500.
        """
        error_message = ""

        if response.status == 400 or response.status == 401 or response.status == 404:
        ...
        exception = PAIGException(error_message_template, response_status=response.status,
                                  response_data=f"{error_message}\n")
        _logger.error(exception)
        raise exception

    def handle_error_response(self, exception: Exception, error_message_template: ErrorMessage):
        """
        Handle error response from the shield server.

        Args:
            exception (Exception): The exception received while making the request.
            error_message_template (ErrorMessage): The error message to be displayed.

        Raises:
            PAIGException: If request is not successful.
        """
        error_message = ""
        ...
        exception = PAIGException(error_message_template, response_status=0, response_data=f"{error_message}\n")
        _logger.error(exception)
        raise exception
    ...
    def is_access_allowed(self, request: ShieldAccessRequest) -> ShieldAccessResult:
        ...
        try:
            ...
            if response.status == 200:
                ...
                self.handle_unsuccessful_response(response, ErrorMessage.SHIELD_SERVER_AUTHORIZATION_FAILED)
        except PAIGException as e:
            raise e
        except Exception as e:
            self.handle_error_response(e, ErrorMessage.SHIELD_SERVER_AUTHORIZATION_FAILED)
    ...
    def init_shield_server(self, application_key) -> None:
        ...
        try:
            ...
            if response_status == 200:
                ...
                self.handle_unsuccessful_response(response, ErrorMessage.SHIELD_SERVER_INITIALIZATION_FAILED)
        except PAIGException as e:
            raise e
        except Exception as e:
            self.handle_error_response(e, ErrorMessage.SHIELD_SERVER_INITIALIZATION_FAILED)
    ...
    def get_filter_expression(self, request: VectorDBAccessRequest) -> VectorDBAccessResult:
        ...
        try:
            ...
            if response.status == 200:
                ...
                self.handle_unsuccessful_response(response, ErrorMessage.SHIELD_SERVER_GET_FILTER_EXPRESSION_FAILED)
        except PAIGException as e:
            raise e
        except Exception as e:
            self.handle_error_response(e, ErrorMessage.SHIELD_SERVER_GET_FILTER_EXPRESSION_FAILED)
    ...
    def log_stream_access_audit(self, request: StreamAccessAuditRequest):
        ...
        try:
            ...
            if response.status == 200:
                ...
                self.handle_unsuccessful_response(response, ErrorMessage.SHIELD_SERVER_LOG_STREAM_ACCESS_AUDIT_FAILED)
        except PAIGException as e:
            raise e
        except Exception as e:
            self.handle_error_response(e, ErrorMessage.SHIELD_SERVER_LOG_STREAM_ACCESS_AUDIT_FAILED)