...
class Account:
    ...
    def __init__(self, email: str = None, username: str = None, password: str = None, session: Client = None, **kwargs):
        ...
        proxy_config_path = kwargs.get('proxy_config_path', None)
        self.proxy_manager = ProxyManager(proxy_config_path)
        
        if username:
            proxy_dict = self.proxy_manager.get_proxy_dict(username)
            if proxy_dict:
                kwargs['proxy'] = proxy_dict
                self.logger.info(f"为账号 {username} 使用代理配置")
                
                if self.debug:
                    for protocol, url in proxy_dict.items():
                        self.logger.debug(f"  {protocol}: {url}")
    ...
    def _upload_media(self, media: str, is_dm: bool = False) -> str:
        """
        上传媒体文件
        ...
        file = Path(media)
        if not file.exists():
            raise Exception(f"Media file not found: {media}")
        
        # 媒体文件信息
        total_bytes = file.stat().st_size
        media_type = mimetypes.guess_type(file)[0]
        media_category = get_media_category(media, is_dm)
            ...
            try:
                ...
                if r.status_code >= 400:
                    if retry < max_retries - 1:
                        log_message('warning', f"初始化媒体上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                        time.sleep(retry_delay * (retry + 1))  # 延迟增加
                        continue
                    else:
                        raise Exception(f"初始化媒体上传失败: {r.text}")
                
                media_id = r.json()['media_id']
                break  # 成功获取media_id，退出重试循环
            except Exception as e:
                if retry < max_retries - 1:
                    log_message('warning', f"初始化媒体上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                    time.sleep(retry_delay * (retry + 1))
                else:
                    raise Exception(f"初始化媒体上传时发生错误: {str(e)}")
        
        if not media_id:
            raise Exception("无法获取媒体上传ID")
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    for retry in range(max_retries):
                        try:
                            ...
                            _headers.update({k.encode('utf-8'): v.encode('utf-8') for k, v in headers.items()})
                            ...
                            if r.status_code >= 400:
                                if retry < max_retries - 1:
                                    log_message('warning', f"分片 {i} 上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                                    time.sleep(retry_delay * (retry + 1))
                                    continue
                                else:
                                    raise Exception(f"分片 {i} 上传失败: {r.text}")
                            
                            pbar.update(len(chunk))
                            break  # 成功上传分片，退出重试循环
                        except Exception as e:
                            if retry < max_retries - 1:
                                log_message('warning', f"分片 {i} 上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                                time.sleep(retry_delay * (retry + 1))
                            else:
                                raise Exception(f"分片 {i} 上传时发生错误: {str(e)}")
                    i += 1
        ...
        for retry in range(max_retries):
            try:
                ...
                if r.status_code >= 400:
                    if retry < max_retries - 1:
                        log_message('warning', f"完成媒体上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                        time.sleep(retry_delay * (retry + 1))
                        continue
                    else:
                        raise Exception(f"完成媒体上传失败: {r.text}")
                
                processing_info = r.json().get('processing_info', {})
                if processing_info.get('state') != MEDIA_UPLOAD_SUCCEED and processing_info.get('check_after_secs'):
                    if not self.debug:
                        print(f"\rprocessing media in {processing_info['check_after_secs']} seconds", end='')
                    time.sleep(processing_info['check_after_secs'])
                    
                    # 检查上传状态（带重试逻辑）
                    for status_retry in range(max_retries):
                        try:
                            params = {'command': 'STATUS', 'media_id': media_id}
                            r = self.session.get(url=url, headers=headers, params=params)
                            
                            if r.status_code >= 400:
                                if status_retry < max_retries - 1:
                                    log_message('warning', f"检查媒体状态失败，将在 {retry_delay} 秒后重试 ({status_retry+1}/{max_retries})")
                                    time.sleep(retry_delay * (status_retry + 1))
                                    continue
                                else:
                                    raise Exception(f"检查媒体状态失败: {r.text}")
                            
                            processing_info = r.json().get('processing_info', {})
                            
                            if processing_info.get('state') == MEDIA_UPLOAD_FAIL:
                                error = processing_info.get('error', {})
                                raise Exception(f"媒体处理失败: {error.get('message', '未知错误')}")
                            ...
                            if status_retry < max_retries - 1:
                                log_message('warning', f"检查媒体状态时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({status_retry+1}/{max_retries})")
                                time.sleep(retry_delay * (status_retry + 1))
                            else:
                                raise Exception(f"检查媒体状态时发生错误: {str(e)}")
                break  # 成功完成上传，退出重试循环
            except Exception as e:
                if retry < max_retries - 1:
                    log_message('warning', f"完成媒体上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                    time.sleep(retry_delay * (retry + 1))
                else:
                    raise Exception(f"完成媒体上传时发生错误: {str(e)}")

        log_message('info', f"媒体上传成功: {media} (ID: {media_id})")
        return media_id
    ...
    def _init_logger(self, **kwargs) -> Logger:
        ...
        if kwargs.get('debug'):
            ...
            # 如果没有启用debug，创建一个null处理器的logger
            # 这样不会输出任何日志，但可以安全地调用logger的方法
    ...
    def _validate_session(*args, **kwargs):
        ...
        if proxy:
            # 检测是否是SOCKS5代理
            if isinstance(proxy, dict) and any("socks5" in val for val in proxy.values()):
                # 为SOCKS5代理添加额外选项
                client_options["http2"] = False  # 禁用HTTP/2，解决某些SOCKS5代理的兼容性问题
            elif isinstance(proxy, str) and "socks5" in proxy:
                # 字符串形式的SOCKS5代理
                client_options["http2"] = False
        ...
        if isinstance(cookies, dict) and all(cookies.get(c) for c in {'ct0', 'auth_token'}):
            ...
            _session = Client(cookies=cookies, proxies=proxy_dict, **client_options)
        ...
        if isinstance(cookies, str):
            ...
            _session = Client(cookies=orjson.loads(Path(cookies).read_bytes()), 
                            proxies=proxy_dict,
                            **client_options)
            _session._init_with_cookies = True
            _session.headers.update(get_headers(_session))
