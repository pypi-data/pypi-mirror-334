    ...
    def _upload_media(self, media: str, is_dm: bool = False) -> str:
            ...
            try:
                ...
                if r.status_code >= 400:
                    if retry < max_retries - 1:
                        self.logger.warning(f"初始化媒体上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                ...
                if retry < max_retries - 1:
                    self.logger.warning(f"初始化媒体上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    for retry in range(max_retries):
                        try:
                            ...
                            if r.status_code >= 400:
                                if retry < max_retries - 1:
                                    self.logger.warning(f"分片 {i} 上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                                    time.sleep(retry_delay * (retry + 1))
                                    continue
                                else:
                            ...
                            if retry < max_retries - 1:
                                self.logger.warning(f"分片 {i} 上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                                time.sleep(retry_delay * (retry + 1))
                            else:
        ...
        for retry in range(max_retries):
            try:
                ...
                if r.status_code >= 400:
                    if retry < max_retries - 1:
                        self.logger.warning(f"完成媒体上传失败，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                        time.sleep(retry_delay * (retry + 1))
                        continue
                    else:
                        ...
                        try:
                            ...
                            if r.status_code >= 400:
                                if status_retry < max_retries - 1:
                                    self.logger.warning(f"检查媒体状态失败，将在 {retry_delay} 秒后重试 ({status_retry+1}/{max_retries})")
                            ...
                            if status_retry < max_retries - 1:
                                self.logger.warning(f"检查媒体状态时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({status_retry+1}/{max_retries})")
                                time.sleep(retry_delay * (status_retry + 1))
                            else:
                ...
                if retry < max_retries - 1:
                    self.logger.warning(f"完成媒体上传时发生错误: {str(e)}，将在 {retry_delay} 秒后重试 ({retry+1}/{max_retries})")
                    time.sleep(retry_delay * (retry + 1))
                else:
                    raise Exception(f"完成媒体上传时发生错误: {str(e)}")

        self.logger.info(f"媒体上传成功: {media} (ID: {media_id})")
        return media_id
