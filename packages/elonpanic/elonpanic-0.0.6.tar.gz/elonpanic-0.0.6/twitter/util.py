...
def init_session(proxy=None):
    ...
    }, proxies=proxy, follow_redirects=True)
...
def get_media_category(filename: str, is_dm: bool = False) -> str:
    """
    根据文件名和上传类型确定媒体类别
    ...
    if 'gif' in media_type:
        return f'{upload_type}_gif' 
    else:
        # 提取主要类型（image, video, audio）
        main_type = media_type.split("/")[0]
        return f'{upload_type}_{main_type}'
...
def check_media(category: str, size: int) -> None:
    ...
    异常:
        如果文件超过大小限制，抛出异常
    """
    from .constants import MAX_IMAGE_SIZE, MAX_GIF_SIZE, MAX_VIDEO_SIZE
    
    fmt = lambda x: f'{(x / 1e6):.2f} MB'
    msg = lambda x: f'cannot upload {fmt(size)} {category}, max size is {fmt(x)}'
    
    if 'image' in category and size > MAX_IMAGE_SIZE:
        raise Exception(msg(MAX_IMAGE_SIZE))
    if 'gif' in category and size > MAX_GIF_SIZE:
        raise Exception(msg(MAX_GIF_SIZE))
    if 'video' in category and size > MAX_VIDEO_SIZE:
