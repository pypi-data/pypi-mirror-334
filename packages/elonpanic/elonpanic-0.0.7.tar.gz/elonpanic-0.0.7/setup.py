...
about = {}
exec((Path().cwd() / 'twitter' / '__version__.py').read_text(encoding='utf-8'), about)
...
setup(
    ...
    * [Example API Responses](#example-api-responses)
    ...
    You can use proxies with your Twitter accounts in several ways:

    ```python
    from twitter import Account, ProxyManager

    ## Method 1: Direct proxy assignment for a single session
    # HTTP proxy
    account = Account(email, username, password, proxy="http://127.0.0.1:8080")

    # HTTP proxy with authentication
    account = Account(email, username, password, proxy="http://user:pass@127.0.0.1:8080")

    # SOCKS5 proxy
    account = Account(email, username, password, proxy="socks5://127.0.0.1:1080")

    # SOCKS5 proxy with authentication
    account = Account(email, username, password, proxy="socks5://user:pass@127.0.0.1:1080")

    ## Method 2: Using ProxyManager to manage proxies for multiple accounts
    # Initialize proxy manager
    proxy_manager = ProxyManager()

    # Set HTTP proxy for a specific account
    proxy_manager.set_proxy(username="your_username", proxy_url="127.0.0.1:8080", proxy_type="http")
    ...
    # Set SOCKS5 proxy for another account
    proxy_manager.set_proxy(username="another_username", proxy_url="127.0.0.1:1080", proxy_type="socks5")
    ...
    # You can also pass a complete proxy URL with authentication
    proxy_manager.set_proxy(
        username="your_username", 
    ...
    # The account will automatically use the configured proxy
    account = Account(email, username, password)

    # List all configured proxies
    all_proxies = proxy_manager.list_proxies()

    # Remove proxy for a specific account
    proxy_manager.remove_proxy(username="your_username")
    ```

    Proxy configurations are saved in `.twitter_proxies.json` in your home directory by default. You can specify a custom path:

    ```python
    # Custom proxy configuration path
    proxy_manager = ProxyManager(config_path="/path/to/config.json")

    # Or when creating an account
    account = Account(email, username, password, proxy_config_path="/path/to/config.json")
    ```

    #### Proxy Best Practices

    当使用代理时，尤其是在Windows系统上使用SOCKS5代理，可能需要特别注意以下几点：
    ...
    account = Account(
        ...
        debug=1  # 开启调试日志以便跟踪问题
    )
    
    # 对于SOCKS5代理，可以添加额外的选项来提高稳定性：
    from httpx import Limits, Client
    
    # 创建自定义客户端以设置更多选项
    client = Client(
        cookies={"ct0": "...", "auth_token": "..."},
        proxies={"http://": "socks5://user:pass@127.0.0.1:1080", "https://": "socks5://user:pass@127.0.0.1:1080"},
        timeout=30.0,        # 增加超时时间
        verify=True,         # 验证SSL证书
        http2=False,         # 禁用HTTP/2，提高与某些SOCKS5代理的兼容性
        limits=Limits(max_connections=100, max_keepalive_connections=20),  # 调整连接池大小
    ...
    如果在使用代理时遇到连接问题，可以尝试以下解决方案：

    1. 使用HTTP代理替代SOCKS5代理（如果可能）
    2. 确保代理服务器支持HTTPS流量
    3. 增加超时设置
    4. 禁用客户端的HTTP/2支持（如上例所示）
    5. 检查代理服务器是否要求验证及验证凭据是否正确
