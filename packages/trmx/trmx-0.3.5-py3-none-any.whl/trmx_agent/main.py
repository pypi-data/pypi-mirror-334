...
class ChatAgent:
    ...
    def chat(self) -> None:
        ...
        console.print("[bold cyan]Provider:[/bold cyan] "
                      f"[cyan]{self.provider.capitalize()}[/cyan]")
        ...
        while True:
            try:
                ...
                # Check for DeepSeek thinking tags if using Fireworks provider with DeepSeek model
                # and show_thinking is enabled
                if self.show_thinking and self.provider == "fireworks" and "deepseek" in self.model.lower():
