...
class _ToAbstractSyntaxTree(Transformer):
    ...
    def strptime(self, values: List[AbstractSyntaxTreeNode]) -> AbstractSyntaxTreeNode:
        ...
        if timestamp_value.HasField("literal_value"):
            timestamp_str = timestamp_value.literal_value.string_value
        ...
        return AbstractSyntaxTreeNode(
            operation=Operation(operation=OperationType.STRPTIME, operands=[timestamp_value]),
            dtype=TimestampType().proto,
        )

    def _validate_strptime_format(self, format_value: str):
        if "%" not in format_value:
            msg = f"Error parsing expression: no format directives specified in STRPTIME format string: {format_value}"
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        while i < len(format_value):
            if format_value[i] == "%":
                if i + 1 >= len(format_value):
                    msg = f"Error parsing expression: Incomplete format directive at position {i} in {format_value}"
                    raise ExpressionParsingError(msg, self.calculation_name)
                if format_value[i + 1] not in VALID_STRPTIME_DIRECTIVES:
                    msg = f"Error parsing expression: Invalid format directive '{format_value[i:i + 2]}' at position {i} in {format_value}"
                    raise ExpressionParsingError(msg, self.calculation_name)
                i += 2
            else:
                i += 1
        ...
        if "%V" in format_value and "%Y" in format_value:
            msg = f"Error parsing expression: %V is incompatible with year directive %Y. Use the ISO year %G instead in {format_value}"
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        if "%g" not in format_value and "%n" not in format_value:
            try:
                # TODO: swap pytz with zoneinfo when python 3.8 is no longer supported
                now_utc = datetime.now(pytz.utc)
                # Try to format a date using the string
                formatted_date = now_utc.strftime(format_value)
                # Try to parse it back
                datetime.strptime(formatted_date, format_value)
            except Exception:
                msg = f"Error parsing expression: format failed test parsing, invalid format string: {format_value}"
                raise ExpressionParsingError(msg, self.calculation_name)
