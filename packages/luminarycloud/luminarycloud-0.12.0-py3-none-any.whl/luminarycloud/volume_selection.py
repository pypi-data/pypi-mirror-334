...
class VolumeSelection:
    ...
    def unselect(self, volumes: Iterable[Volume]) -> None:
        ...
        if not isinstance(volumes, Iterable[Volume]):
            raise TypeError("volumes must be an iterable of Volume")
        self.__volume_ids -= set(int(v.id) for v in volumes)
    ...
    def shrinkwrap(
        ...
        *,
        ...
        feature_name : str
            The name of the feature.
        """
        has_min = resolution_min is not None
        has_max = resolution_max is not None
