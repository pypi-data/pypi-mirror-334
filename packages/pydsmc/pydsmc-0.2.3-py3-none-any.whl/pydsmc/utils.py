...
def is_bounded(bounds: tuple[float | None, float | None]) -> bool:
    return (bounds[0] is not None and
            bounds[1] is not None and
            not np.isinf(bounds[0]) and
            not np.isinf(bounds[1]))
...
def create_eval_envs(
    ...
    env_seed: int,
    gym_id: str,
    wrappers: list[gym.Wrapper] = None,
    ...
    **gym_kwargs
    ...
    def make_env(env_id, wrap_list=[]):
        def _thunk():
            env = gym.make(gym_id, **gym_kwargs)
            for w in wrap_list:
                env = w(env)
            env.reset(seed = env_seed + env_id) # Seeding only necessary for SB3 I think, doesn't hurt either way
            return env
        return _thunk
    ...
    for i in range(num_threads):
        env_fns = [ make_env(i * num_envs_per_thread + j, wrappers) for j in range(num_envs_per_thread) ]
        env = vecenv_cls(env_fns) # Eval seems to be faster with gym vector envs in preliminary tests
        envs.append(env)
...
class DSMCLogger:
    ...
    class ColorfulFormatter(logging.Formatter):
        ...
        def format(self, record):
            log_fmt = DSMCLogger.ColorfulFormatter.FORMATS.get(record.levelno,
    ...
    def set_colorize(colorize: bool=False):
        new_formatter = DSMCLogger.ColorfulFormatter() if colorize else logging.Formatter(DSMCLogger._LOGGING_FORMAT, datefmt=DSMCLogger._DATE_FORMAT)
