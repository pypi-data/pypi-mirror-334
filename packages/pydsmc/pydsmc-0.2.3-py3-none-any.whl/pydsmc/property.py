...
__PRE_DEFINED_PROPERTIES = {
    ...
    'return':
        {
            'check_fn': lambda self, t: np.sum([t[i][2] * np.power(self.gamma, i) for i in range(len(t))]),
            'binomial': False,
...
class Property:
    def __init__(
        ...
        check_fn: Callable[[SelfProperty, list[tuple[Any, Any, Any, bool, bool, Any]]], float] = None,
        eps: float | None = 0.05,
        kappa: float = 0.05,
        ...
        **kwargs
        ...
        if eps is None and relative_error:
    ...
    def get_interval(self) -> tuple[bool, tuple[float, float] | None]:
        if (self.max_episodes_zero_variance > 0 and self.st_method.count > self.max_episodes_zero_variance and
            self.st_method.mean == 0.0 and self.st_method.variance == 0.0):
            return True, (0.0, 0.0)
    ...
    def setup_eval(self, log_dir: Path, save_full_results: bool = False) -> None:
        ...
        self.__full_results = []
        self.property_dir = log_dir / f"{self.name}_{self.property_id}"
        self.save_path = self.property_dir / 'results.jsonl'
        self.full_results_path = self.property_dir / 'full_results.jsonl'


    def save_settings(self, log_dir: Path, exclude: list = None) -> None:
        if exclude is None:
            exclude = []
        settings = self.__dict__.copy()
        for key in exclude + ['st_method', 'check_fn', 'results_lock', 'save_full_results', 'property_dir', '_Property__full_results', 'save_path', 'full_results_path']:
        ...
        with open(property_dir / 'settings.json', 'w') as f:
    ...
    def save_results(
        ...
        with open(self.save_path, 'w' if overwrite else 'a') as f:
    ...
    def dump_buffer(self, overwrite: bool = False):
        with self.results_lock:
            if self.__full_results:
                with open(self.full_results_path, "w" if overwrite else 'a') as f:
    ...
    def _set_property_id(self) -> None:
...
def select_statistical_method(
    ...
    **kwargs # Ignore additional arguments; unknown args will be used by the Property constructor
    ...
    if sound:
        DSMCLogger.get_logger().warning('Selected a statistics method with soundness guarantees. Converging will probably take a very long time.')
        pass

    args = (eps, kappa, relative_error, bounds, binomial, *(() if min_samples is None else (min_samples,)))

    if eps is not None: # Sequential setting; runs until confidence level is reached
        if binomial:
            if not sound:
            ...
            if relative_error:
                ...
                return st.WilsonScoreIntervalMethod(*args)

        elif is_bounded(bounds) and sound:
            if relative_error:
                return st.EBStopMethod(*args)
            else:
                return st.HoeffdingMethod(*args)

        else: # for unbounded or unsound properties
            return st.StudentsTMethod(*args)

    else: # fixed number of runs setting, reports confidence interval after given number of episodes
        if binomial:
            if sound:
                return st.WilsonScoreIntervalMethod(*args)
            else:
...
def create_predefined_property(
    ...
    **kwargs
    ...
    if name is None: name = property_id

    if property_id not in __PRE_DEFINED_PROPERTIES:
        raise ValueError(f"Pre-defined property '{property_id}' not found")
    ...
    if st_method is None:
        st_method = select_statistical_method(eps=eps, kappa=kappa, relative_error=relative_error,
                                              min_samples=min_samples, sound=sound, **property_parameters)
        DSMCLogger.get_logger().debug(f"Automatically selected statistical method {st_method.__class__.__name__} for predefined property {name}")
...
def create_custom_property(
    ...
    **kwargs
    ...
    if st_method is None:
        st_method = select_statistical_method(eps=eps, kappa=kappa, relative_error=relative_error,
                                              bounds=bounds, binomial=binomial, min_samples=min_samples, sound=sound)
        DSMCLogger.get_logger().debug(f"Automatically selected statistical method {st_method.__class__.__name__} for custom property {name}")

    return Property(name=name, st_method=st_method, check_fn=check_fn, eps=eps, kappa=kappa,
                    relative_error=relative_error, bounds=bounds, binomial=binomial, **kwargs)
