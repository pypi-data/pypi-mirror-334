...
def main(
    ...
    for filename in sorted(allinputs):
        ...
        for key, df in data.items():
            ...
            # store VDDA/D voltages in analysis output
            rounded_results["AR_VDDA_VS_TRIM"] = [round(elem, 3) for elem in vdda]
            rounded_results["AR_VDDD_VS_TRIM"] = [round(elem, 3) for elem in vddd]
