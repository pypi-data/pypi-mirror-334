...
class EndingReason(BabelDesignated):
    ...
    class Meta:
        ...
        abstract = dd.is_abstract_model(__name__, 'EndingReason')
...
if dd.plugins.avanti.with_asylum:
    ...
    class ResidencePermit(BabelDesignated):

        class Meta:
            app_label = 'avanti'
            verbose_name = _("Residence permit")
            verbose_name_plural = _("Residence permits")
            abstract = dd.is_abstract_model(__name__, 'ResidencePermit')

    class ResidencePermits(dd.Table):
        model = 'avanti.ResidencePermit'
        required_roles = dd.login_required(ClientsStaff)

    class ResidenceReason(BabelDesignated):

        class Meta:
            app_label = 'avanti'
            verbose_name = _("Residence reason")
            verbose_name_plural = _("Residence reasons")
            abstract = dd.is_abstract_model(__name__, 'ResidenceReason')

    class ResidenceReasons(dd.Table):
        model = 'avanti.ResidenceReason'
        required_roles = dd.login_required(ClientsStaff)
...
class Client(contacts.Person, BeIdCardHolder, UserAuthored, ClientBase,
    ...
    if dd.plugins.avanti.with_asylum:
        ...
        residence_permit = dd.ForeignKey('avanti.ResidencePermit', blank=True, null=True)
        residence_reason = dd.ForeignKey('avanti.ResidenceReason', blank=True, null=True)
    # else:
    #     residence_state = dd.DummyField()
    #     residence_permit = dd.DummyField()
    #     residence_reason = dd.DummyField()
...
class ClientDetail(dd.DetailLayout):
    ...
    db_tab = dd.Panel("""
    changes.ChangesByMaster #locked_by
...
if dd.plugins.avanti.with_asylum:
    ...
    class ClientDetail(ClientDetail):
        ...
        main = "general person contact family notes db_tab"
        ...
        # card_type #card_number card_issuer card_valid_from card_valid_until
        # needs_work_permit has_contact_pcsw has_contact_work_office
        # clients.ContactsByClient excerpts.ExcerptsByProject:30
        # """,
        #                   label=_("Person"))
        ...
        birth_date age:10 gender:10
        residence_reason reason_of_stay
        client_state user language
        residence_permit residence_state
        """

        # notes = dd.Panel("""
        # comments.CommentsByRFC:60 uploads.UploadsByProject:30
        # """, label=_("Notes"),
        # required_roles=dd.login_required(CareerUser))


    from lino.modlib.comments.ui import CommentsByRFC
    CommentsByRFC.default_display_modes = {None: constants.DISPLAY_MODE_GRID}
    # CommentsByRFC.default_display_modes = {None: constants.DISPLAY_MODE_HTML}
