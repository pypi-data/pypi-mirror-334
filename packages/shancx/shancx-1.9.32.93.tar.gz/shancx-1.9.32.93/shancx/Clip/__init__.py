...
def clip_lat(data, bounds, step, pad=False):
    return clip(data, bounds.n, bounds.s, step, pad)

def clip_lon(data, bounds, step, pad=False):
    return clip(data, bounds.w, bounds.e, step, pad)
"""
if __name__ == "__main__":
    bounds = Bounds(65, -65, 40, 170)
    lat_clipped = clip_lat(lat, bounds, 0.04)
    print(f"Clipped Latitude Data Length: {len(lat_clipped)}")
    lon_clipped = clip_lon(lon, bounds, 0.04)
