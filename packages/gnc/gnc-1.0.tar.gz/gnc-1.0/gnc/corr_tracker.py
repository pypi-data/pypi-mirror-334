...
"""
SEARCH_RADIUS_FACTOR– коефіцієнт радіусу пошуку об'єкта.
CORRELATION_THRESHOLD=0.85 – поріг кореляції для прийняття відповідності.
ALT_SEARCH_RADIUS_FACTOR=0 – альтернативний коефіцієнт радіусу пошуку (для випадків втрати об'єкта).
SIGMA_EXP=5.0 – параметр згладжування.
...
"""
SEARCH_RADIUS_FACTOR = 4
CORRELATION_THRESHOLD = 0.965
...
class CorrTracker(object):
    ...
    def __init__(self, search_radius_factor=SEARCH_RADIUS_FACTOR, correlation_threshold=CORRELATION_THRESHOLD, alt_search_radius_factor=ALT_SEARCH_RADIUS_FACTOR, sigma_exp=SIGMA_EXP,
                 clone_image=CLONE_IMAGE, depth=DEPTH, zoom_factor=ZOOM_FACTOR, mode_1d=MODE_1D, contrast_threshold=CONTRAST_THRESHOLD, contrast_k_size=CONTRAST_K_SIZE,
        ...
        _gnc.PyTrackerCorr_swiginit(self, _gnc.new_PyTrackerCorr(self.search_radius_factor, self.correlation_threshold,
                                                                 self.alt_search_radius_factor, self.sigma_exp, self.clone_image, self.depth,
                                                                 self.zoom_factor, self.mode_1d, self.contrast_threshold, self.contrast_k_size,
