...
def group_ssr_records(ssrcombo_file, logger):
    ...
    logger.info(f"Total grouped records: {len(merged_data)}")
    
    return merged_data

def filter_hotspot_records(all_records_df, ssrcombo_file, logger=None):
    """Filter records based on cyclical variation only."""
    try:
        if logger:
        ...
        if filtered_groups is not None and not filtered_groups.empty:
            result = filtered_groups.drop(columns=['motif_variations', 'concat_column'])
        else:
            result = pd.DataFrame(columns=all_records_df.columns)
            
        logger.info(f"Records after variation filtering: {len(result)}")
        return result
...
def main(args=None):
    ...
    if hasattr(args, 'reference') and args.reference is not None:
        ...
        if not different_repeats_df.empty:
            ...
            # Write all variations without count columns
            ...
            # Add count columns only for final hotspot filtering
            hotspot_df['repeat_count'] = hotspot_df['repeat'].str.count(':') + 1
            hotspot_df['genomeID_count'] = hotspot_df['genomeID'].str.count(':') + 1
            
            # Apply filtering only for mutational hotspots
            filtered_hotspot_df = hotspot_df[
                (hotspot_df['repeat_count'] >= min_repeat_count) &
                (hotspot_df['genomeID_count'] >= min_genome_count)
            ]
            
            logger.info(f"Records after filtering (repeat_count >= {min_repeat_count}, "
                       f"genomeID_count >= {min_genome_count}): {len(filtered_hotspot_df)}")
            
            filtered_hotspot_df.to_csv(hotspot_file, index=False)
