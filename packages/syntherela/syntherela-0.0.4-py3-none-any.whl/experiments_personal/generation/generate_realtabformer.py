...
args.add_argument("--dataset-name", type=str, default="airbnb-simplified_subsampled_100")
...
join_on = metadata.get_primary_key(root_table_name) if renamed_keys.keys() == 0 else join_on_name
...
if retrain or not(os.path.exists(parent_model_path) and len(os.listdir(parent_model_path)) > 0):
    ...
    parent_model.fit(parent_df.drop(join_on, axis=1), full_sensitivity=full_sensitivity, device="mps")
...
for child_table_name in metadata.get_children(root_table_name):
    ...
    for i in range(1,4):
        ...
        logger.debug("Sampling from parent model...")
        ...
        logger.debug("Sampling from child model...")
        ...
        child_primary_key = metadata.get_primary_key(child_table_name) if child_table_name not in added_keys else 'index'
