...
class EntityXControl:
    ...
    def get_one(self, select=None, where=None, order_by=None, group_by=None, having=None, offset=None, 
        ...
        Args:
            ...
            extra_attrs (list[str]|str, optional):是否递归的最大层级
        ...
        if to_dict:
            ...
            #         res, recurse=recurse, backrefs=backrefs, max_depth=max_depth, only=query.builds['select'])
            
            if extra_attrs and not isinstance(extra_attrs, list):
                extra_attrs = extra_attrs.split(',')
            
            if select:
                im.data = model_to_dict(
                    res, recurse=recurse, backrefs=backrefs, max_depth=max_depth, extra_attrs=extra_attrs, only=query.builds['select'])
            else:
                im.data = model_to_dict(
                    res, recurse=recurse, backrefs=backrefs, max_depth=max_depth, extra_attrs=extra_attrs)
        else:
            im.data = res

        return im

    def get_one_by_id(self, id, select=None, to_dict=False, recurse=False, backrefs=False, max_depth=1):
        """ 按主键id获取一条记录
        ...
        im = self.get_one(select=select, where={self.key_name: id}, 
                    to_dict=to_dict, recurse=recurse, backrefs=backrefs, max_depth=max_depth)
        return im
    ...
    def get_list(self, select=None, where=None, order_by=None, group_by=None, having=None, limit=None, offset=None, 
        ...
        if to_dict:
            # im.data = [model_to_dict(
            #         d, recurse=recurse, backrefs=backrefs, max_depth=max_depth, only=query.builds['select']) for d in query]
            ...
            if select:
                im.data = [model_to_dict(d, recurse=recurse, backrefs=backrefs, extra_attrs=extra_attrs, 
