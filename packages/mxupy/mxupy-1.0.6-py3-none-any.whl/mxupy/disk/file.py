...
def file_name(filename, type='user', user_id=-1):
    """ 获取用户文件完整路径
    ...
    Returns:
        str: 完整路径
    """
    return mu.file_dir(type, user_id) + '\\' + os.path.basename(filename)
    
def file_exists2(filename, type='user', user_id=-1):
    """ 用户文件是否存在
    ...
    Returns:
        bool: 存在否
    """
    ufn = file_name(filename, type, user_id)
...
def can_access_file_exts():
    ...
    """
    # 读取配置信息
    api_server = mu.read_server().get('api_server', {})
    # 可访问哪些扩展名的文件
...
def read_file(filename, type='user', user_id=-1, download=False):
    """ 读取用户文件
...
# @check_access_token
def upload_user_file(file:UploadFile, keep = True, override = False, chunk_index = -1, total_chunks = 1, *, user_id, access_token):
    """ 上传用户文件
    ...
    try:
        ...
        if finish:
            im = combin_upload_file(bn, chunk_index + 1, user_id)
            if im.error:
                return im
...
def combin_upload_file(filename, chunks, user_id):
    """ 合并上传文件
...
def upload_file_to_server(url, file_path, keep = True, override = False, *, user_id, access_token):
    """ 从本地上传文件到服务器
