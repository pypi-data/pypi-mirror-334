...
from mxupy import IM, get_method, read_server, get_attr
...
class ApiServer:
    ...
    def __init__(self):
        ...
        # 读取配置信息
        api_server = read_server().get('api_server', {})
        ...
        self.headers = {'Cache-Control': 'max-age=' + str(self.access_file_max_age)}
    ...
    def call(self, name, params) -> IM:
        ...
        try:
            ...
            if not method:
                im = IM(False, f"Module or function not found.", code=404)
                print(im)
                return im
            ...
            im = IM(False, f"An error occurred: {mu.getErrorStackTrace()}")
            print(im)
            return im

        return im
        
    def call_for_post(self, params:dict) -> IM:
    ...
    # 读取文件/上传文件
    def read_file(self, filename:str, type:str='user', userId:int = -1, download:bool=False) -> FileResponse:
        return mu.read_file(filename, type, userId, download)
    def upload_file(self, file:UploadFile = File(...), keep:bool = Form(True), override:bool = Form(False), 
                    chunk_index:int = Form(-1), total_chunks:int = Form(1), 
                    *, userId:int = Form(...), access_token:str = Form(...)) -> IM:
        return mu.upload_user_file(file, keep, override, chunk_index, total_chunks, user_id=userId, access_token=access_token)
