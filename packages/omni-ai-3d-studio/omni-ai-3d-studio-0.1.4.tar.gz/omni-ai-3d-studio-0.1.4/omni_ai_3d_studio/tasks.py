...
def generate_mesh_task(data):
    ...
    try:
        ...
        print("Replicate API Response:", output)  # Debugging Log

        if not output or not isinstance(output, dict) or len(output) == 0:
            return {"error": "Mesh generation failed or invalid response from Replicate"}
        ...
        for key, file_obj in output.items():
            if isinstance(file_obj, replicate.helpers.FileOutput):
                ...
                output_files[key] = f"http://127.0.0.1:5000/static/uploads/{local_filename}"

        # If no valid files were saved, return an error
        if not output_files:
            return {"error": "No valid output files received from Replicate"}

        return {"generated_files": output_files}  # ✅ JSON Serializable Response
