...
class HyGraphCSVLoader:
    ...
    def load_edges_from_csv(self, csv_path: str, label: str):
        """
        Read the entire edges CSV, sort by start_time, then process in chunks.
        """
        print(f"\n[Edges] Loading from {csv_path} with label={label}")

        # 1) Read entire CSV (eagerly)
        df = pl.read_csv(csv_path, dtypes=EDGES_SCHEMA)

        # 2) Sort globally by 'start_time' ascending
        ...
        while True:
            # slice a chunk
            batch = df.slice(offset, self.max_rows_per_batch)
            if batch.height == 0:
                break

            batch_idx += 1
            print(f"   -> Processing Edge Batch #{batch_idx} with {batch.height} rows (offset={offset})")

            # 3) pass to your existing batch processor
            self._process_edge_batch(batch, label)

            offset += batch.height

        print(f"[Edges] Completed loading {csv_path}, total {total_rows} rows.\n")
