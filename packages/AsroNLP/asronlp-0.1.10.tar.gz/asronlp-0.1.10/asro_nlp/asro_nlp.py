...
class AsroNLP:
    ...
    def initialize_paths(self):
        ...
        self.normalization_path = os.path.join(self.base_path, "data/kamuskatabaku.xlsx")
        self.news_dictionary_path = os.path.join(self.base_path, "data/news_dictionary.txt")
        self.root_words_path = os.path.join(self.base_path, "data/kata-dasar.txt")
        self.lexicon_positive_path = os.path.join(self.base_path, "data/kamus_positive.xlsx")
        self.lexicon_negative_path = os.path.join(self.base_path, "data/kamus_negative.xlsx")
    ...
    def load_news_media(self):
        with open(self.news_dictionary_path, "r", encoding="utf-8") as f:
            return set(normalize_media_name(line.strip()) for line in f.readlines())
    ...
    def process_dataframe(self, df):
        # Ensure the correct text column is available
        text_column = 'full_text' if 'full_text' in df.columns else 'comment' if 'comment' in df.columns else None
        ...
        # Perform sentiment analysis and capture results
        ...
        df['Positive_Words'] = df['Sentiment_Results'].apply(lambda x: ', '.join(x['Positive_Words']))
        df['Negative_Words'] = df['Sentiment_Results'].apply(lambda x: ', '.join(x['Negative_Words']))
        df.drop('Sentiment_Results', axis=1, inplace=True)  # Cleanup
        ...
        df['Source_Type'] = df.apply(lambda x: self.detect_source_type(x[channel_title_column], x[text_column]), axis=1)

    def detect_source_type(self, channel_title, text):
        if not isinstance(channel_title, str):
            return 'Individual'
        normalized_title = normalize_media_name(channel_title)
        if any(media_name in normalized_title for media_name in self.news_media):
            return 'Media'
        return 'Individual'
 ...
 #   nlp = AsroNLP()
  #  nlp.preprocess_and_analyze('input.xlsx')
