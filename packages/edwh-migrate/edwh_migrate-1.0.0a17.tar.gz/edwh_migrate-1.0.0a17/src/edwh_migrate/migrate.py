...
class Migration:
    ...
    def check_requires(self, db: DAL, seen: set[str] = None) -> None:
        """
        ...
        Raises:
            RequimentsNotMet
        """
        requires = set(self.requires) - (seen or set())
        ...
        installed = {
            _.name: _.installed
            for _
            in db(db.ewh_implemented_features.name.belongs(requires)).iterselect()
        }
        ...
        if len(installed) < len(requires) or not all(installed.values()):
            db.close()

            missing = requires - set(installed.keys())
...
def activate_migrations(config: Optional[Config] = None, max_time: int = TEN_MINUTES) -> bool:
    ...
    successes = []
    installed = set() # for some reason `ewh_implemented_features` doesn't always work well?? Quick fix
    ...
    for name, migration in migrations:
        ...
        if should_run(db, name):
            ...
            migration.check_requires(db, installed)
