...
def migration(
    ...
    if func is None and requires:
        ...
        def decorator(decorated: Migration) -> Migration:
            @wraps(decorated)
            def with_requires(db: DAL) -> bool:
                # check requirements

                print(
                    "debug:",
                    db(db.ewh_implemented_features).select().as_list(),
                    db.executesql(f"""
                    SELECT * FROM {db.ewh_implemented_features._rname};
                    """)
                )
                ...
                if len(installed) < len(required_names):
                    ...
                    missing = set(required_names) - set(installed)
                    print(decorated.__name__, f"REQUIREMENTS NOT MET ({missing = })")
...
def activate_migrations(config: Optional[Config] = None, max_time: int = TEN_MINUTES) -> bool:
    ...
    for name, function in migrations:
        ...
        if should_run(db, name):
            ...
            # after actual commit/rollback - also write status:
            mark_migration(db_for_this_function, name, installed=result)
            db_for_this_function.commit()
