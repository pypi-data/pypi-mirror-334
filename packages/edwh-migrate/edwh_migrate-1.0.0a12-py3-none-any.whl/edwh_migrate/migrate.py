...
class Migration:
    ...
    def check_requires(self, db: DAL) -> None:
        ...
        print(
            "latest:",
            db(db.ewh_implemented_features).select(orderby=~db.ewh_implemented_features.last_update_dttm).first(),
            "depends:",
            db(db.ewh_implemented_features.name.belongs(self.requires))
            .select(db.ewh_implemented_features.installed)
            .column("installed"),
        )

        installed = (
            db(
                db.ewh_implemented_features.name.belongs(self.requires)
        ...
        if len(installed) < len(self.requires):
            db.close()

            missing = set(self.requires) - set(installed)
            print(f"debug: {self.requires = }; {installed = }")
