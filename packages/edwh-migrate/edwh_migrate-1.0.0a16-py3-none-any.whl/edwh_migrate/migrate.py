...
class Migration:
    ...
    def check_requires(self, db: DAL) -> None:
        ...
        installed = {
            _.name: _.installed
            for _
            in db(db.ewh_implemented_features.name.belongs(self.requires)).iterselect()
        }

        latest = db(db.ewh_implemented_features).select(orderby=~db.ewh_implemented_features.last_update_dttm).first()
