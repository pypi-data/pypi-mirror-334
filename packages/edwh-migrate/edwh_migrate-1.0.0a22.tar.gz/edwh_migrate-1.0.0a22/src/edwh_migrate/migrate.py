...
def try_setup_db(config: Optional[Config] = None, max_time: int = TEN_MINUTES, **options) -> typing.Optional[DAL]:
    """
    ...
    while time.time() - started < max_time:
        try:
            db = setup_db(config=config, **options)
...
def activate_migrations(config: Optional[Config] = None, max_time: int = TEN_MINUTES) -> bool:
    ...
    db = try_setup_db(config, max_time=max_time, appname="edwh-migrate")
    if not db:
    ...
    for name, migration in migrations:
        print("test:", name)
        print("debug pre", db(db.ewh_implemented_features.installed == True).count(), "implemented features")
        ...
        print("debug post", db(db.ewh_implemented_features.installed == True).count(), "implemented features")
