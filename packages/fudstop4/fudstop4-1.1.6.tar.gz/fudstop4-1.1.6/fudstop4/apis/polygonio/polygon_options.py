                ...
                async with self.pool.acquire() as connection:
                    ...
                    if batch_data:
                        try:
                            ...
                            logging.error(f"Error during final batch insert: {e.args[0].split(' ')[1].strip()}")
                            missing_column = f"{e.args[0].split(' ')[1].strip()}"
                            try:
                                if '.' in missing_column:
                                    missing_column = missing_column.replace('.','')
                                await self.add_column_to_table(table_name=table_name, column_name=missing_column, column_type="double precision")
                                print(f"Missing column added!")
                            except Exception as e:
                                await self.add_column_to_table(table_name=table_name, column_name=missing_column, column_type="TEXT")
