...
class NeonClient:
    """A client for interacting with the Telegram API."""
    ...
    def __init__(
        ...
        parser_path = os.path.join(workdir, "schema", "api.tl")
        self.parser = TLParser(parser_path)
        self.running = False
    ...
    async def start(self) -> None:
        ...
        self.running = True  # Set running to True before invoking any methods
        if not self.session.auth_key or not self.session.server_salt:
            ...
            await self.invoke(req_pq)  # Now safe to call since self.running is True
