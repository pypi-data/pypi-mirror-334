...
class NeonClient:
    ...
    def __init__(
        ...
        self.auth = Auth(self)
        self.parser = TLParser(os.path.join(workdir, "parser", "api.tl"))
        self.running = False
        self.loop: Optional[asyncio.AbstractEventLoop] = None

    async def start(self) -> None:
        """Starts the client and authenticates if necessary."""
    ...
    async def stop(self) -> None:
        """Stops the client and saves the session."""
        self.running = False
        if self.connection:
    ...
    async def invoke(self, function: TLFunction) -> bytes:
        ...
        self.connection.send(serialized)
