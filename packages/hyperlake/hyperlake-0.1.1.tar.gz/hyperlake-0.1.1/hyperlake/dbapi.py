...
import hyperlake.client
import hyperlake.exceptions
import hyperlake.logging
from hyperlake import constants
from hyperlake.constants import LENGTH_TYPES
from hyperlake.constants import PRECISION_TYPES
from hyperlake.constants import SCALE_TYPES
from hyperlake.exceptions import DatabaseError
from hyperlake.exceptions import DataError
from hyperlake.exceptions import Error
from hyperlake.exceptions import IntegrityError
from hyperlake.exceptions import InterfaceError
from hyperlake.exceptions import InternalError
from hyperlake.exceptions import NotSupportedError
from hyperlake.exceptions import OperationalError
from hyperlake.exceptions import ProgrammingError
from hyperlake.exceptions import Warning
from hyperlake.transaction import IsolationLevel
from hyperlake.transaction import NO_TRANSACTION
from hyperlake.transaction import Transaction
...
class Connection:
    """Hyperlake supports transactions and the ability to either commit or rollback
    ...
    def __init__(
        ...
        self._client_session = hyperlake.client.ClientSession(
            user=user,
            catalog=catalog,
            schema=schema,
            source=source,
        ...
        if http_session is None:
            self._http_session = hyperlake.client.HyperlakeRequest.http.Session()
    ...
    def _create_request(self):
        return hyperlake.client.HyperlakeRequest(
            self.host,
            self.port,
    ...
    def _use_legacy_prepared_statements(self):
        ...
        if value is None:
            try:
                query = hyperlake.client.HyperlakeQuery(
...
class Cursor:
    ...
    def rowcount(self):
        ...
        and CTAS statements based on `update_count` returned by the Hyperlake
        API.

        If the rowcount can't be determined, -1 will be returned.

        Hyperlake cannot reliably determine the number of rows returned for DQL
    ...
    def setinputsizes(self, sizes):
        raise hyperlake.exceptions.NotSupportedError
    ...
    def _prepare_statement(self, statement: str, name: str) -> None:
        ...
        query = hyperlake.client.HyperlakeQuery(self.connection._create_request(), query=sql,
    ...
    def _execute_prepared_statement(
        ...
        return hyperlake.client.HyperlakeQuery(self._request, query=sql, legacy_primitive_types=self._legacy_primitive_types)
    ...
    def _execute_immediate_statement(self, statement: str, params):
        ...
        return hyperlake.client.HyperlakeQuery(
    ...
    def _format_prepared_param(self, param):
        ...
        raise hyperlake.exceptions.NotSupportedError("Query parameter of type '%s' is not supported." % type(param))
    ...
    def execute(self, operation, params=None):
        if params:
            ...
            self._query = hyperlake.client.HyperlakeQuery(self._request, query=operation,
                                                  legacy_primitive_types=self._legacy_primitive_types)
            self._iterator = iter(self._query.execute())
        return self
    ...
    def fetchone(self) -> Optional[List[Any]]:
        ...
        try:
            ...
            return None
        except hyperlake.exceptions.HttpError as err:
            raise hyperlake.exceptions.OperationalError(str(err))
    ...
    def describe(self, sql: str) -> List[DescribeOutput]:
        ...
        try:
            ...
            self._query = hyperlake.client.HyperlakeQuery(
                self._request,
                query=sql,
                legacy_primitive_types=self._legacy_primitive_types,
            )
...
ROWID = DBAPITypeObject()  # nothing indicates row id in Hyperlake
