...
# SQLAlchemy dialect for Hyperlake, forked from the Trino Python client
...
from hyperlake import dbapi as hyperlake_dbapi
from hyperlake import logging
...
from hyperlake.dbapi import Cursor
from hyperlake.sqlalchemy import compiler
from hyperlake.sqlalchemy import datatype
from hyperlake.sqlalchemy import error
...
class HyperlakeDialect(DefaultDialect):
    def __init__(self,
    ...
    statement_compiler = compiler.HyperlakeSQLCompiler
    ddl_compiler = compiler.HyperlakeDDLCompiler
    type_compiler = compiler.HyperlakeTypeCompiler
    preparer = compiler.HyperlakeIdentifierPreparer
    ...
    # DML
    # Queries of the form `INSERT () VALUES ()` is not supported by hyperlake.
    ...
    def create_connect_args(self, url: URL) -> Tuple[Sequence[Any], Mapping[str, Any]]:
        ...
        if "source" in url.query:
            ...
            kwargs["source"] = "hyperlake-sqlalchemy"
    ...
    def get_pk_constraint(self, connection: Connection, table_name: str, schema: str = None, **kw) -> Dict[str, Any]:
        """Hyperlake has no support for primary keys. Returns a dummy"""
    ...
    def get_foreign_keys(
        ...
        """Hyperlake has no support for foreign keys. Returns an empty list."""
        return []
    ...
    def get_temp_table_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Hyperlake has no support for temporary tables. Returns an empty list."""
        return []
    ...
    def get_temp_view_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Hyperlake has no support for temporary views. Returns an empty list."""
        return []
    ...
    def get_sequence_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Hyperlake has no support for sequences. Returns an empty list."""
        return []

    def get_unique_constraints(
        self, connection: Connection, table_name: str, schema: str = None, **kw
    ) -> List[Dict[str, Any]]:
        """Hyperlake has no support for unique constraints. Returns an empty list."""
        return []

    def get_check_constraints(
        self, connection: Connection, table_name: str, schema: str = None, **kw
    ) -> List[Dict[str, Any]]:
        """Hyperlake has no support for check constraints. Returns an empty list."""
        return []
    ...
    def get_table_comment(self, connection: Connection, table_name: str, schema: str = None, **kw) -> Dict[str, Any]:
        ...
        except error.HyperlakeQueryError as e:
            if e.error_name in (
    ...
    def has_sequence(self, connection: Connection, sequence_name: str, schema: str = None, **kw) -> bool:
        """Hyperlake has no support for sequence. Returns False indicate that given sequence does not exists."""
        return False
    ...
    def _raw_connection(self, connection: Union[Engine, Connection]) -> hyperlake_dbapi.Connection:
        if isinstance(connection, Engine):
            return connection.raw_connection()
        return connection.connection

    def _get_default_catalog_name(self, connection: Connection) -> Optional[str]:
        dbapi_connection: hyperlake_dbapi.Connection = self._raw_connection(connection)
    ...
    def do_rollback(self, dbapi_connection: hyperlake_dbapi.Connection):
        if dbapi_connection.transaction is not None:
            dbapi_connection.rollback()

    def set_isolation_level(self, dbapi_conn: hyperlake_dbapi.Connection, level: str) -> None:
        dbapi_conn._isolation_level = hyperlake_dbapi.IsolationLevel[level]

    def get_isolation_level(self, dbapi_conn: hyperlake_dbapi.Connection) -> str:
        return dbapi_conn.isolation_level.name

    def get_default_isolation_level(self, dbapi_conn: hyperlake_dbapi.Connection) -> str:
        return hyperlake_dbapi.IsolationLevel.AUTOCOMMIT.name
