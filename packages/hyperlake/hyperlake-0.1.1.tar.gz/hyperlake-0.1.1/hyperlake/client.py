...
This module implements the Hyperlake Trino protocol to submit SQL statements, track
their state and retrieve their result as described in
https://github.com/trinodb/trino/wiki/HTTP-Protocol
and Trino source code (Note: This is fork of the Trino Python Client).
...
Hyperlake queries are managed by the ``HyperlakeQuery`` class. HTTP requests are
managed by the ``HyperlakeRequest`` class. the status of a query is represented
by ``HyperlakeStatus`` and the result by ``HyperlakeResult``.


The main interface is :class:`HyperlakeQuery`: ::

    >> request = HyperlakeRequest(host='coordinator', port=8080, user='test')
    >> query =  HyperlakeQuery(request, sql)
...
from hyperlake import constants
from hyperlake import exceptions
from hyperlake._version import __version__
from hyperlake.auth import Authentication
from hyperlake.exceptions import HyperlakeExternalError
from hyperlake.exceptions import HyperlakeQueryError
from hyperlake.exceptions import HyperlakeUserError
from hyperlake.mapper import RowMapper
from hyperlake.mapper import RowMapperFactory
...
class ClientSession:
    ...
    :param catalog: to query. The *catalog* is associated with a Hyperlake 
    ...
    :param properties: set specific Hyperlake behavior for the current
...
class HyperlakeRequest:
    """
    Manage the HTTP requests of a Hyperlake query.
    ...
    Please refer to :class:`HyperlakeStatus` to access the status returned by
    :meth:`HyperlakeRequest.process`.
    ...
    @staticmethod
    def _process_error(error, query_id: Optional[str]) -> Union[HyperlakeExternalError, HyperlakeQueryError, HyperlakeUserError]:
        error_type = error["errorType"]
        if error_type == "EXTERNAL":
            raise exceptions.HyperlakeExternalError(error, query_id)
        elif error_type == "USER_ERROR":
            return exceptions.HyperlakeUserError(error, query_id)

        return exceptions.HyperlakeQueryError(error, query_id)
    ...
    def process(self, http_response: Response) -> HyperlakeStatus:
...
class HyperlakeResult:
    """
    Represent the result of a Hyperlake query as an iterator on rows.
...
class HyperlakeQuery:
    """Represent the execution of a SQL statement by Hyperlake."""

    def __init__(
            self,
            request: HyperlakeRequest,
            query: str,
        ...
        self._next_uri = None
        self._query = query
        self._result: Optional[HyperlakeResult] = None
    ...
    def execute(self, additional_http_headers=None) -> HyperlakeResult:
        """Initiate a Hyperlake query by sending the SQL statement
        ...
        """
        if self.cancelled:
            raise exceptions.HyperlakeUserError("Query has been cancelled", self.query_id)

        try:
            response = self._request.post(self._query, additional_http_headers)
        except requests.exceptions.RequestException as e:
            raise hyperlake.exceptions.HyperlakeConnectionError("failed to execute: {}".format(e))
        ...
        self._result = HyperlakeResult(self, rows)
    ...
    def fetch(self) -> List[Union[List[Any]], Any]:
        ...
        try:
            ...
            raise hyperlake.exceptions.HyperlakeConnectionError("failed to fetch: {}".format(e))
        status = self._request.process(response)
        self._update_state(status)
        if status.next_uri is None:
            self._finished = True
    ...
    def cancel(self) -> None:
        ...
        try:
            ...
            raise hyperlake.exceptions.HyperlakeConnectionError("failed to cancel query: {}".format(e))
...
# Hyperlake Spooled protocol transfer objects
