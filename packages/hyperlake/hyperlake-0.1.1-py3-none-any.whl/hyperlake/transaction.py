...
class Transaction:
    def __init__(self, request: hyperlake.client.HyperlakeRequest) -> None:
    ...
    @property
    def request(self) -> hyperlake.client.HyperlakeRequest:
        return self._request

    def begin(self) -> None:
        response = self._request.post(START_TRANSACTION)
        if not response.ok:
            raise hyperlake.exceptions.DatabaseError(
    ...
    def commit(self) -> None:
        query = hyperlake.client.HyperlakeQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self) -> None:
        query = hyperlake.client.HyperlakeQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise hyperlake.exceptions.DatabaseError(
