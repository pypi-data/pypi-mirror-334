...
class HyperlakeSQLCompiler(compiler.SQLCompiler):
    def limit_clause(self, select, **kw):
        """
        Hyperlake support only OFFSET...LIMIT but not LIMIT...OFFSET syntax.
        """
        text = ""
    ...
    def visit_table(self, table, asfrom=False, iscrud=False, ashint=False,
        ...
        sql = super(HyperlakeSQLCompiler, self).visit_table(
    ...
    def add_catalog(sql, table):
        ...
        if (
                'hyperlake' not in table.dialect_options
                or 'catalog' not in table.dialect_options['hyperlake']
        ):
            return sql

        catalog = table.dialect_options['hyperlake']['catalog']
        sql = f'"{catalog}".{sql}'
        return sql
...
class HyperlakeDDLCompiler(compiler.DDLCompiler):
    pass


class HyperlakeTypeCompiler(compiler.GenericTypeCompiler):
...
class HyperlakeIdentifierPreparer(compiler.IdentifierPreparer):
    reserved_words = RESERVED_WORDS

    def format_table(self, table, use_schema=True, name=None):
        result = super(HyperlakeIdentifierPreparer, self).format_table(table, use_schema, name)
        return HyperlakeSQLCompiler.add_catalog(result, table)
