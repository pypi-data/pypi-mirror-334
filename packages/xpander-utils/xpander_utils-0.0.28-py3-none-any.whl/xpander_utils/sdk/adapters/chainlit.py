...
class ChainlitAdapter(SDKAdapter):
    ...
    async def process_tool_calls(self, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], on_tool_call: Optional[Callable], local_tools: Dict[str, Callable] = None) -> str | None:
        """
        ...
        tool_calls_names = [tc.name for tc in tool_calls_list]

        # Count tool calls per execution
        execution_id = self.agent.execution.id

        if execution_id not in self.execution_tool_calls:
            self.execution_tool_calls[execution_id] = tool_calls_names
        else:
            # Add new tool calls to the execution tracking
            self.execution_tool_calls[execution_id].extend(tool_calls_names)

            # Check max tool call count
            if len(self.execution_tool_calls[execution_id]) >= MAX_TOOL_CALLS:
                cl.context.current_step.show_input = False
                cl.context.current_step.default_open = True
                await cl.context.current_step.update()
                raise Exception(f"Oops! You've reached the maximum allowed tool executions ({MAX_TOOL_CALLS}). Please adjust your input or try again later.")
            ...
            if len(self.execution_tool_calls[execution_id]) > MAX_SAME_TOOL_IN_A_ROW:
                ...
                if len(set(last_three_calls)) == 1:  # All three are the same
                    ...
                    await cl.context.current_step.update()
                    raise Exception(f"Oops! The last {MAX_SAME_TOOL_IN_A_ROW} tool calls were the same ({last_three_calls[0]}). Please adjust your input.")
        ...
        for raw_tool_call in tool_calls.values():
            ...
            graph_item = self.agent.graph.find_node_by_item_id(item_id=tool_call.name)
            ...
            if tool_agent_id:
                ...
                if sub_agent_node:
                    if on_tool_call:
                        await on_tool_call(tool_call=tool_call,graph_item=sub_agent_node)
                    tool_name = sub_agent_node.name
            else:
                # report to parent window
                graph_item = self.agent.graph.find_node_by_item_id(item_id=tool_call.name)
                if graph_item and on_tool_call:
                    await on_tool_call(tool_call=tool_call,graph_item=graph_item)
