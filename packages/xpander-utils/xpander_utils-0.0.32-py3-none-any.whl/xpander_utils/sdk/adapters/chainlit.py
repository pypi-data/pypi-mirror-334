    ...
    async def process_tool_calls(self, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], on_node_enter: Optional[Callable], on_node_error: Optional[Callable], local_tools: Dict[str, Callable] = None) -> str | None:
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if tool_call.type == ToolCallType.LOCAL:
                ...
                if is_repeating_tool_call:
                    tool_call_result = ToolCallResult(function_name=tool_call.name,tool_call_id=tool_call.tool_call_id,payload=tool_call.payload,result=f"Oops! The last {MAX_SAME_TOOL_IN_A_ROW} tool calls were the same ({last_three_calls[0]}). The agent cannot proceed with this tool.",is_success=False,is_error=True)
                    self.agent.add_tool_call_results(tool_call_results=[tool_call_result])
                else:
