...
class ChainlitAdapter(SDKAdapter):
    ...
    async def process_tool_calls(self, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], on_node_enter: Optional[Callable], on_node_error: Optional[Callable], agent_custom_functions: list[GraphItem] = [], local_tools: Dict[str, Callable] = None) -> str | None:
        """
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if is_sub_agent:
                if is_agent_end_tool:
                    ...
                    tool_name = f"{self.agent.name} - {graph_item.name}"
