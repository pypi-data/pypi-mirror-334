...
@shell.define
class Fixel2Tsf(specs.ShellDef["Fixel2Tsf.Outputs"]):
    ...
    fixel_in: ImageIn = shell.arg(
        argstr="",
        position=1,
        help="""the input fixel data file (within the fixel directory)""",
    )
    ...
    angle: float | None = shell.arg(
        default=None,
        argstr="-angle",
        help="""the max anglular threshold for computing correspondence between a fixel direction and track tangent (default = 45 degrees)""",
    )
    ...
    class Outputs(specs.ShellOutputs):
        tsf: File = shell.outarg(
            argstr="",
            position=3,
            path_template="tsf.txt",
