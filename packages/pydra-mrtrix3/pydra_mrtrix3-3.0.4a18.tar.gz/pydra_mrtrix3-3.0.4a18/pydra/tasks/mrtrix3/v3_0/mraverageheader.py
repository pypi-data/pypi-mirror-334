...
@shell.define
class MrAverageheader(specs.ShellDef["MrAverageheader.Outputs"]):
    ...
    # Arguments
    in_file: MultiInputObj[ImageIn] = shell.arg(
        argstr="",
        position=1,
    ...
    # Options
    padding: float | None = shell.arg(
        default=None,
        argstr="-padding",
        help="""boundary box padding in voxels. Default: 0""",
    )
    spacing: str | None = shell.arg(
        default=None,
        argstr="-spacing",
        help="""Method for determination of voxel spacings based on the set of input images and the average header axes (see Description). Valid options are: min_projection,mean_projection,min_nearest,mean_nearest; default = mean_projection""",
        allowed_values=[
            "min_projection",
            "mean_projection",
            "min_nearest",
    ...
    fill: bool = shell.arg(
        default=False,
        argstr="-fill",
        help="""set the intensity in the first volume of the average space to 1""",
    )
