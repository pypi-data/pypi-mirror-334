...
@shell.define
class DirRotate(specs.ShellDef["DirRotate.Outputs"]):
    ...
    executable = "dirrotate"

    # Arguments
    in_: File = shell.arg(
        argstr="",
        position=1,
        help="""the input direction file""",
    )
    ...
    number: int | None = shell.arg(
        default=None,
        argstr="-number",
        help="""number of rotations to try (default: 10000000)""",
    )
    ...
    class Outputs(specs.ShellOutputs):
        out: File = shell.outarg(
            ...
            help="""the output direction file""",
        )
