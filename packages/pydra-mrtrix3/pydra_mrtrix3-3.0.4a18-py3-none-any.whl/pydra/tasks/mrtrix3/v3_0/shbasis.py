...
@shell.define
class ShBasis(specs.ShellDef["ShBasis.Outputs"]):
    ...
    # Arguments
    SH: MultiInputObj[ImageIn] = shell.arg(
        argstr="",
        position=1,
        help="""the input image(s) of SH coefficients.""",
    )

    # Options
    convert: str | None = shell.arg(
        default=None,
        argstr="-convert",
        help="""convert the image data in-place to the desired basis; options are: old,new,force_oldtonew,force_newtoold.""",
        allowed_values=["old", "new", "force_oldtonew", "force_newtoold"],
    )
