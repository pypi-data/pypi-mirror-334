...
@shell.define
class FixelReorient(specs.ShellDef["FixelReorient.Outputs"]):
    ...
    warp: ImageIn = shell.arg(
        argstr="",
        position=2,
        help="""a 4D deformation field used to perform reorientation.""",
    )
    ...
    class Outputs(specs.ShellOutputs):
        fixel_out: Directory = shell.outarg(
            ...
            help="""the output fixel directory. If the the input and output directories are the same, the existing directions file will be replaced (providing the -force option is supplied). If a new directory is supplied, then the fixel directions and all other fixel data will be copied to the new directory.""",
        )
