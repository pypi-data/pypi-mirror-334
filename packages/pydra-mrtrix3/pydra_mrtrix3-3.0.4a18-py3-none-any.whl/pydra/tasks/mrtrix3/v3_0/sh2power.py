...
@shell.define
class Sh2Power(specs.ShellDef["Sh2Power.Outputs"]):
    ...
    SH: ImageIn = shell.arg(
        argstr="",
        position=1,
        help="""the input spherical harmonics coefficients image.""",
    )

    # Options
    spectrum: bool = shell.arg(
        default=False,
        argstr="-spectrum",
        help="""output the power spectrum, i.e., the power contained within each harmonic degree (l=0, 2, 4, ...) as a 4-D image.""",
    )
    ...
    class Outputs(specs.ShellOutputs):
        power: ImageOut = shell.outarg(
            argstr="",
            position=2,
            path_template="power.mif",
            help="""the output power image.""",
        )
