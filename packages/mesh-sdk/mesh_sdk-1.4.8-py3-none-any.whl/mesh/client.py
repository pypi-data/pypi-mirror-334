...
class MeshClient:
    ...
    def chat(
        ...
        **kwargs
        ...
        # If model is specified, normalize it
        if model:
            model = normalize_model_name(model)
            
            # If provider not specified, determine it from the model using mappings
            if not provider:
                provider = get_provider_for_model(model)
                self.logger.debug(f"Provider not specified, using {provider} based on model mapping for {model}")
        
        # If no model, use provider (or default) to get model
        else:
