...
from core.telemetry import is_telemetry_globally_disabled
import functools
...
class Computer:
    ...
    def __init__(
        ...
        self._cached_wrapped_interface = None
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            # Record the action in telemetry (if enabled)
    ...
    def interface(self):
        ...
        Returns:
            The interface wrapped with telemetry logging
        """
        ...
        # Create and cache the wrapped interface
        if self._cached_wrapped_interface is None:
            self._cached_wrapped_interface = TelemetryInterface(
                self._interface, self, self._telemetry_enabled
            )
        return self._cached_wrapped_interface
    ...
    def enable_telemetry(self) -> None:
        """Enable telemetry for this computer instance."""
        self._telemetry_enabled = True

    def disable_telemetry(self) -> None:
        """Disable telemetry for this computer instance."""
...
class TelemetryInterface:
    """A wrapper for Computer interfaces that logs telemetry."""

    def __init__(self, interface, computer, telemetry_enabled: bool = True):
        """Initialize the TelemetryInterface.

        Args:
            interface: The underlying interface to wrap
            computer: The parent Computer instance
            telemetry_enabled: Whether telemetry should be enabled for this interface
        """
    ...
    def __getattr__(self, name: str) -> Any:
        """Get an attribute from the underlying interface and wrap it with telemetry logging.

        Args:
            name: The name of the attribute to get

        Returns:
            The attribute, wrapped with telemetry logging if it's a callable
        """
        attr = getattr(self._interface, name)

        # Only wrap methods, not attributes
        if not callable(attr):
            return attr

        @functools.wraps(attr)
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            """Wrapper that logs telemetry before and after method calls."""
            # Only record telemetry if enabled at both the global and instance level
            should_record = not is_telemetry_globally_disabled() and self._telemetry_enabled
            ...
            try:
                ...
                # Record the completion of the action
                if should_record:
                    record_computer_action(
                        action_type=action_type,
    ...
    async def to_screen_coordinates(self, x: float, y: float) -> tuple[float, float]:
        ...
        Returns:
            tuple[float, float]: Screen coordinates (x, y)
        """
        return await self._interface.to_screen_coordinates(x, y)
    ...
    async def to_screenshot_coordinates(self, x: float, y: float) -> tuple[float, float]:
        ...
        Returns:
            tuple[float, float]: (x, y) coordinates in screenshot space
        """
        screen_size = await self._interface.get_screen_size()
        screenshot = await self._interface.screenshot()
