...
class ProgressBar(Iterator[_T]):
    ...
    def __init__(self, iterable:Iterable[_T]=None, total=None):
        ...
        self.setting_style_format_infinity = "{prefix}{incremented} {unit} [Elapsed={time_elapsed}, Speed={speed:.1f}/s]{suffix}"
        ...
        self.setting_style_format_measurable = "{prefix}|{bar}| {percentage:.1f}% [Elapsed={time_elapsed}|ETA={time_remaining}|Speed={speed:.1f}/s|{unit}={incremented}/{total}]{suffix}"
        '''the style used for when the progressbar has a known total'''
        self.setting_format_mapping = {'unit': 'pcs', 'prefix':'', 'suffix':''}
