...
from ..models.test_run import PromptVersionConfig, WorkflowConfig, YieldedOutputTokenUsage
...
class TestRunBuilder(Generic[T]):
    ...
    def __process_entry(
        ...
        if output_function is not None:
            ...
            raise ValueError(
                "Found no output function to execute, please make sure you have either `yields_output`, `with_prompt_version_id` or `with_workflow_id` set."
            )
    ...
    def _run_test_with_local_data(
        ...
        def process_row(
            ...
            try:
                ...
                if any(isinstance(e, BaseEvaluator) for e in self._config.evaluators):
    ...
    def _run_test_with_dataset_id(
        ...
        def process_dataset_entry(
            ...
            try:
                rowData: ManualData = row.to_dict()["data"](index)
                if rowData is None:
                    raise ValueError(f"Dataset entry {index} is missing")
