...
class Command(BaseCommand):
    ...
    def handle(self, options: argparse.Namespace) -> None:
       ...
       # Obtiene la carpeta anterior al directorio actual
        ...
        #obtener la direccion de el app.zip
        app_zip_dir = os.path.join(self.flutter_dir, 'app', 'app.zip')

        # Directorio de destino para descomprimir
        app_extract_dir = os.path.join(self.flutter_dir, 'app')

        try:
            # Verificar si el archivo app.zip existe
            if os.path.exists(app_zip_dir):
                print(f"Descomprimiendo {app_zip_dir} en {app_extract_dir}...")
                
                # Descomprimir el archivo app.zip
                with zipfile.ZipFile(app_zip_dir, 'r') as zip_ref:
                    zip_ref.extractall(app_extract_dir)
                
                print("Descompresión completada.")
                
                # Eliminar el archivo ZIP
                os.remove(app_zip_dir)
                print(f"Archivo {app_zip_dir} eliminado.")
            else:
                print(f"El archivo {app_zip_dir} no existe.")
        except Exception as e:
            print(f"Error durante la descompresión o eliminación: {e}")
        ...
        self.raiz_app = app_extract_dir

        def listar_y_actualizar_pubspec():
            # Construir la ruta completa de la carpeta raíz
            ruta_actual = os.getcwd()
            ruta_parainformales = os.path.join(ruta_actual, self.raiz_app)
            pubspec_path = os.path.join(self.flutter_dir, 'pubspec.yaml')

            if not os.path.exists(ruta_parainformales):
                print(f"La carpeta '{self.raiz_app}' no existe en la ubicación actual.")
                return

            # Lista de rutas formateadas
            rutas_flutter = ["app/",]

            # Longitud de la ruta raíz para eliminarla al calcular las rutas relativas
            longitud_raiz = len(ruta_parainformales) + 1

            # Recorrer las subcarpetas de la raíz
            for subcarpeta_raiz, subdirs, _ in os.walk(ruta_parainformales):
                # Ignorar carpetas que contengan "__pycache__"
                if "__pycache__" in subcarpeta_raiz:
                    continue

                # Generar ruta relativa eliminando la longitud de la raíz
                ruta_relativa = subcarpeta_raiz[longitud_raiz:]  # Eliminar la longitud de la raíz
                if not ruta_relativa:  # Saltar la carpeta raíz
                    continue

                # Formatear la ruta para Flutter
                ruta_flutter = f"app/{ruta_relativa}/"
                rutas_flutter.append(ruta_flutter)

            # Mostrar las rutas generadas
            print("Subcarpetas encontradas para Flutter:")
            for ruta in rutas_flutter:
                print(f"  - {ruta}")

            # Actualizar el archivo pubspec.yaml
            if not os.path.exists(pubspec_path):
                print(f"El archivo '{pubspec_path}' no existe.")
                return

            try:
                # Leer el contenido del archivo YAML
                with open(pubspec_path, 'r') as archivo:
                    pubspec_data = yaml.safe_load(archivo) or {}

                # Asegurar que la sección "flutter" y "assets" existen
                if 'flutter' not in pubspec_data:
                    pubspec_data['flutter'] = {}
                pubspec_data['flutter']['assets'] = rutas_flutter

                # Guardar el archivo actualizado
                with open(pubspec_path, 'w') as archivo:
                    yaml.dump(pubspec_data, archivo, default_flow_style=False, sort_keys=False)

                print("\nArchivo 'pubspec.yaml' actualizado con las rutas de assets.")

                # Imprimir el contenido final del pubspec.yaml
                print("\nContenido final del archivo 'pubspec.yaml':")
                print(yaml.dump(pubspec_data, default_flow_style=False, sort_keys=False))

            except Exception as e:
                print(f"Error al actualizar 'pubspec.yaml': {e}")

        listar_y_actualizar_pubspec()
