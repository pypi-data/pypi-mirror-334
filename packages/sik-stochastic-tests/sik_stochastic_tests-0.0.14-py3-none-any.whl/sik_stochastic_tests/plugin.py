    ...
    def thread_worker():
        try:
            ...
            try:
                ...
                # First wrap the test function execution in a task to ensure proper separation
                async def runner():
                    return await _run_stochastic_tests(
                ...
                thread_loop.run_until_complete(runner())
                result_queue.put(("success", None))
            except Exception as e:
                result_queue.put(("error", e))
            finally:
                # Properly clean up async generators before closing the loop
                # This helps prevent "pop from empty deque" errors during loop cleanup
                try:
                    # First cancel all remaining tasks
                    to_cancel = asyncio.all_tasks(thread_loop)
                    if to_cancel:
                    ...
                    # Then shutdown async generators
                    # This is a critical step that avoids most "pop from empty deque" errors
                    try:
                        # Use a short timeout to avoid hanging
                        shutdown_future = thread_loop.create_future()
                        thread_loop.call_soon(lambda: asyncio.ensure_future(
                            thread_loop.shutdown_asyncgens(), loop=thread_loop
                        ).add_done_callback(lambda _: shutdown_future.set_result(None)))
                        thread_loop.run_until_complete(asyncio.wait_for(shutdown_future, timeout=5))
                    except (asyncio.TimeoutError, Exception):
                        pass  # Continue with cleanup if shutdown_asyncgens fails or times out
                except Exception:
                    pass
...
def _run_stochastic_tests_in_thread(
    ...
    def thread_worker() -> None:
        ...
        3. Safely runs the tests in the isolated event loop with proper async generator cleanup
        4. Implements a more robust approach to loop shutdown that avoids race conditions
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Then shutdown async generators
                    # This is a critical step that avoids most "pop from empty deque" errors
                    # But we need to handle errors here too
                    try:
                        # Use a short timeout to avoid hanging
                        shutdown_future = thread_loop.create_future()
                        thread_loop.call_soon(lambda: asyncio.ensure_future(
                        ...
                        thread_loop.run_until_complete(asyncio.wait_for(shutdown_future, timeout=5))  # noqa: E501
                    except (TimeoutError, Exception):
                        pass  # Just continue with cleanup if shutdown_asyncgens fails or times out
                except Exception:
                    pass
