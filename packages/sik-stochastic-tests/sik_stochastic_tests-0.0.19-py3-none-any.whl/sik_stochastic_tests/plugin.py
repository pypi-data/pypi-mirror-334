...
"""
    ...
    try:
        ...
        # If we're not in an event loop context, create one
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        
    # Run the stochastic tests directly in the current event loop
    # This avoids creating/destroying threads and event loops
...
# Helper function to run stochastic tests without threads
def _run_stochastic_tests_directly(
    ...
    """
    Run stochastic tests directly in the current event loop without threading.
    
    This approach uses the main event loop to run all test samples, avoiding
    the creation/destruction of event loops or threads that can cause race
    conditions in asyncio's internals.
    ...
    # Simply run the stochastic tests in the current event loop
...
def pytest_pyfunc_call(pyfuncitem: pytest.Function) -> bool | None:
    ...
    # Run the stochastic test directly in the current event loop
    # This avoids creating/destroying threads and event loops
    _run_stochastic_tests_directly(
        testfunction=testfunction,
        funcargs=funcargs,
        stats=stats,
        samples=samples,
...
async def _run_stochastic_tests(
    ...
    async def safe_gather(coroutines: list[asyncio.Task], batch_size: int | None) -> list[bool]:
        ...
        Using a single gathering approach to minimize event loop interactions.
        """
        if not coroutines:
            return []

        if batch_size and batch_size > 0:  # Use batching
            results = []
            # Process tests in batches
            for i in range(0, len(coroutines), batch_size):
                batch = coroutines[i:i + batch_size]
                
                # Create tasks for this batch and gather them
                ...
                try:
                    ...
                    # If gather itself fails (very rare), count all tests in this batch as failed
                    results.extend([False] * len(batch))
            
            return results
        else:
            # Run all tests at once - simpler case
            try:
                # Create tasks for all coroutines
                all_tasks = [asyncio.create_task(coro) for coro in coroutines]
                
                # Gather all results
                results = await asyncio.gather(*all_tasks, return_exceptions=True)
                
                # Convert exceptions to False (test failed)
                return [r if not isinstance(r, Exception) else False for r in results]
            except Exception:
                # If gather itself fails, count all tests as failed
