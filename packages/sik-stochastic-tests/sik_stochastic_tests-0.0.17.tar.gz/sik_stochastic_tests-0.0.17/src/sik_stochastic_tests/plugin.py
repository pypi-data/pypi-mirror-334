...
"""
    ...
    def thread_worker():
        # Run tests in an isolated thread with its own event loop
        try:
            ...
            def custom_exception_handler(loop, context):
                ...
                if isinstance(exc, IndexError) and str(exc) == "pop from an empty deque":
                    # Silently ignore this specific error that happens during cleanup
                    return
                # Let the default handler process all other exceptions
            ...
            try:
                ...
                # Standard asyncio cleanup sequence - this order is important
                # 1. Cancel all pending tasks
                pending = asyncio.all_tasks(thread_loop)
                for task in pending:
                    task.cancel()
                
                # 2. Allow tasks to respond to cancellation
                if pending:
                ...
                # 3. Shutdown async generators - critical for preventing deque errors
                with contextlib.suppress(Exception):
                    thread_loop.run_until_complete(thread_loop.shutdown_asyncgens())
                
                # 4. Close the loop and reset thread state
                thread_loop.close()
...
def _run_stochastic_tests_in_thread(
    ...
    This function implements the standard pattern for handling nested event loops
    in Python/asyncio. We run each test in a completely isolated thread with its
    own event loop, ensuring no interference with pytest-asyncio's event loop.
    
    The key to this approach is the proper cleanup sequence that ensures all
    asyncio resources are properly released before closing the loop.
    ...
    worker_thread = threading.Thread(target=thread_worker, daemon=True)
    worker_thread.start()
    worker_thread.join()  # Wait for completion
