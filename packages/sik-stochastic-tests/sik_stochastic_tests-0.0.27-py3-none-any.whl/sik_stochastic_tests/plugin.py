    ...
    def thread_worker() -> None:
        ...
        try:
            ...
            try:
                ...
                # Allow a small delay for any async cleaning resources
                # This is important for clients that have deferred cleanup
                async def delayed_cleanup():
                    # Small delay to allow any client cleanup to start
                    await asyncio.sleep(0.1)
                    return
                
                try:
                    # Run delayed cleanup first
                    thread_loop.run_until_complete(delayed_cleanup())
                    
                    # Get all pending tasks and run them to completion
                    current_task = asyncio.current_task(thread_loop)
                    ...
                    # Clean up asyncgens and executors
                    ...
                    # Set a global loop error handler to silence "empty deque" errors
                    # This prevents those errors from being reported during loop shutdown
                    def silence_empty_deque(loop, context):
                        exc = context.get('exception')
                        if exc and isinstance(exc, IndexError) and str(exc) == "pop from an empty deque":
                            return  # Silently ignore this specific error
                        # For any other errors, use default handling
                        loop.default_exception_handler(context)
                        
                    thread_loop.set_exception_handler(silence_empty_deque)
                    
                    # Close the loop and reset the thread's event loop
                    thread_loop.close()
...
async def run_test_function(
    ...
    if is_async:
        ...
        # For sync functions, we need to handle timeouts differently
        
        # When timeout is specified, always use the executor approach to enforce it
        if timeout is not None:
            # Additional handling for httpx clients
            # This is critical because httpx uses anyio which has special cleanup needs
            try:
                # Check if 'httpx' is in the module path or function args
                test_module = getattr(testfunction, '__module__', '')
                has_httpx = 'httpx' in test_module or 'httpx' in str(funcargs)
                
                if has_httpx:
                    # For httpx clients, we need to be more careful about cleanup
                    def run_with_httpx():
                        # Run the function and capture the result
                        result = testfunction(**funcargs)
                        
                        # Return the result without awaiting anything
                        return result
                    
                    result = await loop.run_in_executor(None, run_with_httpx)
                    
                    # If it returned an awaitable, we need to handle it specially
                    if inspect.isawaitable(result):
                        try:
                            remaining_timeout = timeout
                            return await asyncio.wait_for(result, remaining_timeout)
                        except TimeoutError:
                            raise TimeoutError(f"Test result awaitable timed out after {timeout} seconds")
                    
                    return result
            except Exception:
                # Fall back to standard approach if special handling fails
                pass
                
            # Standard approach for non-httpx functions
            future = loop.run_in_executor(None, lambda: testfunction(**funcargs))
            try:
                result = await asyncio.wait_for(future, timeout=timeout)
                
                # Handle the case where the function returned an awaitable
                if inspect.isawaitable(result):
                    try:
                        # Apply the same timeout to the awaitable result
                        remaining_timeout = timeout  # Could be more precise by tracking elapsed time
                        return await asyncio.wait_for(result, remaining_timeout)
                    except TimeoutError:
                        raise TimeoutError(f"Test result awaitable timed out after {timeout} seconds")
            ...
            # No timeout specified, but still need to handle httpx specially
            try:
                # Check if 'httpx' is in the module path or function args
                test_module = getattr(testfunction, '__module__', '')
                has_httpx = 'httpx' in test_module or 'httpx' in str(funcargs)
                ...
                if has_httpx:
                    # For httpx clients, ensure proper cleanup
                    def run_with_httpx():
                        # Run the function and capture the result
                        result = testfunction(**funcargs)
                        return result
            ...
            try:
                ...
                # If we got an asyncio-related error, try the executor approach
                if isinstance(e, (RuntimeError, AttributeError)) and "asyncio" in str(e).lower():
                    # Run in executor to isolate any internal asyncio usage
                    result = await loop.run_in_executor(None, lambda: testfunction(**funcargs))
                    
                    # Handle awaitable result from executor
                    if inspect.isawaitable(result):
                        return await result
                    
                    return result
                    
                # Re-raise original exception if not asyncio-related
                raise
