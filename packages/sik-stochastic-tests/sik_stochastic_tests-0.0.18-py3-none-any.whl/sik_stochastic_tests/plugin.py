...
"""
    ...
    def thread_worker():
        # Run all test samples in a single thread with a single event loop
        try:
            # Create a new event loop for this thread - used for ALL test samples
            thread_loop = asyncio.new_event_loop()
            asyncio.set_event_loop(thread_loop)

            try:
                # Run all test samples in this single event loop
                ...
                # Standard cleanup sequence for a single event loop
...
def _run_stochastic_tests_in_thread(
    ...
    """
    Run stochastic tests in a dedicated thread with a single event loop.
    
    This function creates a single isolated thread with a dedicated event loop
    that runs all test samples. Unlike the previous implementation, we don't
    create/destroy multiple event loops, which helps prevent race conditions
    in asyncio's internals.
