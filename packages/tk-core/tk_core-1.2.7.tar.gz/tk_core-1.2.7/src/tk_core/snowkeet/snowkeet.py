...
class Snowkeet:
    ...
    def count_table(self, table_name: str) -> int:
        ...
        count_sql = "SELECT (SELECT COUNT(*) FROM ?) AS COUNT"
    ...
    def execute_sql(self, sql_query_str: str, params: tuple = None) -> list[Row]:
        """
    ...
    def check_if_rows_exist(self, table_name: str, filter_var: str, filter_value: str) -> bool:
        ...
        Returns:
            bool: True if rows exist, False if not.
        """
        query_str = "SELECT exists(select top 1 1 from ? where ? = '?') as exist_check"
    ...
    def custom_dml(self, sql: str, params: tuple = None) -> dict:
        """
