...
class NER(nn.Module):
    ...
    def __init__(self, pretrained_path, num_classes, bidirectional=True, num_train_layers=0, rnn=nn.GRU):
        ...
        # 初始化设备信息
        self.device = None
        self.has_attention_mask: bool = None
        # 存储类别数量
    ...
    def _logits(self, tokenizies: Mapping[str, torch.Tensor]):
        ...
        def check_device():
            if self.device:
                return self._do_logits(tokenizies)
            else:
        ...
        if self.has_attention_mask is None:
            self.has_attention_mask = 'attention_mask' in tokenizies
        return check_device()
    
    def _do_logits(self, tokenizies: Mapping[str, torch.Tensor]):
        """
        :param tokenizies: 分词后的输入数据
        """
