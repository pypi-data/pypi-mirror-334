...
class NER(nn.Module):
    ...
    def forward(self, tokenizies: dict):
        ...
        mask = tokenizies['attention_mask']
        for m in mask:
            length = m.sum() - 2
            m[length:] = 0
        # CRF解码
        return self._crf_decode(emissions, mask.bool())

    def fit(self, tokenizies: dict, labels: torch.Tensor):
        """
        训练模型
        :param tokenizies: 分词后的输入数据
        :param labels: 标签
        ...
        # CRF解码
        return loss, self._crf_decode(emissions, mask)

    def _logits(self, tokenizies: dict):
        """
        :param tokenizies: 分词后的输入数据
        """
        ...
        lengths = torch.stack([(m == 1).sum() for m in tokenizies['attention_mask']])
        # 将输入数据移动到指定设备
    ...
    def _crf_decode(self, emissions: torch.Tensor, mask: torch.BoolTensor = None):
        """
        CRF解码
        :param emissions: 发射分数
        :return: 解码后的结果
        """
        # CRF解码并转换为tensor
        return torch.tensor(self.crf.decode(emissions, mask), dtype=torch.long)
