...
import logging
from vsaiortc import RTCIceServer, RTCPeerConnection, RTCSessionDescription, RTCRtpTransceiver, MediaStreamTrack
...
class AiortcHandler(HandlerInterface):
    ...
    async def send(
        ...
        sendingRtpParameters: RtpParameters = self._sendingRtpParametersByKind[options.track.kind].copy(
            deep=True)
        sendingRtpParameters.codecs = reduceCodecs(
            sendingRtpParameters.codecs, options.codec)

        sendingRemoteRtpParameters: RtpParameters = self._sendingRemoteRtpParametersByKind[options.track.kind].copy(
            deep=True)
        ...
        transceiver = self.pc.addTransceiver(
            options.track, direction='sendonly')
        ...
        if options.encodings:
            layers = smParse(
                options.encodings[0].scalabilityMode if options.encodings[0].scalabilityMode else '')
        else:
            layers = smParse('')
    ...
    async def replaceTrack(self, localId, track=None):
        ...
        if track:
            logging.debug(
                f'replaceTrack() [localId:{localId}, track.id:{track.id}]')
        else:
    ...
    async def sendDataChannel(
        ...
        self._nextSendSctpStreamId = (
            self._nextSendSctpStreamId + 1) % SCTP_NUM_STREAMS.get('MIS', 1)
        # If this is the first DataChannel we need to create the SDP answer with
        # m=application section.
        if not self._hasDataChannelMediaSection:
            offer: RTCSessionDescription = await self.pc.createOffer()
            localSdpDict = sdp_transform.parse(offer.sdp)
            offerMediaDicts = [m for m in localSdpDict.get(
                'media') if m.get('type') == 'application']
    ...
    async def receive(
        ...
        try:
            await self.pc.setRemoteDescription(offer, rtpParameters.codecs, kind, track_buffer_size)
        except Exception as e:
            logging.error(f"error while setRemoteDescription {e}")
        answer: RTCSessionDescription = await self.pc.createAnswer()
        localSdpDict = sdp_transform.parse(answer.sdp)
        answerMediaDict = [m for m in localSdpDict.get(
            'media') if str(m.get('mid')) == localId][0]
        ...
        transceivers = [t for t in self.pc.getTransceivers()
                        if t.mid == localId]
        if not transceivers:
