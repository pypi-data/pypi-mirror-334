...
//	while(1) {
	while(i < maxIterNum) {
		d = -1 * log(1 - (P-theta)/(1-theta)); //the distance between nodes a and b
	...
	if (i >= maxIterNum)
	{
		...
		printf("Type II Divergence, the Computing Number Exceeds the maximun Number of Iteration !"); 
	}
	...
	//NOTE:The meaning of theata value is the proportion of sites expected to be \
	functional-divergence-related (F1 state). Of course theata value is between 0 and 1, \
	however, we use the statistical method to estimate the theata, the estimate is not the "true value".
	...
	// 为什么会出现小于0：样本数过少导致计算错误，故将小于0的情况改为0
		rets.push_back(R);
	}
