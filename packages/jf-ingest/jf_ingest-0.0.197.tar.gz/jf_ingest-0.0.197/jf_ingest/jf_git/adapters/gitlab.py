...
class GitlabAdapter(GitAdapter):
    ...
    def get_organizations(self) -> List[StandardizedOrganization]:
        ...
        if not self.config.discover_organizations:
            ...
            if not self.client.supports_get_all_organizations:
                logger.info(
                    'Discover organizations is enabled, but this instance does not support getting all organizations. '
                    'This is because the instance is using the public gitlab.com domain and will return other public '
                    'organizations. Skipping organization discovery...'
                )
                return orgs
