...
import time
import av
import atc2txt.lib.utils as utils
...
class TranscriptionTeeClient:
    ...
    def __call__(
        self, audio=None, rtsp_url=None, hls_url=None, mp3_url=None, save_file=None
    ):
        """
        ...
        """
        assert (
            sum(source is not None for source in [audio, rtsp_url, hls_url, mp3_url])
            <= 1
        ...
        elif mp3_url is not None:
            self.process_http_mp3_stream(mp3_url)
        else:
            self.record()
    ...
    def process_http_mp3_stream(self, mp3_url, save_file=None):
        """
        Connect to an HTTP MP3 source, process the audio stream, and send it for transcription.

        Args:
            mp3_url (str): The URL of the HTTP MP3 stream source.
            save_file (str, optional): Local path to save the network stream.
        """
        print("[INFO]: Connecting to HTTP MP3 stream...")
        try:
            container = av.open(mp3_url)
            self.process_av_stream(container, stream_type="MP3", save_file=save_file)
        except Exception as e:
            print(f"[ERROR]: Failed to process HTTP MP3 stream: {e}")
        finally:
            for client in self.clients:
                client.wait_before_disconnect()
            self.multicast_packet(Client.END_OF_AUDIO.encode("utf-8"), True)
            self.close_all_clients()
            self.write_all_clients_srt()
            print("[INFO]: HTTP MP3 stream processing finished.")
