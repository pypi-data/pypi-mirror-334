...
class Landing:
    ...
    def set_null_column_mismatch(self, null_column_mismatch):
        """
        ...
        - None
        """
        if not isinstance(null_column_mismatch, bool):
            raise ValueError("The 'null_column_mismatch' parameter must be a boolean value.")

        self._null_column_mismatch = null_column_mismatch

        return self
    
    def get_null_column_mismatch(self):
        return self._null_column_mismatch
    ...
    def _check_bronze_ingestion(self):
        ...
        if self._renamefiles:
          ...
          distinct_files_df = self._spark.sql(f'''
    ...
    def put_bronze(self):
        ...
        if not table_exists:
            ...
            try:
                ...
                if (missing_fields or extra_fields) and not self._null_column_mismatch:
                ...
                if self._null_column_mismatch:
                    for field in missing_fields:
                        if self._verbose:
                            print(f"Field '{field}' is missing in stage DataFrame. Adding NULL column to stage data.\n")
                        try:
                            stage = stage.withColumn(field, lit(None).cast(delta_fields[field]))
                        except Exception as e:
                            error_message += f"Failed to add missing column '{field}' to stage data: {e}\n"

                    for field in extra_fields:
                        try:
                            #Alter table to add the missing column
                            self._spark.sql(f"ALTER TABLE {self._bronze_table_path} ADD COLUMNS ({field} {delta_fields[field]})")
                        except Exception as e:
                            error_message += (f"Failed to add missing column '{field}' to target table: {e}")
                if error_message != '':
    ...
    def _clean_column_name(self, column_name):
        ...
        protected_columns = ['dbxcreated', 'dbxupdated', 'isdeleted', 'sourcefile', 'sourcetabname']
