...
class Payment:
    ...
    def is_email_required(self):
        return getattr(self.backend, 'email_is_required', False)
    ...
    def diagnostic(self, **kwargs):
        assert getattr(
            self.backend, 'diagnostic', None
        ), f'backend {self.backend} does not support diagnostic()'
        return self.backend.diagnostic(**kwargs)
