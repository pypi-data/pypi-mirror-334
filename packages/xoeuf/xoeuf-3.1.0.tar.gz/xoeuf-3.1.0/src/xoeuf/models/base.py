...
def iter_pages(
    ...
    *,
    ...
    This allows to process large models in chunks (pages).  If `order` is not
    given, use 'id'.

    Each iteration returns a recordset with at most `page_size` records.  If
    `show_progress` is true, use ``tqdm`` to show a progress bar, in this case,
    ``**progress_kwargs`` is passed to the function ``tqdm``.

    """
    order = order or "id"
    total = self.search(domain, count=True)
    if show_progress:
        if not progress_kwargs:
            progress_kwargs = {"leave": False}
        feedback = tqdm(total=total, **progress_kwargs)
    else:
        feedback = _silent()
    with feedback as progress:
        for offset in range(0, total + 1, page_size):
            page = self.search(domain, order=order, offset=offset, limit=page_size)
            progress.update(len(page))
            yield page


models.BaseModel.iter_pages = iter_pages
