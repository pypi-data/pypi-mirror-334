...
def printlf_dump_v1(_writeOnFile, _fullfilename='', _file=None, *_messaagePrintlf, **kwargs):
    ...
    if len(_messaagePrintlf) != 0:
        ...
        if _writeOnFile:
            if f is None:
                try:
                    print(f"Membuka file: {_fullfilename}")  # Debugging
                    f = open(_fullfilename, 'w')  # Buka file dalam mode write
                    print(f"File berhasil dibuka: {_fullfilename}")  # Debugging
                    f.write(cT7() + '\n')  # Tulis timestamp ke file
                    print(f"Timestamp berhasil ditulis ke file.")  # Debugging
                except Exception as e:
                    print(f"Error opening file: {e}")  # Debugging
                    return None
            try:
                f.write(cT7() + ' ' + text_message + '\n')  # Tulis pesan ke file
                print(f"Pesan berhasil ditulis ke file: {text_message}")  # Debugging
            except Exception as e:
                print(f"Error writing to file: {e}")  # Debugging
                return None
            finally:
                if f is not None:
                    f.close()  # Pastikan file ditutup setelah selesai
                    print(f"File berhasil ditutup: {_fullfilename}")  # Debugging
