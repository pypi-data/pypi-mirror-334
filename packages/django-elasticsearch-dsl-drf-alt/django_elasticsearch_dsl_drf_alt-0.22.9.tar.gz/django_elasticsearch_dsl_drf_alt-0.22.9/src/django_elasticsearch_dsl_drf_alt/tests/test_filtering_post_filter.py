...
__title__ = "django_elasticsearch_dsl_drf_alt.tests.test_filtering_post"
__author__ = "Artur Barseghyan <artur.barseghyan@gmail.com>"
__copyright__ = "2017-2020 Artur Barseghyan"
__license__ = "GPL 2.0/LGPL 2.1"
__all__ = ("TestFilteringPostFilter",)


@pytest.mark.django_db
class TestFilteringPostFilter(BaseRestFrameworkTestCase, AddressesMixin, BooksMixin):
    ...
    def _field_filter_term(self, field_name, filter_value):
        ...
        self.assertEqual(len(filtered_response.data["results"]), self.published_count)
    ...
    def test_field_filter_term_explicit(self):
        """Field filter term."""
        return self._field_filter_term(
            "state__term", constants.BOOK_PUBLISHING_STATUS_PUBLISHED
        )
    ...
    def test_field_filter_range(self):
        ...
        return self._field_filter_value("id__range", value, self.published_count)
    ...
    def test_field_filter_prefix(self):
        ...
        """
        return self._field_filter_value("title__prefix", self.prefix, self.prefix_count)
    ...
    def test_field_filter_in(self):
        ...
        return self._field_filter_value(
            "id__in",
            SEPARATOR_LOOKUP_COMPLEX_VALUE.join([str(__b.id) for __b in self.prefixed]),
            self.prefix_count,
        )
    ...
    def test_field_filter_terms_list(self):
        ...
        return self._field_filter_terms_list(
            "id", [str(__b.id) for __b in self.prefixed], self.prefix_count
        )
    ...
    def test_field_filter_exists_true(self):
        ...
        """
        return self._field_filter_value("tags__exists", "true", self.all_count)
    ...
    def test_field_filter_exists_false(self):
        ...
        """
        return self._field_filter_value(
            "non_existent_field__exists", "false", self.all_count
        )
    ...
    def test_field_filter_wildcard(self):
        ...
        """
        return self._field_filter_value(
            "title__wildcard", "*{}*".format(self.prefix[1:6]), self.prefix_count
        )
    ...
    def test_field_filter_isnull_true(self):
        ...
        """
        self._field_filter_value("null_field__isnull", "true", self.all_count)
        self._field_filter_value("tags__isnull", "true", self.no_tags_count)
    ...
    def test_field_filter_isnull_false(self):
        ...
        """
        self._field_filter_value("null_field__isnull", "false", 0)
        self._field_filter_value(
            "tags__isnull", "false", self.all_count - self.no_tags_count
        )
    ...
    def test_field_filter_gt_with_boost(self):
        ...
        return self._field_filter_gte_lte("id", self.in_progress[0].id, "gt", "2.0")
    ...
    def test_field_filter_lt_with_boost(self):
        ...
        """
        # TODO: check boost value
        return self._field_filter_gte_lte("id", self.in_progress[0].id, "lt", "2.0")
    ...
    def test_ids_filter(self):
        ...
        return self._field_filter_value(
            "ids", SEPARATOR_LOOKUP_COMPLEX_VALUE.join(__ids), self.published_count
        )
    ...
    def _list_results_with_facets(self):
        ...
        # Should contain `self.all_count` results
        self.assertEqual(len(facet_state_response.data["results"]), self.all_count)
        ...
        self.assertEqual(
            len(
                facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"]
            ),
            3,
        )
        ...
        self.assertIn(
            "publisher", facet_state_response.data["facets"]["_filter_publisher"]
        )

        self.assertIn("_filter_state", facet_state_response.data["facets"])
        self.assertIn("state", facet_state_response.data["facets"]["_filter_state"])
        self.assertIn(
            "buckets", facet_state_response.data["facets"]["_filter_state"]["state"]
        )
        # self.assertIn(
        #     'buckets',
        ...
        # )
        self.assertIn(
            {"doc_count": 10, "key": "published"},
            facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"],
        )
        self.assertIn(
            {"doc_count": 10, "key": "in_progress"},
            facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"],
        )
        self.assertIn(
            {"doc_count": 7, "key": "rejected"},
            facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"],
        )
        ...
        # Make request
        facet_state_filtered_response = self.client.get(facet_state_filtered_url, data)
        self.assertEqual(facet_state_filtered_response.status_code, status.HTTP_200_OK)

        # Should contain `self.published_count` results
        self.assertEqual(
            len(facet_state_filtered_response.data["results"]), self.published_count
        )
        ...
        self.assertEqual(
            len(
                facet_state_filtered_response.data["facets"]["_filter_state"]["state"][
                    "buckets"
                ]
            ),
            3,
        ...
        self.assertIn(
            {"doc_count": self.in_progress_count, "key": "in_progress"},
            facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"],
        )
        self.assertIn(
            {"doc_count": self.rejected_count, "key": "rejected"},
            facet_state_response.data["facets"]["_filter_state"]["state"]["buckets"],
        )
