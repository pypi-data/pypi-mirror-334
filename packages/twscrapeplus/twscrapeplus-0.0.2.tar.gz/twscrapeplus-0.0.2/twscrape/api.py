...
class API:
    ...
    async def _get_ext_account(self, queue="default_ext"):
        ...
        if not account:
            return None
        return ExtAccount(account, debug=self.debug)

    async def _with_ext_account(self, queue, callback):
        """使用 ExtAccount 执行操作并自动管理账号锁定和释放"""
        account = await self.pool.get_for_queue(queue)
        if not account:
            return None
        
        try:
            ext_acc = ExtAccount(account, debug=self.debug)
    ...
    async def send_dm(self, text: str, receivers: list[int], media: str = '', queue="send_dm"):
        ...
        async def _send(ext_acc):
            return await ext_acc.dm(text, receivers, media)
        
        return await self._with_ext_account(queue, _send)
