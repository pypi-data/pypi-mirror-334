...
class Operation:
    ...
    UpdateListBanner = ('', 'UpdateListBanner')
    DeleteListBanner = ('', 'DeleteListBanner')
    UpdatePinnedLists = ('', 'UpdatePinnedLists')
    ListAddMember = ('', 'ListAddMember')
    ListRemoveMember = ('', 'ListRemoveMember')
    DeleteList = ('', 'DeleteList')
    FollowTopic = ('', 'FollowTopic')
    UnfollowTopic = ('', 'UnfollowTopic')
    EnableFollowerNotifications = ('', 'EnableFollowerNotifications')
    DisableFollowerNotifications = ('', 'DisableFollowerNotifications')
...
class ExtAccount:
    ...
    async def dm(self, text: str, receivers: list[int], media: str = '') -> dict:
        ...
        if find_key(res, 'dm_validation_failure_type'):
            if self.debug:
                self.logger.debug(f"Failed to send DM(s) to {receivers}")
    ...
    async def follow_topic(self, topic_id: int) -> dict:
        ...
        return await self.gql('POST', Operation.FollowTopic, variables)
    
    async def unfollow_topic(self, topic_id: int) -> dict:
        """取消关注话题"""
        variables = {'topicId': str(topic_id)}
        return await self.gql('POST', Operation.UnfollowTopic, variables)
