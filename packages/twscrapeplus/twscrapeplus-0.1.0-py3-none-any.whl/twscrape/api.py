...
class API:
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False) -> int | None:
        ...
        # 使用QueueClient来保证请求使用正确的账号和代理
        async with QueueClient(self.pool, "UploadMedia", self.debug, proxy=self.proxy) as client:
            # INIT 阶段
            params = {
                'command': 'INIT', 
            ...
            if rep.status_code >= 400:
                raise Exception(f'{rep.text}')

            media_id = rep.json()['media_id']
            ...
            with open(file, 'rb') as fp:
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    try:
                        ...
                        headers = {b'content-type': b'multipart/form-data; boundary=----WebKitFormBoundary' + pad}
                        rep = await client.post(url=url, params=params, content=data, headers=headers)
                    except Exception:
                        try:
                            files = {'media': chunk}
            ...
            if rep.status_code == 400:
                if self.debug:
                    print(f'Upload failed: {rep.status_code} {rep.text}')
                return None
            ...
            while processing_info:
                ...
                if state == MEDIA_UPLOAD_FAIL:
                    if self.debug:
                        print(f'Processing failed: {rep.status_code} {rep.text}')
                    return None
    ...
    # DM发送方法
    async def dm(self, text: str, receivers: list[int], media: str = None) -> dict:
        """
        发送私信给指定用户
        ...
        # 使用_gql_item发送请求
        res = await self._gql_item(OP_SendMessageMutation, variables)
        if res:
            return res.json()
    ...
    async def _add_alt_text(self, media_id: int, text: str):
        ...
        async with QueueClient(self.pool, "AltText", self.debug, proxy=self.proxy) as client:
