...
class ExtAccount:
    ...
    async def init_client(self):
        ...
        client.verify = False
    # 增加超时设置，解决上传媒体时的超时问题
        client.timeout = httpx.Timeout(connect=10.0, read=90.0, write=90.0, pool=60.0)
    
        if self.debug and hasattr(client, 'proxies'):
            ...
            with open(file, 'rb') as fp:
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    try:
                        ...
                        if self.debug:
                            self.logger.debug(f"APPEND #{segment_index} 响应状态: {r.status_code}")
                        ...
                        self.logger.error(f"上传分块 #{segment_index} 过程中发生异常: {str(e)}")
                        import traceback
                        self.logger.error(traceback.format_exc())
                        return None
