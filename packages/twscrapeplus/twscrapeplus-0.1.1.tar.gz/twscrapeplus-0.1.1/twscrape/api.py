...
class API:
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False) -> int | None:
        ...
        # 使用原始的httpx.AsyncClient通过get_account获取账号
        account = await self.pool.get_for_queue_or_wait("UploadMedia")
        if not account:
            if self.debug:
                print("No available account for media upload")
            return None
    ...
    async def dm(self, text: str, receivers: list[int], media: str = None) -> dict:
        ...
        # 使用_gql_item发送请求
        res = await self._gql_item(OP_SendMessageMutation, variables, method="POST")
        if res:
            return res.json()
