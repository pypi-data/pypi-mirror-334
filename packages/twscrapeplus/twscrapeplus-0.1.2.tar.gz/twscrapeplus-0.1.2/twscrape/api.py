...
class API:
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False, max_retries=2) -> int | None:
        ...
        url = 'https://upload.twitter.com/1.1/media/upload.json'  # 使用1.1版本API
    ...
    async def dm(self, text: str, receivers: list[int], media: str = None, wait_for_account=False) -> dict:
        ...
        if not account and not wait_for_account:
            # 获取下一个可用时间
            next_available = await self.pool.next_available_time(queue)
            if next_available:
                return {
    ...
    async def reset_locks(self, queue: str = None):
        ...
        Args:
            queue: 可选，指定要重置的队列名称。如果为None，重置所有队列的锁。
            
        Returns:
            操作结果
        """
        return await self.pool.reset_locks(queue)
