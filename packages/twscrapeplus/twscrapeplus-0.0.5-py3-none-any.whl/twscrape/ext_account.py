...
class ExtAccount:
    ...
    async def init_client(self):
        """初始化并返回HTTP客户端"""
        # 传递API级别的代理设置，确保代理优先级与原生方法一致
        return self.account.make_client(proxy=self.proxy)
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False) -> int | None:
        ...
        try:
            ...
            if r.status_code >= 400:
                self.logger.error(f"初始化上传失败: {r.text}")
                return None
            ...
            with open(file, 'rb') as fp:
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    # 尝试方法1: 使用multipart/form-data
                    try:
                        files = {'media': chunk}
                        r = await client.post(url=url, headers=headers, params=params, files=files)
                    except Exception as e:
                        self.logger.error(f"上传分块失败: {e}")
                        return None

                    if r.status_code < 200 or r.status_code > 299:
                        self.logger.error(f"上传分块失败: {r.status_code} {r.text}")
                        return None
            ...
            if r.status_code == 400:
                self.logger.error(f"定稿失败: {r.status_code} {r.text}")
                return None
            ...
            while processing_info:
                ...
                if state == MEDIA_UPLOAD_FAIL:
                    self.logger.error(f"媒体处理失败: {r.text}")
                    return None
