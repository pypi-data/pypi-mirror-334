...
class API:
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False, max_retries=2) -> int | None:
        ...
        # 确保代理信息显示在调试输出中
        if self.debug and self.proxy:
            print(f"使用代理进行媒体上传: {self.proxy}")
        ...
        while retries <= max_retries:
            ...
            try:
                ...
                # 对所有请求使用一致的基础headers，不修改内部属性
                base_headers = client.headers.copy()
                ...
                # 不修改content-type，让httpx自己处理
                rep = await client.post(url=url, headers=base_headers, params=params)
                ...
                with open(file, 'rb') as fp:
                    ...
                    while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                        ...
                        # 直接使用files参数让httpx自动处理multipart/form-data
                        ...
                        try:
                            ...
                            rep = await client.post(
                                ...
                                params=params, 
                                files=files,
                                headers=base_headers  # 使用一致的headers，不要修改
                            )
                ...
                try:
                    ...
                    while processing_info:
                        ...
                        rep = await client.get(url=url, headers=base_headers, params=params)
