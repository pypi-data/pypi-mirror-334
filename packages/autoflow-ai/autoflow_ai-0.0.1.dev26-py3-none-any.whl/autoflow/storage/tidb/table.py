...
from autoflow.storage.tidb.types import VectorDataType, TableModel, DistanceMetric
...
class Table(Generic[T]):
    ...
    def bulk_insert(self, data: List[T]) -> List[T]:
        ...
        for field_name, config in self._vector_field_configs.items():
            ...
            vector_embeddings = config["embed_fn"].get_source_embeddings(sources_to_embedding)
    ...
    @overload
    def search(self, query: VectorDataType, query_type: SearchType = SearchType.VECTOR_SEARCH) -> VectorSearchQuery: ...
