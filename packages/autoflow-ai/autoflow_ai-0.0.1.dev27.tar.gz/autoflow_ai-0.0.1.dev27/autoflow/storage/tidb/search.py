...
from autoflow.storage.tidb.types import DistanceMetric, VectorDataType, TableModel
...
class VectorSearchQuery(SearchQuery):
    ...
    def distance_range(self, lower_bound: float = 0, upper_bound: float = 1) -> "VectorSearchQuery":
    ...
    def _execute(self, db_session: Session) -> Result:
        ...
        if self._distance_metric == DistanceMetric.L2:
            distance_column = vector_column.l2_distance(self._query).label(distance_label)
        else:
            distance_column = vector_column.cosine_distance(self._query).label(distance_label)
    ...
    def to_pandas(self) -> "DataFrame":
        try:
            ...
            raise ImportError("Failed to import pandas, please install it with `pip install pandas`")
