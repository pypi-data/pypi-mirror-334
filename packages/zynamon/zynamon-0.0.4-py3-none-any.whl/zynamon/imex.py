...
from zynamon.utils import * 
from zynamon.zeit import TS_ISO_FORMAT
...
_MAKE_BACKUP = True # switch [bool] to use a "_BACKUP" folder for original files in each path
_HEARTBEAT = 250000 # number of lines after which to display "heartbeat" (if any)
_DEBUG_LOW = 3  # verbosity for "low-level" functions (see 'zynamon.imex.import...' for details)
                # 0 = off | 1 = short summary | 2 = detailed summary | 3 = heartbeat | 4 = full)


def import_csv(csv_file, col_time=None, col_data=None, col_meta=None,
               headers=None, encoding=None, enforce=None, new_name=None,
               keep_meta=True, ensure_causal=True, save_file=True, save_fmt='json',
               max_ts=int(99), max_lines=int(1e9), verbose=0):
    ...
    *automagically* based on a keyword approach. In total, the operational precedence is as
    ...
    (1) 'time' will only consider the 1st matching column!
    (2) 'data' may also map limit values (e.g. 'max_val')
        ...
        while (True):
            try: # Note: Safe approach in case NULL is found in files!
                ...
                if (m == max_lines):
                    ...
                    break
                elif ((verbose >= 3) and (not m%_HEARTBEAT)):
                ...
                for n, item in enumerate(k_data):
                    if (enforce is None): # use plain data...
                        ...
                        if (enforce[0] is not None):
                            the_time = convert_time(line[k_time], target=enforce[0])
                        else:
...
def import_csv_mixed(csv_file, col_name, col_time, col_data,
                     headers=None, encoding=None, enforce=None, new_name=None,
                     keep_meta=True, ensure_causal=True, save_file=True, save_fmt='json',
                     max_ts=int(1e5), max_lines=int(1e9), verbose=0):
...
def write_ts_file(collection, the_file, overwrite=False, save_fmt='h5', target='dict',
                  verbose=False):
...
def pack_ts_file(the_file, agg_params=[(5*60, 'avg')], save_fmt='h5', overwrite=False, verbose=0):
    ...
    Args:
        ...
        agg_params (list, optional): List of 2-tuples as (agg_time, agg_mode) acc. to
            'TimeSeries.samples_pack()'. Note that applying several compression options at once
    ...
    if (verbose): print(f"Loading '{os.path.basename(the_file)}'")
    objects = read_ts_file(the_file, target='dict', verbose=(verbose>=1))
    ...
    for (agg_time, agg_mode) in agg_params:
        ...
        for n, ts_name in enumerate(objects):
            if (verbose): print(f"  - Aggregating '{ts_name}'")
            comp[ts_name] = objects[ts_name].samples_pack(agg_time, agg_mode, inplace='False')

        # save compressed collection
        if (verbose): print(f"Saving '{fname_out+'.'+save_fmt}'")
        write_ts_file(objects, file_out, overwrite, save_fmt, target='dict', verbose=(verbose>=1))
...
def batch_import_files(path, sub_folders, csv_type, col_name, col_time, col_value,
    ...
    Args:
        ...
        allow_reimport (bool, optional): Switch to allow re-importing files w/ 'TS_IMPORT_FORMATS'.
    ...
    for sub in the_folders:
        ...
        if (file_store):
            ...
            loaded = read_ts_file(file_store, target='list', verbose=True)
            for ts in loaded:
        ...
        if (not file_store or allow_reimport):
            ...
            for fname in os.listdir(sub):
                ...
                if (csv_type == 'mixed'):
                    objects = import_csv_mixed(os.path.join(path_sub, fname),
        ...
        if (collected[sub] != []):
            ...
            write_ts_file(collected[sub], fname_sub, True, save_fmt[1], target='dict')
    ...
    if (collected_all != []):
        ...
        write_ts_file(collected_all, file_all, True, save_fmt[1], target='dict')
...
def batch_merge_files(path, sub_folders, base, causal=True, save_fmt='pk', fname_out='',
    ...
    write_ts_file(collected_all, file_out, overwrite=True, save_fmt=save_fmt, target='dict')
...
def summarize(collection, print_out=True, print_file=None, intro_lines=[],
    ...
    Args:
        ...
        print_file (str, optional): File to write to, extension '.ts.info' will be enforced.
    ...
    for D in display:   
        ...
        eval(f'{D['eng']}("Joint interval: [ {interval[0]} <=> {interval[1]} ] {D['new']}")')
        eval(f'{D['skip']}')
        if (show_series):
            eval(f'{D['eng']}("============ {D['new']}")')
            ...
            for n, ts in enumerate(infos):
                eval(f'{D['eng']}("{n+1: 5}  {ts[0]:60}  {ts[1]: 9}  [ {ts[2][0]} <-> {ts[2][1]} ] {D['new']}")')
            eval(f'{D['eng']}("{'-'*128} {D['new']}")')
            eval(f'{D['skip']}')
        if (show_meta):
            eval(f'{D['eng']}("================= {D['new']}")')
