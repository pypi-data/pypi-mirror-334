...
####_ISO_TIMEZONE = False   #todo: add support for timezones? (see also explanation at bottom)
...
# Note: "All-two-digits" formats might be mixed up --> i.e. 'yy/mm/dd' ~ 'dd/mm/yy'!
# (DAT) -> times
...
class TimeSeries():
    ...
    def samples_add(self, new_samples, analyse=True, incognito=False):
        ...
        if (mode == 'ts'):
            ...
            # Note: Setting 'time'unknown' to NaN values yields trouble when serializing!
    ...
    def time_analyse(self):
        ...
        if (len(arr_t) == 1): # single sample (yet)?
            self.time.Ts = -1
            return
        elif (arr_t.iloc[0] == arr_t.iloc[-1]): # samples w/o time? (i.e. mode 'list-flat')
            self.time.Ts = -1
            return
    ...
    def values_purge(self, mode='patch_local', params={'N': 3}, outliers=None,
        ...
        if (len(outliers)):
            ...
            if (mode == 'remove'):
                ...
                for item in outliers:
                    df_new.loc[item[0],'x'] = local_val(self.df.x, item[0], params['N'], mode='avg')
                    # df_new.x[item[0]] = local_val(self.df.x, item[0], params['N'], mode='avg')

            elif (mode == 'patch_global'):
                global_patch = self.df.x.mean()
                for item in outliers:
                    df_new.loc[item[0],'x'] = global_patch
    ...
    def to_numpy(self, type_t=None, type_x=None):
        ...
        """
        # types = (self.time.type, self.get_type_x())
...
class TimeSpec():
    ...
    def __init__(self, obj=None):
        ...
        if (obj is not None):
            ...
            if ((type(obj) in (int, float, list, np.ndarray, pd.Series)) # scalar/array data    
                or ((type(obj) is str) and re.search(S_NUMBER, obj))): # string w/ actual numbers                
                self.from_data(obj)
            
            elif ((obj in _TIME_TYPE.keys())                               # obj matches type
                  or ((type(obj) is str) and (obj in _TIME_TYPE.values())) # obj matches string (of type)
                  or ((type(obj) is str) and (True))): # obj matches string (assuming detailed format ;) 
                self.from_setting(obj, check_known=True)
    ...
    def from_setting(self, t_param, check_known=True):
        """ Sets time-spec acc. to a desired in 't_param' either by data type or strings.

        Arg:
            t_param (type or str): Time specification to be set w/ possible ways of definition
        ...
        Note: If no specific format is given for string type/'iso', 'TS_ISO_FORMAT' is used
        as default!

        Examples:
            (type)     't_param':   int       |float  |str  |dt.datetime
            (string)   't_param':   'stamp_ns'|'stamp'|'iso'|'obj'
            (detailed) 't_param':   '%Y-%m-%d %H:%M:%S.%f' (acc. to ISO 8601 / RFC3339)
        """
        self.orig_type = type(t_param)

        # determine time-spec "type"
        if (self.orig_type is type):
            self.type = _TIME_TYPE[t_param]
        elif (t_param in set(_TIME_TYPE.values())):
            self.type = t_param
        elif (self.orig_type is str):
            self.type = 'iso'
        else:
            raise ValueError(f"Unknown time-spec input {t_param}")

        # determine time-spec "format"?
        if (self.type == 'iso'):
            if ((t_param is str) or (t_param == 'iso')):
                self.fmt = TS_ISO_FORMAT
            else:
                if (check_known):
                    # try to match w/ known DATE patterns...
                    if ((_RX_SET_DATE_HYPH_Y4F_MNUM.match(t_param) is not None) or
                        (_RX_SET_DATE_HYPH_Y2F_MNUM.match(t_param) is not None) or
                        (_RX_SET_DATE_HYPH_Y4F_MNAME.match(t_param) is not None) or
                        (_RX_SET_DATE_HYPH_Y2F_MNAME.match(t_param) is not None) or
                        (_RX_SET_DATE_DASH_Y4F_MNUM.match(t_param) is not None) or
                        (_RX_SET_DATE_DASH_Y2F_MNUM.match(t_param) is not None) or
                        (_RX_SET_DATE_DASH_Y4F_MNAME.match(t_param) is not None) or
                        (_RX_SET_DATE_DASH_Y2F_MNAME.match(t_param) is not None)): # -> date ok
                        # ...then try to match w/ known TIME pattern / precision
                        if ((_RX_SET_TIME_USEC_DOT.search(t_param) is not None) or
                            (_RX_SET_TIME_USEC_COMMA.search(t_param) is not None) or
                            (_RX_SET_TIME_SEC.search(t_param) is not None) or
                            (_RX_SET_TIME_MIN.search(t_param) is not None)): # -> time ok
                            self.fmt = t_param
                        else:
                            raise ValueError("Unknown time pattern in '{t_param}'")
                    else:
                        raise ValueError("Unknown date pattern in '{t_param}'")
                else:
                    self.fmt = t_param # i.e. apply w/o checking
        else:
    ...
    def from_data(self, t_in):
        ...
        if (self.type == 'iso'):
            ...
            if (_RX_DAT_DATE_HYPH_Y4F_MNUM.match(t_chk) is not None):
                ...
                fmt_date = '%y/%m/%d'
            elif (_RX_DAT_DATE_DASH_Y4F_MNAME.match(t_chk) is not None):
                fmt_date = '%Y/%b/%d'
            elif (_RX_DAT_DATE_DASH_Y2F_MNAME.match(t_chk) is not None):
                fmt_date = '%y/%b/%d'
...
def convert_time(t_in, target, factor=None, offset=None):
    ...
    if (src.type == 'stamp'):
        ...
        for n, tn in enumerate(t_in):
            #fixme: introduce 'timedelta' handling as well?
            #   -> i.e. as work-around if not 'date' (but only 'time' is given)
            # if (not src.has_date):
                # ...add 1970-01-01 to values / format? would this help?
            # #check this
            # myla = '00:12:00'
            # dt.datetime.strptime(myla, '%H:%M:%S')
            # myla = '1970-01-01 00:12:00'
            # dt.datetime.strptime(myla, '%H:%M:%S')
            # fixme
