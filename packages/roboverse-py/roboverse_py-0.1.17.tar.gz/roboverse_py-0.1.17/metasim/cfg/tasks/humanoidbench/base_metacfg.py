...
class HumanoidTaskMetaCfg(BaseRLTaskMetaCfg):
    ...
    def observation_space(envstates: list[EnvState]) -> torch.Tensor:
        """Observation space for humanoid tasks."""
...
class BaseLocomotionReward:
    ...
    def __call__(self, states: list[EnvState]) -> torch.FloatTensor:
        ...
        ret_infos = []
        for state in states:
            stable_reward = StableReward(self.robot_name)(state)
            if self._move_speed == 0:
                ...
                ret_rewards.append(stable_reward * dont_move)
                ret_infos.append({
