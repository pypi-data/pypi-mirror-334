...
Version 1.0.9 | Type 'pentit -h' for help\033[0m
"""
...
class pentitTool:
    ...
    def _create_parser(self):
        parser = argparse.ArgumentParser(
            description="pentit - Universal Cybersecurity CLI Tool",
            usage="pentit <command> [options]"
        )
    ...
    def _get_banner(self, ip, port):
        ...
        common_requests = {
            80: b"GET / HTTP/1.1\r\nHost: %s\r\n\r\n" % ip.encode(),
            25: b"HELO pentit.tool\r\n",
            22: b"SSH-2.0-pentit_1.0\r\n",
            21: b"HELP\r\n",
            23: b"\r\n",
            110: b"CAPA\r\n",
            143: b"A1 CAPABILITY\r\n"
    ...
    def geolocate_ip(self, ip, output_file=None):
        ...
        if output_file:
            result_content = f"# pentit Geolocation Results\n"
    ...
    def whois_lookup(self, domain, output_file=None):
        ...
        try:
            ...
            # Save to file if requested
            if output_file:
                result_content = f"# pentit WHOIS Results\n"
    ...
    def find_subdomains(self, domain, wordlist_path=None, timeout=1.0, output_file=None):
        ...
        try:
            ...
            if found_subdomains:
                ...
                # Save to file if requested
                if output_file:
                    result_content = f"# pentit Subdomain Scan Results\n"
            ...
            if latest_version > current_version:
                ...
                if force or input("\n\033[1;33m[?] Do you want to update now? (y/n): \033[0m").lower() == 'y':
                    ...
                    if success:
                        ...
                        print("\033[1;32m[+] Please restart pentit to use the new version.\033[0m")
                    else:
                        print(f"\n\033[1;31m[!] Update failed: {message}\033[0m")
                        print("\033[1;33m[!] Try updating manually with: pipx upgrade pentit\033[0m")
                else:
                    print("\n\033[1;33m[!] Update canceled by user\033[0m")
            else:
                print(f"\n\033[1;32m[+] pentit is already up to date (v{current_version})\033[0m")
        except Exception as e:
            self.stop_progress_animation()
            print(f"\n\033[1;31m[!] Update check failed: {e}\033[0m")
            print("\033[1;33m[!] You can manually update with: pipx upgrade pentit\033[0m")
    ...
    def get_current_version(self):
        ...
        try:
            ...
            return version("pentit")  # Get installed package version
        except Exception as e:
    ...
    def fetch_latest_version_info(self):
        ...
        try:
            ...
            with urllib.request.urlopen("https://pypi.org/pypi/pentit/json") as response:
