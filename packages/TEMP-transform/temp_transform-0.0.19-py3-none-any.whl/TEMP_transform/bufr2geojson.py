        ...
        while codes_bufr_keys_iterator_next(key_iterator):
            ...
            if xx < 9:
                ...
                if value is not None or description is not None:
                    ...
                    match headers['internationalDataSubCategory']:
                        case 4:
                            code_form = 'TEMP'
                        case 5:
                            code_form = 'TEMP_SHIP'
                        case 6:
                            code_form = 'TEMP_MOBIL'
                        case 7:
                            code_form = 'TEMP_DROP'
                    if identification["type"] == "callsign":
                        station_id = identification['callsign']
                        feature_id = f"{code_form}_CALLSIGN_{identification['callsign']}_{characteristic_date}T{characteristic_time}"  # noqa
                    else:
                        station_id = identification['wsi']
                        feature_id = f"{code_form}_WIGOS_{identification['wsi']}_{characteristic_date}T{characteristic_time}"  # noqa
