...
def search_helper(
    ...
    with console.status("Waiting for search results...", spinner="aesthetic"):
        ...
        results: SearchEntitiesResults = search_api.search_entities(
            search_entities_options=opts
        )

    resulting_print_obj_list = {}

    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_CATALOG_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_CATALOG_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_catalog_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_INVENTORY_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_INVENTORY_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_inventory_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_MACHINE_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_MACHINE_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_machine_design_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_CABLE_CATALOG_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_CABLE_CATALOG_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_cable_catalog_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_CABLE_INVENTORY_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_CABLE_INVENTORY_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_cable_inventory_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_CABLE_DESIGN_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_CABLE_DESIGN_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_cable_design_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_LOCATION_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_LOCATION_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_location_results, factory=factory
            )
        )
    if search_domain == DOMAIN_ALL_OPT or search_domain == DOMAIN_MAARC_OPT:
        resulting_print_obj_list["%s Results" % DOMAIN_MAARC_OPT] = (
            create_search_results_printout(
                result_list=results.item_domain_maarc_results, factory=factory
            )
        )

    cliBase.print_results(
        console, resulting_print_obj_list, format, pager, table_style=TABLE_STYLE
    )
    ...
    * Wildcard (*): Use * to capture any amount of characters
    ...
    while interactive:
        ...
        if search_res is None:
            search_res: SearchEntitiesResults = search_helper(
                factory=factory,
                console=console,
        ...
        if (
            ...
            or search_domain == DOMAIN_CABLE_INVENTORY_OPT
        ):
        ...
        for item in searched_items:
            choice = Choice(
                name=str(item.object_id) + "- " + item.object_name, value=item
            )
        ...
        while True:
            ...
            item_selection = inquirer.rawlist(
                message="Select Item:", choices=itr_choices
            ).execute()

            if item_selection is not None:
                cdbInfo_result = cdbInfo_helper(
                    cli=cli,
                    console=console,
                    item_id=item_selection.object_id,
                    all=all_opt,
                    pager=pager,
                    format=format,
                )
