...
jonway_picker_deps = HTMLDependency(
    "jonway_picker",
    "1.0.0",
    source={
        "package": "jonway_picker",
...
def input_jonway_picker(id: str):
    """
    A shiny input.
    """
    return Tag(
...
# Output component
class render_jonway_picker(Renderer[str]):
    """
    ...
    def auto_output_ui(self) -> Tag:
        return output_jonway_picker(self.output_id)

    # # There are no parameters being supplied to the `output_jonway_picker` rendering function.
...
def output_jonway_picker(id: str):
    """
    Show a color
    """
    return Tag(
        "jonway-picker-output",
        jonway_picker_deps,
        id=resolve_id(id),
    )
