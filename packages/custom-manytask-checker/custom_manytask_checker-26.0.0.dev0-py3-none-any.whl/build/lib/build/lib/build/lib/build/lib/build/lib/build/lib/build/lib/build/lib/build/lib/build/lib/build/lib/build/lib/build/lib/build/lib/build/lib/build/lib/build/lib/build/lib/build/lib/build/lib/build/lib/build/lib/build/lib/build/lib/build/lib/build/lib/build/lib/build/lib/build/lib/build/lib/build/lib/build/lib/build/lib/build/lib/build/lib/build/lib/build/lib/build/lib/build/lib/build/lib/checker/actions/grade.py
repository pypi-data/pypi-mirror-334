...
def grade_on_ci(
    ...
    if not task and not group:
        ...
        fs_tasks: dict[str, Task] = dict()
        if task:
            for fs_task in course_schedule.get_tasks(enabled=True):
                if fs_task.name in task:
                    fs_tasks[fs_task.name] = fs_task
        if group:
            for filesystem_group in course_schedule.get_groups(enabled=True):
                if filesystem_group.name in group:
                    for fs_task in filesystem_group.tasks:
                        fs_tasks[fs_task.name] = fs_task
        if fs_tasks:
            print_info(f"Checking tasks: {', '.join(fs_tasks.keys())}")

        tasks=list(fs_tasks.values()) if fs_tasks else None,
