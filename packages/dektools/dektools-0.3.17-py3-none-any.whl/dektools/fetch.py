...
def download_file(url, path=None, username=None, password=None, retries=None, proxy=None, block_size=6 * 1024):
    url = _get_url(url, username, password)
    if path is None:
                ...
                with http.request('GET', url.value, preload_content=False, decode_content=False) as rsp:
                    if rsp.status != HTTP_200_OK:
                        if times > 0:
                            ...
                            raise HTTPError(f'Download error status {rsp.status}')
                    cursor_size = 0
                    ...
                    with open(path_target_temp, 'wb') as f:
                        ...
                        for chunk in rsp.stream(block_size):
                            if full_size:
                                ...
                                sys.stdout.write('\rDownloading [%s]: %s' % (filename, format_file_size(cursor_size)))
