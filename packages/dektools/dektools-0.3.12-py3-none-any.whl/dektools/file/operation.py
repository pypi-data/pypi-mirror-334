...
def split_file(path, limit, chunked_size=64 * 2 ** 10):
    count = math.ceil(os.stat(path).st_size / limit)
    chunked_size = min(chunked_size, limit)
    with open(path, 'rb') as file:
        for i in range(count):
            with open(path + f'.{i}', 'wb') as f:
                total = 0
                for chunk in iter(lambda: file.read(chunked_size), b""):
                    ...
                    if total + chunked_size > limit:
                        rest = limit - total
                        if rest > 0:
                            f.write(file.read(rest))
                        break


def list_split_files(path):
    ...
    for name in os.listdir(path_dir):
        names = name.rsplit('.', 1)
        if len(names) == 2 and names[0] == path_name and re.fullmatch(r'\d+', names[1]):
            targets.add(int(names[1]))
    if not targets:
        return []
    loss = set(range(max(targets))) - targets
    if loss:
        raise FileNotFoundError(f'Combined files of `{path}` miss items: `{loss}`')
    return [path + f'.{i}' for i in range(len(targets))]


def remove_split_files(path):
    for target in list_split_files(path):
        remove_path(target)


def combine_split_files(path, chunked_size=64 * 2 ** 10):
    targets = list_split_files(path)
    if targets:
