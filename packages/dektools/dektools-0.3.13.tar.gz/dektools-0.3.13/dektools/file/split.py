import os
import math
from crc import Crc32, Register
from .path import normal_path
from .operation import remove_path, read_file
...
def split_file(path, limit, chunked_size=64 * 2 ** 10):
    ...
    hc = Register(Crc32.CRC32)
    hc.init()
    with open(path, 'rb') as file:
        for i in range(count):
            with open(path + f'.{i + 1}', 'wb') as f:
                total = 0
    ...
    with open(path + '.0', 'wb') as f:
        ...
        f.write(hc.digest().to_bytes(4, 'big', signed=False))
...
def meta_split_file(path):
    if callable(path):
        ...
        def getter(index):
            p = f"{normal_path(path)}.{index}"
            try:
                return p, os.stat(p).st_size
            except FileNotFoundError:
                return None, None
    path_meta, _ = getter(0)
    ...
    return parts, int.from_bytes(bs[8:], 'big', signed=False), total_size, path_meta
...
def combine_split_files(path, clear=False, chunked_size=64 * 2 ** 10):
    parts, checksum_correct, _, _ = meta_split_file(path)
    if parts:
        hc = Register(Crc32.CRC32)
        hc.init()
