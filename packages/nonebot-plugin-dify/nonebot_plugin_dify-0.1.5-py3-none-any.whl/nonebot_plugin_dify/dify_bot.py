...
class DifyBot():
    ...
    async def _handle_agent(self, query: str, session: DifySession):
        ...
        # response = requests.post(chat_url, headers=headers, json=payload)
        async with httpx.AsyncClient() as client:
            response = await client.post(chat_url, headers=headers, json=payload, timeout=config.dify_timeout_in_seconds)
        if response.status_code != 200:
            error_info = f"[DIFY] response text={response.text} status_code={response.status_code}"
            logger.warning(error_info)
            return [""], [error_info]
    ...
    async def _handle_workflow(self, query: str, session: DifySession):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(workflow_url, headers=headers, json=payload, timeout=config.dify_timeout_in_seconds)
        if response.status_code != 200:
            error_info = f"[DIFY] response text={response.text} status_code={response.status_code}"
            logger.warning(error_info)
            return [""], [error_info]
