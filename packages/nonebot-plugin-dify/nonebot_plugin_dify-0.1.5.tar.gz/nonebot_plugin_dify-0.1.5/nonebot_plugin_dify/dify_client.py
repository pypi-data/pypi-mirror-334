...
class DifyClient:
    ...
    async def _send_request(self, method, endpoint, json=None, params=None, stream=False):
        ...
        async with httpx.AsyncClient() as client:
            if stream:
                async with client.stream(method, url, json=json, params=params, headers=headers, timeout=config.dify_timeout_in_seconds) as response:
                    return response
            else:
                response = await client.request(method, url, json=json, params=params, headers=headers, timeout=config.dify_timeout_in_seconds)
                return response
    ...
    async def _send_request_with_files(self, method, endpoint, data, files):
        ...
        url = f"{self.base_url}{endpoint}"
        async with httpx.AsyncClient() as client:
            response = await client.request(method, url, data=data, headers=headers, files=files, timeout=config.dify_timeout_in_seconds)
