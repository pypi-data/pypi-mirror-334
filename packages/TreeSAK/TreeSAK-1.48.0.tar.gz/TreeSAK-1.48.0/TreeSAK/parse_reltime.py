...
def parse_reltime(args):
    ...
    op_txt_complete_info = '%s/%s_all_info.%s' % (f_path,f_base, f_ext)
    ...
    op_txt_handle = open(op_txt_complete_info, 'w')
    line_num_index = 0
    for each_line in open(reltime_txt):
        each_line_split = each_line.strip().split('\t')
        each_line_split = [i.strip() for i in each_line_split]
        if line_num_index == 0:
            op_txt_handle.write('Leaves\tDescription\t%s\n' % ('\t'.join(each_line_split)))
        else:
            if len(each_line_split) > 1:
                node_id = each_line_split[1]
    ...
    op_txt_2_handle = open(op_txt, 'w')
    op_txt_2_handle.write('Node\tDivTime\tCI_Lower\tCI_Upper\n')
    line_num_index = 0
    for each_line in open(op_txt_complete_info):
        if line_num_index > 0:
            each_line_split = each_line.strip().split('\t')
            ...
            op_txt_2_handle.write('%s\t%s\t%s\t%s\n' % (desc, div_time, ci_lower, ci_upper))
        line_num_index += 1
    op_txt_2_handle.close()
