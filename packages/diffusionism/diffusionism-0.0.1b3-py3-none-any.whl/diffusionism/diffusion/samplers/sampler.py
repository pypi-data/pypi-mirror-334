...
class Sampler(Diffusion, schedule=DiffusionSchedule, diffuser=Diffuser):
        ...
        **kwargs
        ...
        Args:
            ...
            *args: The arguments that drive both the diffusion process and the backbone model.
            ...
            mask (Optional[Tensor]): The area(s) of the input that users want to inpaint.
                If ``None``, there will not be inpaint mode.
            variables (Union[Sequence[Any], Dict[str, Any], None]): The variables returned from the
                :attr:`initialize_sampling` method.
            parameterization (Parameterization): The parameterization that informs which prediction
                mode is used.
            output_handlers (Sequence[SamplingOutputHandler]): The handler that handles the output at each
                timestep.
    ...
    def sample(
        ...
        **kwargs
        ...
        initialization = cls.initialize_sampling(
            timesteps,
            diffusion_schedule,
            input_end,
            initial_state,
        ...
        if len(initialization) == 3:
            x_t, timestep_iters, num_steps = initialization
            variables = None
        elif len(initialization) == 4:
            x_t, variables, timestep_iters, num_steps = initialization
        else:
            raise ValueError(f"'{cls.initialize_sampling.__qualname__}' should return 3 or 4 values")
        ...
        for i, timesteps in sampling_progress(num_steps, *timestep_iters):
            timesteps = tuple(torch.tensor([timestep], device=x_t.device, dtype=_timestep_dtype(timestep)) for timestep in timesteps)
