...
def get_current_notebook_path() -> str:
    ...
    Returns:
        Path to the current notebook file
        
    Raises:
        RuntimeError: If not running in a Jupyter notebook
    """
    try:
        ...
        notebook_path = ipython.config['IPKernelApp']['parent_appname']
        if not notebook_path:
            raise RuntimeError("Could not determine notebook path")
            
        return notebook_path
    except Exception as e:
        raise RuntimeError(f"Could not get current notebook path: {str(e)}")
...
def generate_toc(notebook_path: str = None) -> str:
    """
    Generate a table of contents from a Jupyter notebook.
    
    Args:
        notebook_path: Path to the Jupyter notebook file. If None, uses the current notebook.
    ...
    Raises:
        ...
        RuntimeError: If not running in a Jupyter notebook and no path is provided
    """
    # If no path provided, try to get current notebook
