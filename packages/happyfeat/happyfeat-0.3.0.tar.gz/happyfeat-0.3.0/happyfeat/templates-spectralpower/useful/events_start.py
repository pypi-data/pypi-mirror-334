...
class Events(Node):
    ...
    def __init__(self,start_session="Start", number_of_trials=4, first_class="OVTK_GDF_Left", second_class="OVTK_GDF_Right", baseline_duration=20, wait_for_beep_duration=1, wait_for_cue_duration=10, display_cue_duration=10, feedback_duration=5, end_of_trial_min_duration=2, end_of_trial_max_duration=5):
        super().__init__()
    ...
    def update(self):
        ...
        if match_events(self.i_events, self.start_session) is not None:
            timestamp = self.i_events.data.index[0]  # Get the last event
