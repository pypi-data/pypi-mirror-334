...
def find_prediction_current_week(trained_models, current_week_data, current_week_data_ml, lagged_amount, confidence_interval=0.05):
    """
    Generates predictions for the current week for a specific delay and prediction length.

    Parameters:
    - trained_models (dict): Dictionary containing trained models for a specific delay and prediction length.
    ...
    Returns:
    - DataFrame: Predictions for the given delay and prediction length.
    """
...
def wave_prediction_with_models(delay_definition_flag, delayed_start_matrix, predictions, prediction_length, delay, WAVE, waveID,
                    training_period, lag_reserve, lagged_amount, overlap, confidence_interval, tuned_model_dict, models):
    """
    ...
    """
    #print("wave_prediction_with_models: Prediction Length:", prediction_length, "Delay:", delay, "Wave:", waveID, "delay_definition_flag: ", delay_definition_flag)
    ...
    single_test, full_test, pred_weeks_stat = stats_data(test.head(prediction_length))
    # if (delay == 4 and prediction_length == 1 and waveID == 1):
    ...
    #### Generate ML Data
    shift = delay if delay_definition_flag == 'Delay' else 0
    train_x, test_x, train_y, test_y, fit_weeks_ml, pred_weeks_ml = gen_ml(
        wave, delay_definition_flag, prediction_length, lagged_amount, prediction_length, predictions, False, shift
    )
...
def model_eval_on_future(
    ...
    predictions, tuned_model_names, tuned_model_dict, delayed_start_matrix, training_period, lag_reserve,
    ...
    for wave in WAVES_future:
        ...
        for index, row in wave_df.iterrows():
            ...
            delay_list = [0]
            delay_definition_flag = 'Delay'

            for prediction_length in predictions:
                wave_delay_tuned_params = {}

                for delay in delay_list:  # Iterate over all delay values
                    for tuned_model_name in tuned_model_names:
                        wave_delay_model_key = f'wave {waveID_history} delay {delay}{tuned_model_name}'
                        wave_delay_tuned_params[tuned_model_name] = tuned_model_dict[wave_delay_model_key]

                    # Perform wave prediction

                    metric_row, trained_models = wave_prediction_with_models(
                        delay_definition_flag, delayed_start_matrix, predictions,
                        prediction_length, delay, WAVE_history, waveID_history,
                        training_period, lag_reserve, lagged_amount, overlap,
                        confidence_interval, wave_delay_tuned_params, models)

                    wave_delay_prediction_length_key = f'wave {waveID_history} delay {delay} prediction_length {prediction_length}'

                    # Store evaluation metrics and trained models
                    model_evaluation_dictionary[wave_delay_prediction_length_key] = metric_row
                    trained_models_dictionary[wave_delay_prediction_length_key] = trained_models
            ...
            concise_data, concise_data_dictionary = create_prediction_df(
                [WAVE_history], predictions, delay_list, models, model_evaluation_dictionary,
                inFoldername_pre, task_name, filename)
            #WAVES, predictions, delay_list, models, model_evaluation_dictionary, inFoldername_pre, task_name, filename
            # print('concise done')
            print('concise_data....\n', concise_data.dtypes)
            ...
            stan_optimal_weights = find_stan_optimal_weights(
                diff_data, predictions, delay_list, observed_column_name, models)
            ...
            for prediction_length in predictions:
                for delay in delay_list:
                    wave_delay_prediction_length_key = f'wave {waveID_history} delay {delay} prediction_length {prediction_length}'
                    print(wave_delay_prediction_length_key)

                    if wave_delay_prediction_length_key not in trained_models_dictionary:
                        print(f"Warning: No trained models found for {wave_delay_prediction_length_key}")
                        continue

                    trained_models = trained_models_dictionary[wave_delay_prediction_length_key]
                    ...
                    current_week_prediction_df.insert(3, 'Delay', delay)
                    current_week_prediction_df.insert(4, 'Length', prediction_length)

                    print(f"Current Week Prediction for Wave {waveID_history}, Delay {delay}, Length {prediction_length}:")
                    # print(current_week_prediction_df)

                    # Store results
                    all_predictions.append(current_week_prediction_df)
                    #weekly_prediction_future[(delay, prediction_length)] = current_week_prediction_df  # Save in dictionary
            ...
            # print(current_week_prediction_MLAMA_df)
            # Add `prediction_length` and `delay` columns
            #I am here only doing one week prediction no delay in future
            # current_week_prediction_MLAMA_df['Length'] = prediction_length_future
            # current_week_prediction_MLAMA_df['Delay'] = delay_future

            # Store the result in the dictionary
            weekly_prediction_future[week] = current_week_prediction_MLAMA_df#for this week all delay, length should be here
