...
def download_js_comparable(token, verbose=False, maxtime=15, chunk_size=64*1024):
    """
    Perform a download test using a persistent HTTP connection that cycles through 
    the URLs provided by fast.com. When one stream ends, it immediately switches to the next URL,
    continuing until the overall maxtime is reached.
    
    Returns:
        int: Download speed in Mbps (rounded to 0 decimals).
    """
    if not token:
        if verbose:
            print("No token provided. Aborting download test.")
        return 0
    ...
    urls = [item['url'] for item in parsedjson]
    if verbose:
        print("Using the following download URLs (cycled):")
        for u in urls:
            print(u)
    ...
    while time.perf_counter() - start < maxtime:
        url_to_use = urls[current_url_index % len(urls)]
        if verbose:
            print("Downloading from:", url_to_use)
        try:
            r = requests.get(url_to_use, stream=True, timeout=maxtime+5)
            ...
            if verbose:
                print("Download error on", url_to_use, ":", e)
        current_url_index += 1
    elapsed = time.perf_counter() - start
    speed_bps = (total_bytes * 8) / elapsed
    Mbps = speed_bps / 1e6
    if verbose:
        print(f"Downloaded {total_bytes} bytes in {round(elapsed)} s, speed: {round(Mbps)} Mbps")
    return round(Mbps)
...
def upload_test(token, verbose=False, min_time=5, max_time=15, 
...
def fast_com_ng(verbose=False, maxtime=15, ping_count=5, forceipv4=False, forceipv6=False, payload_size=10*1024*1024, max_payload_size=50*1024*1024):
    token = initiate(verbose=verbose)
    if not token:
        print("Failed to get token. All test values will be 0.")
        print("Download speed: 0 Mbps")
        print("Upload speed: 0 Mbps")
        print("Unloaded ping: 0 ms")
        print("Loaded ping: 0 ms")
        return
    avg_ping_unloaded, ping_unloaded_vals = fast_com_ping_unloaded(token=token, verbose=verbose, count=ping_count)
    dl_speed_loaded, avg_ping_loaded, ping_loaded_vals = fast_com_download(token=token, verbose=verbose, maxtime=maxtime, ping_count=ping_count, ping_interval=1)
    upload_speed_dynamic = upload_js_comparable_dynamic(token=token, verbose=verbose, maxtime=maxtime)
    print("Download speed:", round(dl_speed_loaded), "Mbps")
    print("Upload speed:", round(upload_speed_dynamic), "Mbps")
    print("Unloaded ping:", round(avg_ping_unloaded), "ms")
    print("Loaded ping:", round(avg_ping_loaded), "ms")
...
def main():
    ...
    print("Average unloaded ping:", round(avg_ping_unloaded), "ms")
    
    print("\nRunning download test:")
    dl_speed_loaded, avg_ping_loaded, ping_loaded_vals = fast_com_download(token=token, verbose=False, maxtime=10, ping_count=5, ping_interval=1)
    print("Download speed:", round(dl_speed_loaded), "Mbps")
    print("Average loaded ping:", round(avg_ping_loaded), "ms")
