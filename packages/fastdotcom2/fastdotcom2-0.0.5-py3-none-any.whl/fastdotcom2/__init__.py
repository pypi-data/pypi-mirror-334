...
def fast_com2(verbose=False, maxtime=15, ping_count=5, forceipv4=False, forceipv6=False,
    ...
    # Return a CSV-like string with two comma-separated values for download (speed and loaded ping)
    # and two comma-separated fields for upload (upload speed plus an empty field).
    result = (
        f"Download: {round(dl_speed_loaded)},{round(avg_ping_loaded)}\n"
        f"Upload: {round(upload_speed_dynamic)},,\n"
        f"Unloaded ping: {round(avg_ping_unloaded)} ms\n"
        f"Loaded ping: {round(avg_ping_loaded)} ms"
    )
    return result
...
def main():
    ...
    # Print two comma-separated values: download speed and average loaded ping.
    print("Download test results: {},{}".format(round(dl_speed_loaded), round(avg_ping_loaded)))
    
    print("\nRunning upload test:")
    upload_speed_dynamic = upload_js_comparable_dynamic(token=token, verbose=False, maxtime=10)
    # Print two comma-separated fields for upload; here the second field is empty.
    print("Upload test result: {},,".format(round(upload_speed_dynamic)))
