...
| `analyze_layout(model="yolo", confidence=0.2, existing="replace")` | Detect layout regions using ML models | `model`: Model to use ("yolo", "tatr")<br>`confidence`: Confidence threshold<br>`existing`: How to handle existing regions | `ElementCollection`: Detected regions |
