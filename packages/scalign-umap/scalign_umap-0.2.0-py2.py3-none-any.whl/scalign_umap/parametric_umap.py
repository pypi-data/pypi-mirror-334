import numpy as np
from scalign_umap import UMAP
from warnings import warn, catch_warnings, filterwarnings
from numba import TypingError
import os
from scalign_umap.spectral import spectral_layout
...
try:
    ...
    raise ImportError("umap.parametric_umap requires Keras >= 3.0") from None
...
class ParametricUMAP(UMAP):
    ...
    def fit(self, X, y=None, precomputed_distances=None, landmark_positions=None, **kwargs):
        """
        ...
        if self.metric == "precomputed":
            ...
            return super().fit(
                precomputed_distances, y, landmark_positions=landmark_positions, **kwargs
            )

        else:
            return super().fit(X, y, landmark_positions=landmark_positions, **kwargs)
    ...
    def fit_transform(
        ...
        **kwargs
        ...
        if self.metric == "precomputed":
            if precomputed_distances is None:
                raise ValueError(
                    "Precomputed distances must be supplied if metric is precomputed."
                )
            # prepare X for training the network
            self._X = X
            ...
            # landmark positions are cleaned up inside the
            # .fit() component of .fit_transform()
            return super().fit_transform(X, y, landmark_positions=landmark_positions, **kwargs)
    ...
    def save(self, save_location, verbose=True):
        ...
        if self.parametric_model is not None:
            parametric_model_output = os.path.join(save_location, "parametric.keras")
            parametric_weights_output = os.path.join(save_location, "parametric.weights.h5")
            self.parametric_model.save(parametric_model_output)
            self.parametric_model.save_weights(parametric_weights_output)
            if verbose:
...
def load_pumap(save_location, verbose = True, n_latent = 10):
    """
    ...
    else: model.decoder = None
    
    # save parametric_model
    parametric_model_output = os.path.join(save_location, "parametric.weights.h5")
    if os.path.exists(parametric_model_output):
        ...
        model.optimizer = keras.optimizers.Adam(**optimizer_config)
        model.define_model()
        # run the model for a test batch,
        # this will force the initialization of the just-created optimizer.
        model.parametric_model.build(input_shape = (1024, 10))
        model.transform(np.random.rand(1,10))
        model.parametric_model.load_weights(os.path.join(save_location, "parametric.weights.h5"))
