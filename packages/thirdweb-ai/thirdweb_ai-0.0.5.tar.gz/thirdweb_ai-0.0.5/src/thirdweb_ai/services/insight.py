...
class Insight(Service):
    ...
    def get_all_transactions(
        ...
        params: dict[str, Any] = {"sort_by": "block_timestamp", "sort_direction": "desc"}
        chain = chain or self.chain_ids
        if chain:
            params["chain"] = chain
        if from_address:
    ...
    @tool(
        description="Get transactions involving a specific contract address. Useful for analyzing contract usage, monitoring interactions, or debugging contract behavior."
    )
    def get_contract_transactions(
        self,
        contract_address: Annotated[
            str, "The contract address to get transactions for (e.g., '0x1234...'). Must be a valid contract address."
        ],
        chain: Annotated[
            list[int] | int | None,
            "Chain ID(s) to query (e.g., 1 for Ethereum, 137 for Polygon). Specify multiple IDs as a list for cross-chain queries.",
        ...
        from_address: Annotated[
            str | None,
            "Filter transactions sent from this address to the contract. Identifies who is interacting with the contract.",
        ] = None,
        function_selector: Annotated[
            str | None,
            "Filter by function selector (e.g., '0x095ea7b3' for approve). Isolates specific contract functions being called.",
        ] = None,
        decode: Annotated[
            bool | None,
            "Set to True to decode transaction input data. Makes function calls and parameters human-readable.",
        ] = None,
        block_number_gte: Annotated[
            int | None, "Only return transactions from blocks with number greater than or equal to this value."
        ] = None,
        limit: Annotated[
            int | None,
            "Maximum number of transactions to return per request. Default is 20, adjust based on your needs.",
        ...
        if page:
            params["page"] = page
        return self._get(f"transactions/{contract_address}", params)
