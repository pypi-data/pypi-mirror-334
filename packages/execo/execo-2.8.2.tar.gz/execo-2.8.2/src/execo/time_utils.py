...
_num_str_date_re = re.compile(r'^((\d*\.)?\d+)$')
_str_date_re = re.compile(r'^\s*((((\d\d)?(\d\d)-)?((\d?\d)-))?(\d?\d)[Tt ])?(\d?\d):(\d?\d):(\d?\d)(\.\d+)?([Zz]|([+-])(\d?\d):(\d?\d))?\s*$')
...
_num_str_duration_re = re.compile(r'^((\d*\.)?\d+)$')
_str_duration_re = re.compile(r'^(\d+):(\d?\d):(\d?\d)(\.\d+)?$')
