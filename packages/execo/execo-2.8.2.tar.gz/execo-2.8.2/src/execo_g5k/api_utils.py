...
__g5k_host_group_regex = re.compile(r'^([a-zA-Z]+\d*)-\d+(\.(\w+))?')
...
__canonical_host_name_regex = re.compile(r'^([a-zA-Z]+\d*-\d+)(-(?!kavlan-\d+)(?!ipv6)(\w+))?(-kavlan-\d+)?(-ipv6)?(\.([.\w]+))?$')
...
__host_longname_regex = re.compile(r'^([^.]*)(\.([^.]+))?')
