...
def oarsub(job_specs, frontend_connection_params = None, timeout = False, abort_on_error = False):
    ...
    for process in processes:
        job_id = None
        if process.ok:
            mo = re.search(r'^OAR_JOB_ID=(\d+)\s*$', process.stdout, re.MULTILINE)
            if mo != None:
...
def get_current_oar_jobs(frontends = None,
    ...
    for process in processes:
        if process.ok:
            jobs = re.findall(r'^(\d+)\s', process.stdout, re.MULTILINE)
...
def get_oar_job_info(oar_job_id = None, frontend = None,
    ...
    process.run()
    job_info = dict()
    start_date_result = re.search(r'^\s*start(T|_t)ime = (\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s*$', process.stdout, re.MULTILINE)
    if start_date_result:
        start_date = oar_date_to_unixts(start_date_result.group(2))
        job_info['start_date'] = start_date
    walltime_result = re.search(r'^\s*walltime = (\d+:\d?\d:\d?\d)\s*$', process.stdout, re.MULTILINE)
    if walltime_result:
        walltime = oar_duration_to_seconds(walltime_result.group(1))
        job_info['walltime'] = walltime
    scheduled_start_result = re.search(r'^\s*scheduled(S|_s)tart = (\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s*$', process.stdout, re.MULTILINE)
    if scheduled_start_result:
        scheduled_start = oar_date_to_unixts(scheduled_start_result.group(2))
        job_info['scheduled_start'] = scheduled_start
    state_result = re.search(r'^\s*state = (\w*)\s*$', process.stdout, re.MULTILINE)
    if state_result:
        job_info['state'] = state_result.group(1)
    name_result = re.search(r'^\s*name = ([ \t\S]*)\s*$', process.stdout, re.MULTILINE)
    if name_result:
...
def get_oar_job_nodes(oar_job_id = None, frontend = None,
    ...
    process = get_process("(oarstat -sj %(oar_job_id)i | grep 'Running\\|Terminated\\|Error') > /dev/null 2>&1 && oarstat -pj %(oar_job_id)i | oarprint network_address -f - | sort" % {'oar_job_id': oar_job_id},
                          host = get_frontend_host(frontend),
                          connection_params = make_connection_params(frontend_connection_params,
                                                                     default_frontend_connection_params))
...
def get_oar_job_subnets(oar_job_id = None, frontend = None, frontend_connection_params = None, timeout = False):
    ...
    if process_net.ok and process_ip.ok:
        subnet_addresses = re.findall(r'(\S+)\s+(\S+)', process_ip.stdout, re.MULTILINE)
