...
_ksoh_deployed_nodes_header_re1 = re.compile(r'^Nodes correctly deployed on cluster \w+\s*$')       # for kadeploy3 < 3.2
_ksoh_undeployed_nodes_header_re1 = re.compile(r'^Nodes not correctly deployed on cluster \w+\s*$') # for kadeploy3 < 3.2
_ksoh_deployed_nodes_header_re2 = re.compile(r'^The \w+ is successful on nodes\s*$')                # for kadeploy3 >= 3.2
_ksoh_undeployed_nodes_header_re2 = re.compile(r'^The \w+ failed on nodes\s*$')                     # for kadeploy3 >= 3.2
_ksoh_deployed_node_re = re.compile(r'^(\w+-\d+\.\w+\.grid5000\.fr)\s*$')
_ksoh_undeployed_node_re = re.compile(r'^(\w+-\d+\.\w+\.grid5000\.fr)(\s+\(.*)?\s*$')
...
_host_site_re1 = re.compile(r'^[^ \t\n\r\f\v\.]+\.([^ \t\n\r\f\v\.]+)\.grid5000.fr$')
_host_site_re2 = re.compile(r'^[^ \t\n\r\f\v\.]+\.([^ \t\n\r\f\v\.]+)$')
_host_site_re3 = re.compile(r'^[^ \t\n\r\f\v\.]+$')
...
class KaconsoleProcess(SshProcess):
    ...
    def __init__(self, host, frontend_connection_params=None, connection_timeout=20, prompt=r'root@[^#]*#', **kwargs):
