...
def plot_all_chan(f,nCh,chRegs,psd,bsnm,session_screening,saveFolder):
    """
    ...
    for i in range(0,nCh):
        ...
        plt.plot(f,psd[i],color=colors[iTetrode],alpha=transparency[modulo_ch])
    ...
    plt.title('Power Spectrum (Welch ''s method) - '+bsnm+' - '+session_screening)


    #plt.ylim((min(min_pwr)-1,max(max_pwr)+1))
    plt.legend(loc='center left',labels=chRegs,bbox_to_anchor=(1,0.5),fontsize=3)
    plt.savefig(saveFolder + 'PSD_All_Channels_0_600Hz_'+bsnm+'_'+session_screening+'.png', dpi=300)
...
def correlation_coefficient(data,chRegions,path,pathtbl,sub,sess,save=1):
    '''
    ...
    for iTetrode in range(int(data.shape[0]/4)):
        ...
        data_tetrode = data[iTetrode:iTetrode+4]
...
def variance_normalized(data,chRegions,path,pathtbl,sub,sess,save=1):
    '''
...
def deviation(data,chRegions,path,pathtbl,sub,sess, save=1):
    '''
