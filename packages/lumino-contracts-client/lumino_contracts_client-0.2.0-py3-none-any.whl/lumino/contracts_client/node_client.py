...
from lumino.contracts_client.constants import (
    ...
    DEFAULT_TEST_MODE,
    EPOCH_STATE, DEFAULT_STORAGE_DIR
)
...
def get_artifacts_password() -> str:
    ...
    if not artifacts_password:
        ...
        lumino_dir = os.path.expanduser('~/.lumino')
...
def initialize_lumino_node() -> LuminoNode:
    ...
    if not compute_rating:
        ...
        # Use os.path.expanduser to properly handle the home directory path
        storage_path = os.path.expanduser(f'{DEFAULT_STORAGE_DIR}/')
        hpi = HardwarePoolIntegration(storage_path)
