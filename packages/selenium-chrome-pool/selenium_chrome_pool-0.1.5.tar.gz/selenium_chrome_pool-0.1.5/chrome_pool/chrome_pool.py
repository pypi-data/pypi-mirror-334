...
class SeleniumChromePool:
    ...
    def __init__(self, max_connections=3,  max_idle_time=30,max_timeout_time=300, headless=True, chrome_options=None, remote_url=None, proxy=None):
        """
        初始化连接池。
        ...
        self.idle_times = {} # 新增：记录空闲连接的空闲时间
    ...
    def _initialize_pool(self):
        ...
        for _ in range(self.max_connections):
            ...
            if driver:
                ...
                self.idle_times[driver] = time.time() # 记录初始空闲时间
            else:
    ...
    def get_connection(self, timeout=60):
        ...
        try:
            ...
            with self._lock:
                ...
                if driver in self.idle_times:
                    del self.idle_times[driver]  # 从空闲时间记录中移除
    ...
    def release_connection(self, driver):
        ...
        with self._lock:
            if driver in self.busy_connections:
                ...
                self.available_connections.put(driver)
                self.idle_times[driver] = time.time() # 记录释放后的空闲时间
    ...
    def cleanup_idle_connections(self):
        ...
        while True:
            ...
            with self._lock:
                now = time.time()
                idle_drivers = []
                for driver, idle_time in self.idle_times.items():
                    if (now - idle_time) > self.max_idle_time:
                        idle_drivers.append(driver)

                for driver in idle_drivers:
                    try:
                        # 确保driver 还在 available_connections 队列中
                        if self._is_driver_available(driver):
                            self.available_connections.queue.remove(driver) # 直接从queue中移除
                            self.close_connection(driver)
                            del self.idle_times[driver]  # 从 idle_times 移除
                            # 创建一个新的连接来补充连接池
                            new_driver = self._create_driver()
                            if new_driver:
                                self.available_connections.put(new_driver)
                                self.idle_times[new_driver] = time.time()
                                logging.info("已创建新的连接补充连接池。")
                            else:
                                logging.warning("创建新的连接失败，无法补充连接池。")
                        else:
                            logging.warning(f"尝试清理空闲连接，但 {driver} 不在 available_connections 中")
                    except Exception as e:
            ...
            with self._lock:
                ...
                for driver, last_used in self.busy_connections.items():
                    if (now - last_used) > self.max_timeout_time:
                        timeout_drivers.append(driver)

                for driver in timeout_drivers:
                    logging.info(f"清理超时连接: 连接已使用超过 {self.max_timeout_time} 秒.")
                    try:
                        del self.busy_connections[driver]
                        self.close_connection(driver)
                        if driver in self.idle_times:
                            del self.idle_times[driver]
    ...
    def _is_driver_available(self, driver):
        """
        检查driver是否在 available_connections 队列中.
        """
        return driver in self.available_connections.queue