...
class SeleniumChromePool:
    ...
    def __init__(self, max_connections=5,  max_idle_time=300, headless=True, chrome_options=None, remote_url=None, proxy=None):
        """
        初始化连接池。

        Args:
            max_connections (int): 连接池最大连接数。
            max_idle_time (int):  空闲连接的最大保持时间（秒）。  超过这个时间会被销毁.
    ...
    def cleanup_idle_connections(self):
        ...
        while True:
            time.sleep(self.max_idle_time // 2)  # 定期检查，避免一次性清理太多
            with self._lock:
                now = time.time()
                idle_drivers = []
                ...
                for driver in idle_drivers:
                    logging.info(f"清理空闲连接: 连接已空闲超过 {self.max_idle_time} 秒.")
                    try:
                        del self.busy_connections[driver]
