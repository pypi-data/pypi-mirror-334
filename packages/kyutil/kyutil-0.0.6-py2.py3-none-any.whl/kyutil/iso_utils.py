...
from .config import ROOT_PATH_TMP
from .data import REPODATA, XML_GZ_SUFFIX, XML_SUFFIX, REPMOD, ERR_MSG_DICT
from .file import read_sha256sum_from_file
from .http_util import send_request, download_file
from .rpms import get_package_name
from .url import url_reachable, fetch_log_files
...
def download_iso_with_sha256sum_check(url, root_path, logger_=logger) -> str:
    ...
    if url_reachable(sha_url, logger=logger_):
        ...
        ok = download_file(url, str(os.path.join(root_path, random_str + os.path.basename(url))), logger_=logger_)  # 下载iso文件
        if ok:
