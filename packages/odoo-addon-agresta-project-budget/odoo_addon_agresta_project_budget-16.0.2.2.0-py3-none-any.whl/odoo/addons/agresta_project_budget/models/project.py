...
class Project(models.Model):
    ...
    total_budget = fields.Float(
        "Total Budget", related="last_update_id.total_budget", store=True, tracking=True
    )
    hourly_price = fields.Monetary("Hourly Price", tracking=True, group_operator=False)
    last_update_execution_pcnt = fields.Float(
        "Exec. %", related="last_update_id.progress", store=True
    )
    last_update_execution_amount = fields.Monetary(
        "Total Execution", related="last_update_id.total_execution", store=True
    )
    last_update_total_hours = fields.Float(
        "Hours", related="last_update_id.total_hours", store=True
    )
    last_update_material_expenses = fields.Monetary(
        "Material expenses", related="last_update_id.material_expenses", store=True
    )
    last_update_total_incomes = fields.Monetary(
        "Incomes", related="last_update_id.total_incomes", store=True
    )
    allocated_hours = fields.Float(
        string="Allocated Hours", compute="_compute_allocated_hours"
    )
    hourly_expenses = fields.Monetary(
        "Hour expenses", compute="_compute_hourly_expenses", store=True
    )
    total_expenses = fields.Monetary(
        "Total expenses", compute="_compute_total_expenses", store=True
    )
    ...
    def _compute_hourly_expenses(self):
        for record in self:
            record.hourly_expenses = (
                record.hourly_price * record.last_update_total_hours
            )
    ...
    def recalc_execution_active_projects(self):
        ...
        for project in projects:
            _logger.debug(
                "Begin recalculation for project -%s- on -%s-",
                project.name,
                last_month_date,
            )
