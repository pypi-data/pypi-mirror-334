...
pub trait OperatorSymbolTrait {
    ...
    fn pattern(input: &str) -> IResult<&str, &str> {
        tag_no_case(Self::SYMBOL).parse(input)
    }
}
...
impl OperatorSymbolTrait for UnaryNot {
    ...
    fn pattern(input: &str) -> IResult<&str, &str> {
        verify(alpha1, |tok: &str| tok.eq_ignore_ascii_case("not")).parse(input)
    }
}
#[derive(Debug, PartialEq, Clone)]
pub struct Or;
...
impl OperatorSymbolTrait for Concatenate {
    ...
    fn pattern(input: &str) -> IResult<&str, &str> {
        delimited(multispace0, tag("."), multispace1).parse(input)
    }
}
#[derive(Debug, PartialEq, Clone)]
pub struct Add;
