use crate::commands::base::SplCommand;
use crate::commands::cmd::add_totals::AddTotalsParser;
use crate::commands::cmd::bin::BinParser;
use crate::commands::cmd::collect::CollectParser;
use crate::commands::cmd::convert::ConvertParser;
use crate::commands::cmd::data_model::DataModelParser;
use crate::commands::cmd::dedup::DedupParser;
use crate::commands::cmd::eval::EvalParser;
use crate::commands::cmd::event_stats::EventStatsParser;
use crate::commands::cmd::fields::FieldsParser;
use crate::commands::cmd::fill_null::FillNullParser;
use crate::commands::cmd::format::FormatParser;
use crate::commands::cmd::head::HeadParser;
use crate::commands::cmd::input_lookup::InputLookupParser;
use crate::commands::cmd::join::JoinParser;
use crate::commands::cmd::lookup::LookupParser;
use crate::commands::cmd::make_results::MakeResultsParser;
use crate::commands::cmd::map::MapParser;
use crate::commands::cmd::multi_search::MultiSearchParser;
use crate::commands::cmd::mv_combine::MvCombineParser;
use crate::commands::cmd::mv_expand::MvExpandParser;
use crate::commands::cmd::rare::RareParser;
use crate::commands::cmd::regex::RegexParser;
use crate::commands::cmd::rename::RenameParser;
use crate::commands::cmd::return_::ReturnParser;
use crate::commands::cmd::rex::RexParser;
use crate::commands::cmd::s_path::SPathParser;
use crate::commands::cmd::search::SearchParser;
use crate::commands::cmd::sort::SortParser;
use crate::commands::cmd::stats::StatsParser;
use crate::commands::cmd::stream_stats::StreamStatsParser;
use crate::commands::cmd::t_stats::TStatsParser;
use crate::commands::cmd::table::TableParser;
use crate::commands::cmd::tail::TailParser;
use crate::commands::cmd::top::TopParser;
use crate::commands::cmd::where_::WhereParser;
use crate::operators::{OperatorSymbol, OperatorSymbolTrait};
...
// https://github.com/rust-bakery/nom/blob/main/doc/nom_recipes.md#wrapper-combinators-that-eat-whitespace-before-and-after-a-parser
pub fn ws_left<'a, O, E: ParseError<&'a str>, F>(
    inner: F,
...
// https://github.com/rust-bakery/nom/blob/main/doc/nom_recipes.md#wrapper-combinators-that-eat-whitespace-before-and-after-a-parser
pub fn ws_right<'a, O, E: ParseError<&'a str>, F>(
    inner: F,
) -> impl Parser<&'a str, Output = O, Error = E>
where
    F: Parser<&'a str, Output = O, Error = E>,
...
// https://github.com/rust-bakery/nom/blob/main/doc/nom_recipes.md#wrapper-combinators-that-eat-whitespace-before-and-after-a-parser
pub fn parenthesized<'a, O, E: ParseError<&'a str>, F>(
    inner: F,
) -> impl Parser<&'a str, Output = O, Error = E>
where
    F: Parser<&'a str, Output = O, Error = E>,
...
pub fn unwrapped<'a, O, InnerE, E: ParseError<&'a str>, F>(
    inner: F,
) -> impl Parser<&'a str, Output = O, Error = E>
where
    F: Parser<&'a str, Output = Result<O, InnerE>, Error = E>,
    InnerE: Debug,
{
    map(
...
#[allow(dead_code)]
pub fn unwrapped_option<'a, O, E: ParseError<&'a str>, F>(
    inner: F,
) -> impl Parser<&'a str, Output = O, Error = E>
where
    F: Parser<&'a str, Output = Option<O>, Error = E>,
{
    map(
...
macro_rules! make_specialized_separated_list {
    ($name:ident, $separator:expr) => { paste! {
        #[allow(dead_code)]
        pub fn [<$name 1>]<'a, O, E: ParseError<&'a str>, F>(inner: F) -> impl Parser<&'a str, Output=Vec<O>, Error=E>
        where
            F: Parser<&'a str, Output=O, Error=E>,
        {
        ...
        #[allow(dead_code)]
        pub fn [<$name 0>]<'a, O, E: ParseError<&'a str>, F>(inner: F) -> impl Parser<&'a str, Output=Vec<O>, Error=E>
        where
            F: Parser<&'a str, Output=O, Error=E>,
        {
...
pub fn time_unit(input: &str) -> IResult<&str, &str> {
    alt((months, days, hours, minutes, weeks, seconds)).parse(input)
}
...
pub fn double_quoted(input: &str) -> IResult<&str, &str> {
    alt((
        delimited(
            tag("\""),
            recognize((tag("\""), _double_quoted_content, tag("\""))),
            tag("\""),
        ),
...
pub fn str_value(input: &str) -> IResult<&str, ast::StrValue> {
    map(double_quoted, ast::StrValue::from).parse(input)
}

pub fn _token_not_t_f(input: &str) -> IResult<&str, &str> {
    context(
        "_token_not_t_f",
        verify(token, |v: &str| v != "t" && v != "f"),
    )
    .parse(input)
}
...
fn _field_fmt(input: &str) -> IResult<&str, ast::Field> {
    map(
        separated_list1(tag("."), alt((double_quoted, _token_not_t_f))),
        |vs| ast::Field(vs.join(".")),
    )
    .parse(input)
}

pub fn field(input: &str) -> IResult<&str, ast::Field> {
    alt((_field_fmt, into(single_quoted))).parse(input)
}
...
pub fn literal(input: &str) -> IResult<&str, ast::Constant> {
    let (remaining, content) =
        recognize(alt((double_quoted, single_quoted, token_with_extras))).parse(input)?;
    ...
    preceded(
        ...
        map_parser(double_quoted_alt, |s: &str| {
            let s = s
                ...
                .replace(r#"\""#, r#"""#)
                .replace(r#"\\"#, r#"\"#);
            let result = ws(pipeline).parse_complete(s.as_str());
            match result {
                Ok(("", res)) => Ok(("", res)),
...
pub fn term(input: &str) -> IResult<&str, &str> {
    recognize(take_while1(|c: char| c != ' ')).parse(input)
}
...
pub fn stats_call(input: &str) -> IResult<&str, Vec<ast::Expr>> {
    comma_or_space_separated_list1(alt((
        ...
        map(
            verify(token, |v: &str| !v.eq_ignore_ascii_case("by")),
            |v| {
                ast::Call {
                    name: v.into(),
                    args: vec![],
...
pub fn logical_expression_term(input: &str) -> IResult<&str, ast::Expr> {
    alt((
        ...
        map(
            preceded(ws(operators::UnaryNot::pattern), logical_expression_term),
...
mod tests {
    ...
    use crate::commands::cmd::eval::{EvalCommand, EvalParser};
    use crate::commands::cmd::fields::{FieldsCommand, FieldsParser};
    use crate::commands::cmd::search::SearchCommand;
    use crate::commands::cmd::sort::{SortCommand, SortParser};
            ...
            Ok((
                ...
                ast::Pipeline {
                    commands: vec![SearchCommand {
                        expr: _call!(TERM(ast::Wildcard::from("XXXXX*\\\\XXXXX*"))).into(),
                    }
                    .into()],
                }
            ))
            ...
            Ok((
                ...
                ast::Pipeline {
                    commands: vec![
                        SearchCommand {
                            expr: _and(
                                _eq(ast::Field::from("index"), ast::StrValue::from("foo")),
                                _eq(ast::Field::from("bar"), ast::StrValue::from("baz"))
                            )
                        }
                        .into(),
                        EvalCommand {
    ...
    #[rstest]
    fn test_field_eq_literal() {
        assert_eq!(
            logical_expression("sourcetype=crowdstrike:identities"),
            Ok((
                "",
                _eq(
                    ast::Field::from("sourcetype"),
                    ast::StrValue::from("crowdstrike:identities")
                )
            ))
        );
    }
    ...
    #[rstest]
    fn test_unary_not_consuming_entire_word() {
        let spl_not_1 = logical_expression("not e=1")
            .finish()
            .expect("Parse failed")
            .1;
        let spl_not_2 = logical_expression("not(e=1)")
            .finish()
            .expect("Parse failed")
            .1;
        let spl_note = logical_expression("note=1")
            .finish()
            .expect("Parse failed")
            .1;
        assert_eq!(spl_not_1, spl_not_2);
        assert_ne!(spl_not_1, spl_note);
    }

    #[rstest]
    fn test_aliased_field() {
        assert_eq!(_token_not_t_f.parse_complete("group"), Ok(("", "group")));
        assert_eq!(
            _field_fmt.parse_complete("group"),
            Ok(("", ast::Field::from("group")))
        );
        assert_eq!(
            field.parse_complete("group"),
            Ok(("", ast::Field::from("group")))
        );
        assert_eq!(
            alt((token, double_quoted)).parse_complete("user_group"),
            Ok(("", "user_group"))
        );
        assert_eq!(
            separated_pair(field, ws(tag_no_case("AS")), alt((token, double_quoted)))
                .parse_complete("group as user_group")
                .finish(),
            Ok(("", (ast::Field::from("group"), "user_group")))
        );
        assert_eq!(
            aliased_field.parse_complete("group as user_group"),
            Ok((
                "",
                ast::AliasedField {
    ...
    #[rstest]
    fn test_double_quoted_field() {
        assert_eq!(double_quoted.parse_complete("\"group\""), Ok(("", "group")));
        assert!(token.parse_complete("\"group\"").is_err());
        assert_eq!(
            field.parse_complete("\"group\""),
            Ok(("", ast::Field::from("group")))
        );
