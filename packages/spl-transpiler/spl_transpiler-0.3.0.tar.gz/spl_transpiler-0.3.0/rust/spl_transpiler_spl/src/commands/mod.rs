...
macro_rules! make_command {
    ...
    ($name:ident [$modname:ident] , $attrs:tt) => {
        paste!{
            make_command!(
                _,
                $modname,
    ...
    (_, $module_name:ident, $parser_name:ident, $command_name:ident, $spl_ast_name:ident, $name:expr, $alias:expr, { $($attr_name:ident : $attr_type:ty),* }) => {
        ...
        impl From<cmd::$module_name::$spl_ast_name> for crate::ast::Command {
            fn from(val: cmd::$module_name::$spl_ast_name) -> Self {
                crate::ast::Command::$spl_ast_name(val)
            }
        }
    };
}
