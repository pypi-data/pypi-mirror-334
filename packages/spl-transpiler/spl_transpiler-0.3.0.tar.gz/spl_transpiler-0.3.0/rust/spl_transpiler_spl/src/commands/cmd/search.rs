...
use crate::operators;
use crate::operators::OperatorSymbolTrait;
use crate::parser::{
    combine_all_expressions, comma_or_space_separated_list1, literal, logical_expression,
};
use crate::python::*;
use nom::branch::alt;
...
impl SplCommand<SearchCommand> for SearchParser {
    ...
    fn parse_body(input: &str) -> IResult<&str, SearchCommand> {
        map(
            comma_or_space_separated_list1(alt((logical_expression, map(literal, Expr::from)))),
            |exprs| SearchCommand {
    ...
    fn match_name(input: &str) -> IResult<&str, ()> {
        alt((
            map((tag_no_case("search"), alt((multispace1, eof))), |_| ()),
            map(multispace0, |_| ()),
        ))
        .parse(input)
    }
...
mod tests {
    ...
    use crate::parser::{expr, field_in, pipeline};
        ...
        assert_eq!(
            ...
            Ok((
                ...
                SearchCommand {
                    expr: _and(
                        _and(
                            _field_equals("a", ast::StrValue("b".to_string()).into()),
                            _field_equals("b", ast::StrValue("c".to_string()).into())
                        ),
                        _or(
                    ...
                    expr: _and(
                        _and(
                            _or(
                                ...
                                _eq(
                                    ast::Field::from("source"),
                                    ast::StrValue::from("Syslog:Linux-Sysmon/Operational")
                                ),
                            ),
                        ...
                        _and(
                            ...
                            _or(
                                _eq(
                                    ...
                                    ast::StrValue::from("3CXDesktopApp.exe")
                                ),
                                _eq(
    ...
    #[rstest]
    fn test_search_10() {
        // OR is higher precedence than AND, so...
        // x OR y OR z (AND) a OR (b)
        // comes out as (x OR y OR z) AND (a OR (b))
        let query = r#"
                ...
                SearchCommand {
                    expr: _and(
                            ...
                            _or(
                                _or(
                                    _eq(
                                        ...
                                        ast::StrValue::from("wineventlog_security")
                                    ),
                                    _eq(
                                        ast::Field::from("Channel"),
                                        ast::StrValue::from("security")
                                    ),
                                ),
                                _eq(
                                    ast::Field::from("source"),
                                    ast::StrValue::from("XmlWinEventLog:Security")
                                ),
                            ),
                            _or(
                                ...
                                _and(
                                    _and(
                                        _eq(
                                            ast::Field::from("EventCode"),
                                            ast::IntValue::from(5136)
                                        ),
                                        _neq(
                                    ...
                                    _or(
                                        _eq(
                                            ...
                                            ast::StrValue::from("displayName")
                                        ),
                                        _eq(
                                            ast::Field::from("AttributeLDAPDisplayName"),
                                            ast::StrValue::from("gPCFileSysPath")
                                        )
                                    )
                                )
                            ),
                        ...
                        _eq(
                            ...
                            ast::StrValue::from("groupPolicyContainer")
                        )
                    ),
                }
            ))
    ...
    fn test_search_11() {
        ...
        SearchParser::parse(query)
            .finish()
            .expect("Failed or incomplete parse");
    }
    ...
    #[rstest]
    fn test_search_15() {
        field_in("email IN(\"\", \"null\")")
            .finish()
            .expect("Failed or incomplete parse");
        expr("(email IN(\"\", \"null\"))")
            .finish()
            .expect("Failed or incomplete parse");
        expr("NOT (email IN(\"\", \"null\"))")
            .finish()
            .expect("Failed or incomplete parse");
        assert_eq!(
    ...
    #[rstest]
    fn test_search_boolean_1() {
        let eq_expression = all_consuming(logical_expression)
            .parse("x=true")
            .finish()
            .expect("Parse failed")
            .1;
        ...
        assert_eq!(
            eq_expression,
            _eq(ast::Field::from("x"), ast::BoolValue::from(true))
        );
    }

    #[rstest]
    fn test_search_boolean_2() {
        let eq_expression = all_consuming(pipeline)
            .parse("index=main | search x=true")
            .finish()
            .expect("Parse failed")
            .1;
        ...
        assert_eq!(
            ...
            Pipeline {
                commands: vec![
                    SearchCommand {
                        expr: _eq(ast::Field::from("index"), ast::StrValue::from("main"),),
                    }
                    .into(),
                    SearchCommand {
                        expr: _eq(ast::Field::from("x"), ast::BoolValue::from(true),),
                    }
                    .into()
                ],
            }
    ...
    #[rstest]
    fn test_search_with_quoted_field_1() {
        let expression = all_consuming(pipeline)
            .parse(r#"sourcetype=aws:cloudtrail "requestParameters.masterUserPassword"=*"#)
            .finish()
            .expect("Parse failed")
            .1;
            ...
            Pipeline {
                commands: vec![SearchCommand {
                    expr: _and(
                        _eq(
                            ast::Field::from("sourcetype"),
                            ast::StrValue::from("aws:cloudtrail"),
                        ),
                        _eq(
                            ast::Field::from("requestParameters.masterUserPassword"),
                            ast::Wildcard::from("*"),
                        ),
                    )
                }
