...
use crate::transpiler::utils::ctime_with_timezone;
use anyhow::Result;
use anyhow::{bail, ensure};
use log::warn;
use spl_transpiler_spl::commands::cmd::data_model::DataModelCommand;
use spl_transpiler_spl::commands::cmd::t_stats::*;
    ...
    fn transform_standalone(
        ...
        df = match (self.by_fields.clone(), self.by_prefix.clone()) {
            (Some(by_fields), None) => {
                ...
                for column in by_fields {
                    if column.span.is_some() {
                        let span = column.span.clone().unwrap();
                        let field = column.field.clone();
                        let span_text = format!("{} {}", span.value, span.scale);
                        let span_column = ctime_with_timezone(
                            column_like!([window([py_lit(field.0)], [py_lit(span_text)])].start),
                            "%Y-%m-%dT%H:%M:%S.%3Q%:z".parse().unwrap(),
                        )?;
                        df = df.with_column(column.field.clone().0, span_column);
                    }
                    column_list.push(column.field.0);
                }
                df.group_by(column_list)
            }
            (None, Some(_)) => {
...
mod tests {
    ...
    fn test_xsl_script_execution_with_wmic_1() {
        generates(
            ...
            r#"table_source(spark, datamodel="`Endpoint.Processes`").where(
                (
                    (
                        (
                            (
                            ...
                            & F.col("`Processes.process`").ilike("%os get%")
                        )
                        & F.col("`Processes.process`").ilike("%/format:%")
                    )
                    & F.col("`Processes.process`").ilike("%.xsl%")
                )
            ).groupBy([
    ...
    fn test_tstats_2() {
        generates(
            ...
            r#"table_source(spark, datamodel="`Web`").where(
                (
                    (
                        (F.col("`Web.url`") == F.lit("/AHT/AhtApiService.asmx/AuthUser"))
...
table_source(spark,
    ...
    "_time",
    F.date_format(F.window("_time", "1 hours").start, "yyyy-MM-dd'T'HH:mm:ss.SSSxxx")
).groupBy([
    "_time",
    "`Processes.user`",
    "`Processes.process_id`",
            ...
            r#"table_source(spark, datamodel="`Endpoint.Processes`").where(
                (
                    (
                        ...
                        (
                            F.col("`Processes.process`").ilike(r"%\\C$\\%")
                            | F.col("`Processes.process`").ilike(r"%\\Admin$\\%")
                        ) | F.col("`Processes.process`").ilike(r"%\\IPC$\\%")
                    )
                )
            ).groupBy([
...
df_1
    ...
    count FROM datamodel=Endpoint.Registry WHERE (Registry.registry_value_name= "StubPath"
...
table_source(spark, datamodel="`Endpoint.Registry`").where(
    (
        (F.col("`Registry.registry_value_name`") == F.lit("StubPath")) &
        F.col("`Registry.registry_path`").ilike( r"%\\SOFTWARE\\Microsoft\\Active Setup\\Installed Components%" )
    )
).withColumn(
    "_time",
    F.date_format(F.window("_time", "1 hours").start, "yyyy-MM-dd'T'HH:mm:ss.SSSxxx")
