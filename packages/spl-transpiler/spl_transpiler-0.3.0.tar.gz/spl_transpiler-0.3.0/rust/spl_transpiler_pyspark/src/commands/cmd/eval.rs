...
use spl_transpiler_spl::commands::cmd::eval::*;

fn _eval_expr(expr: impl TryInto<Expr, Error = anyhow::Error>) -> Result<ColumnLike> {
...
impl PipelineTransformer for EvalCommand {
        ...
        let kwargs: Result<Vec<(String, RuntimeExpr)>> = self
            ...
            .map(|(field, expr)| {
                Ok((
                    field.0.to_string(),
                    RuntimeExpr::from(
                        _eval_expr(expr.clone().with_context(&state.ctx))?.unaliased(),
                    ),
                ))
            })
            .collect();
    ...
    fn transform_standalone(
        ...
        for (ast::Field(name), value) in self.fields.iter().cloned() {
            df = df.with_column(name, _eval_expr(value.with_context(&state.ctx))?)
        }
        Ok(state.with_df(df))
    }
}
...
#[cfg(test)]
mod tests {
    use crate::utils::test::generates_runtime;
    use rstest::rstest;
