...
use spl_transpiler_spl::commands::cmd::search::*;
...
fn split_conditions(
    ...
    match expr.clone() {
        ast::Expr::Binary(ast::Binary {
            ...
            match (left_is_index, symbol.as_str()) {
                (true, "=") => {
    ...
    fn transform_standalone(
        ...
        let mut df = if !indices.is_empty() {
            ...
            for new_index in indices.into_iter() {
                let new_source = DataFrame::source_index(new_index);
...
mod tests {
    ...
    use nom::Parser;
    use rstest::rstest;
    use spl_transpiler_spl::commands::base::SplCommand;
    ...
    fn test_split_conditions_simple_index(
        ...
        check_split_results(
            ast::Binary::new(ast::Field::from("index"), "=", ast::StrValue::from("lol")),
            vec!["lol".into()],
            None,
            &ctx,
        );
    ...
    #[rstest]
    fn test_split_conditions_boolean_1(
        #[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext,
    ) {
        check_split_results(
            ast::Binary::new(ast::Field::from("x"), "=", ast::BoolValue::from(true)),
            Vec::<String>::new(),
            Some(column_like!([col("x")] == [lit(true)]).into()),
            &ctx,
        );
    }

    #[rstest]
    fn test_split_conditions_boolean_2(
        #[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext,
    ) {
        check_split_results(
            ast::BoolValue::from(true),
            Vec::<String>::new(),
            Some(column_like!(lit(true)).into()),
            &ctx,
        );
    }
    ...
    fn test_search_8() {
        ...
        generates(
            ...
            table_source(spark, index="main").where(
                (
                    ~F.col("query").ilike("SELECT % FROM Win32_ProcessStartTrace WHERE ProcessName = 'wsmprovhost.exe'") &
                    ~F.col("query").ilike("SELECT % FROM __InstanceOperationEvent WHERE TargetInstance ISA 'AntiVirusProduct' OR TargetInstance ISA 'FirewallProduct' OR TargetInstance ISA 'AntiSpywareProduct'")
                )
            )
            "#,
        )
    ...
    #[rstest]
    fn test_search_9() {
        // This actually isn't valid syntax, confirmed that Splunk croaks on this query
        let query = r#"index="lol" sourcetype="src1" len(x)=3"#;

        all_consuming(SearchParser::parse)
            .parse(query)
            .finish()
            .expect_err("Parse finished when it should have failed");
    }
...
df_1
    ...
    fn test_regression_search_bool() {
        generates(
            r#"index=main | search x=true"#,
            r#"table_source(spark, index="main").where((F.col("x") == F.lit(True)))"#,
        )
    }
}
