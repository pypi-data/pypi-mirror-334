...
use spl_transpiler_spl::commands::cmd::convert::{ConvertCommand, FieldConversion};
/*
...
pub fn convert_fn(
...
) -> anyhow::Result<(ColumnLike, String)> {
    ...
    match func.as_str() {
        "ctime" => Ok((
            ctime(column_like!(col(field_name)), timeformat.clone())?,
            field_name.clone(),
        )),
        "memk" => Ok((memk(column_like!(col(field_name))), field_name.clone())),
        "rmunit" => Ok((rmunit(column_like!(col(field_name))), field_name.clone())),
        "rmcomma" => Ok((rmcomma(column_like!(col(field_name))), field_name.clone())),
