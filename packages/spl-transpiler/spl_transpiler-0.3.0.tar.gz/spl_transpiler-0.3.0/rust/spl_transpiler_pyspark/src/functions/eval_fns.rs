...
use crate::ast::*;
use crate::base::{ContextualizedExpr, PysparkTranspileContext, RuntimeSelection, ToSparkExpr};
use crate::functions::shared::{memk, rmcomma, rmunit};
use crate::functions::*;
use crate::transpiler::utils::convert_time_format;
use anyhow::{bail, ensure, Result};
use log::warn;
use spl_transpiler_common::aliasable::Aliasable;
use spl_transpiler_spl::ast;
use spl_transpiler_spl::ast::TimeSpan;
...
mod tests {
    use super::*;
    use crate::base::PysparkTranspileContext;
    use crate::utils::test::assert_python_code_eq;
    use crate::ToSparkQuery;
    use rstest::rstest;

    #[rstest]
    fn test_simple_function_max(#[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext) {
        let result = eval_fn(
            ast::Call {
                name: "max".to_string(),
    ...
    #[rstest]
    fn test_mvindex_split(#[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext) {
        let (_, ast) = spl_transpiler_spl::parser::call(
            r#"mvindex(split(mvindex(BoundaryRanges, -1), ":"), 0)"#,
        )
        .unwrap();
    ...
    #[rstest]
    fn test_mvzip_1(#[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext) {
        let (_, ast) = spl_transpiler_spl::parser::call(r#"mvzip(x,y)"#).unwrap();
        let result = eval_fn(ast, &ctx);
    ...
    #[rstest]
    fn test_mvzip_2(#[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext) {
        let (_, ast) = spl_transpiler_spl::parser::call(r#"mvzip(x,y,"_")"#).unwrap();
        let result = eval_fn(ast, &ctx);
        assert_python_code_eq(
            result.unwrap().unaliased().to_spark_query(&ctx).unwrap(),
    ...
    #[rstest]
    fn test_relative_time_1(#[from(crate::base::test::ctx_bare)] ctx: PysparkTranspileContext) {
        let (_, ast) =
            spl_transpiler_spl::parser::call(r#"relative_time(now(), "-70m@m")"#).unwrap();
        let result = eval_fn(ast, &ctx);
        assert_python_code_eq(
            result.unwrap().unaliased().to_spark_query(&ctx).unwrap(),
            r#"F.date_trunc(
    ...
    #[rstest]
    fn test_cidrmatch(#[from(crate::base::test::ctx_runtime)] ctx: PysparkTranspileContext) {
        let (_, ast) =
            spl_transpiler_spl::parser::call(r#"cidrmatch("123.132.32.0/25",ip)"#).unwrap();
        let result = eval_fn(ast, &ctx);
        assert_python_code_eq(
            result.unwrap().unaliased().to_spark_query(&ctx).unwrap(),
