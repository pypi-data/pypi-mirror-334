...
def correlate_distances(adata: AnnData, is_concatenated=False, cluster_col: str = "cluster", batch: str = "batch"):
    """
    ...
    if is_concatenated:
        ...
        for i in adata.obs[batch].unique():
            subset = adata[adata.obs[batch] == i].copy()
            nearest_df = mesure_distances(adata=subset, cluster_col=cluster_col)
            nearest_df[batch] = i  # Adiciona a coluna de batch
