...
class ShellyPro2(ShellyDevice):
    def getStatus(self, channel: int = 0):
        """
        Retrieves the status of the Shelly Pro 2 for the specified channel (via HTTP or MQTT).
        ...
        if self.useMQTT:
            ...
            logging.info(f"Getting Shelly Pro 2 status, channel {channel} via HTTP")
            return self.sendHTTPRequest(f"rpc/Shelly.GetStatus?id={channel}")

    def on(self, channel: int = 0, timer: int = None):
        """
        Turns the relay of the Shelly Pro 2 for the specified channel on, optionally with a timer (via HTTP or MQTT).
        ...
        if self.useMQTT:
            ...
            logging.info(f"Turning Shelly Pro 2 relay, channel {channel}, on via HTTP with timer: {timer}")
            self.sendHTTPRequest(f"relay/{channel}", params)

    def off(self, channel: int = 0, timer: int = None):
        """
        Turns the relay of the Shelly Pro 2 for the specified channel off, optionally with a timer (via HTTP or MQTT).
        ...
        if self.useMQTT:
            ...
            logging.info(f"Turning Shelly Pro 2 relay, channel {channel}, off via HTTP with timer: {timer}")
            self.sendHTTPRequest(f"relay/{channel}", params)

    def toggle(self, channel: int = 0):
        """
        Toggles the relay of the Shelly Pro 2 for the specified channel (via HTTP or MQTT).
        ...
        if self.useMQTT:
            ...
            logging.info(f"Toggling Shelly Pro 2 relay, channel {channel}, via HTTP")
            self.sendHTTPRequest(f"relay/{channel}", params)
