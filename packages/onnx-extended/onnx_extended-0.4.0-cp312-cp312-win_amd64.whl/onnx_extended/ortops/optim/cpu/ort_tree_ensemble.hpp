...
TreeEnsembleKernel<IFEATURETYPE, TTYPE, OTYPE>::TreeEnsembleKernel(const OrtApi &api,
  ...
  if (base_values.size() == 0) {
    AttOrtValue ort_value =
        KernelInfoGetOptionalAttribute(api, info, "base_values_as_tensor", AttOrtValue());
    if (!ort_value.empty()) {
  ...
  if (is_classifier) {
    cls_type_type_type->set(parallel_tree, parallel_tree_N, parallel_N, batch_size_tree,
                            batch_size_rows, use_node3);
  } else {
    reg_type_type_type->set(parallel_tree, parallel_tree_N, parallel_N, batch_size_tree,
                            batch_size_rows, use_node3);
  }
}
