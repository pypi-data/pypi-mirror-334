...
class BaseExClient(BaseClient):
    ...
    async def set_pmcurexs(self):
        ...
        curexs = [models.Curex(**asdict(c), cur=curs[c.exid], ex=self.ex) for c in cur_pyds.values()]
        # Curex
        ...
        for k, pm in pms_epyds.items():
            ...
            if prev[2] == pm.name and pmu.country == prev[3]:  # оригинальное имя не уникально на этой бирже
                ...
                pmin = PmIn(**{**asdict(pmu), "country": country, "typ": pm.typ, "logo": pm.logo})
                try:
                    pms[k], _ = await models.Pm.update_or_create(**pmin.df_unq(("country",)))
