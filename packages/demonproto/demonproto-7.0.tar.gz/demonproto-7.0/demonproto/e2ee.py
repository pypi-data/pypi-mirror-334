...
class demon:
    def __init__(self):
        ...
        # Generate Identity Key Pair (IK) using RSA for key exchange
    ...
    def generate_signed_prekey(self):
        ...
        self.signed_prekey_private = rsa.generate_private_key(
            public_exponent=65537,
            key_size=3072,
            backend=self.backend
        )
    ...
    def generate_onetime_prekey(self):
        ...
        self.onetime_prekey_private = rsa.generate_private_key(
            public_exponent=65537,
            key_size=3072,
            backend=self.backend
        )
    ...
    def rotate_session_keys(self, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public):
        # Encrypt a random session key with recipient's RSA public keys
        session_key = os.urandom(32)  # 256-bit session key

        encrypted_key1 = recipient_identity_public_key.encrypt(
            session_key,
            padding.OAEP(
        ...
        encrypted_key2 = recipient_signed_prekey_public.encrypt(
            session_key,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                algorithm=hashes.SHA256(),
        ...
        encrypted_key3 = recipient_onetime_prekey_public.encrypt(
            session_key,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                algorithm=hashes.SHA256(),
        ...
        self.current_session_keys = {
            ...
            'encrypted_keys': [encrypted_key1, encrypted_key2, encrypted_key3]
        }
