...
class X3DHProtocol:
    def __init__(self):
        ...
        # Generate identity key pair (IK) using Curve25519
        self.identity_private_key = x25519.X25519PrivateKey.generate()
        self.identity_public_key = self.identity_private_key.public_key()

        # Generate signed prekey (SPK) using Ed25519
        self.signed_prekey_private_key = ed25519.Ed25519PrivateKey.generate()
        self.signed_prekey_public_key = self.signed_prekey_private_key.public_key()

        # Generate one-time prekey (OPK) using Curve25519
        self.onetime_prekey_private_key = x25519.X25519PrivateKey.generate()
        self.onetime_prekey_public_key = self.onetime_prekey_private_key.public_key()

        # Current session keys and replay protection tokens
    ...
    def generate_onetime_prekey(self):
        ...
        self.onetime_prekey_private_key = x25519.X25519PrivateKey.generate()
        self.onetime_prekey_public_key = self.onetime_prekey_private_key.public_key()
        return self.onetime_prekey_public_key

    def sign_message(self, message):
        """Signs the message with the Ed25519 private key."""
        signature = self.signed_prekey_private_key.sign(message.encode())
        return signature
    ...
    def rotate_session_keys(self, recipient_identity_public_key, recipient_signed_prekey_public_key, recipient_onetime_prekey_public_key):
        """Performs the X3DH key exchange and rotates session keys."""
        # Generate ephemeral private key (EK) using Curve25519
        ephemeral_private_key = x25519.X25519PrivateKey.generate()
        ephemeral_public_key = ephemeral_private_key.public_key()

        # Perform X3DH exchange using Curve25519
        shared_key = ephemeral_private_key.exchange(recipient_identity_public_key)
        shared_key_signed_prekey = ephemeral_private_key.exchange(recipient_signed_prekey_public_key)
        shared_key_onetime_prekey = ephemeral_private_key.exchange(recipient_onetime_prekey_public_key)

        # Combine the shared keys using HKDF
        combined_shared_key = shared_key + shared_key_signed_prekey + shared_key_onetime_prekey
        session_key = HKDF(
    ...
    def encrypt(self, content, recipient_identity_public_key, recipient_signed_prekey_public_key, recipient_onetime_prekey_public_key):
        """Encrypts a message for the recipient with forward secrecy."""
        ephemeral_public_key = self.rotate_session_keys(recipient_identity_public_key, recipient_signed_prekey_public_key, recipient_onetime_prekey_public_key)
    ...
    def decrypt(self, encrypted_content, private_key):
        ...
        # Derive the shared key using the private key and ephemeral public key
    ...
    def prepare_message(self, message, recipient_identity_public_key_bytes, recipient_signed_prekey_public_key_bytes, recipient_onetime_prekey_public_key_bytes):
        ...
        recipient_signed_prekey_public_key = serialization.load_pem_public_key(
            recipient_signed_prekey_public_key_bytes,
            backend=self.backend
        )
        recipient_onetime_prekey_public_key = serialization.load_pem_public_key(
            recipient_onetime_prekey_public_key_bytes,
            backend=self.backend
        )
        return self.encrypt(message, recipient_identity_public_key, recipient_signed_prekey_public_key, recipient_onetime_prekey_public_key)
