...
class demon:
    ...
    def rotate_session_keys(self, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public):
        try:
            # Perform Diffie-Hellman key exchange with the recipient's public keys
            print("Starting key exchange with recipient's public keys...")
            shared_key_identity = self.identity_private_key.exchange(
                recipient_identity_public_key
            )
            print("Shared key (identity):", shared_key_identity.hex())

            shared_key_signed_prekey = self.identity_private_key.exchange(
                recipient_signed_prekey_public
            )
            print("Shared key (signed prekey):", shared_key_signed_prekey.hex())

            shared_key_onetime_prekey = self.identity_private_key.exchange(
                recipient_onetime_prekey_public
            )
            print("Shared key (one-time prekey):", shared_key_onetime_prekey.hex())
            ...
            print("Combined shared key:", combined_shared_key.hex())
