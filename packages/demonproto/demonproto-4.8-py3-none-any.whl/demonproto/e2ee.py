...
class demon:
    ...
    def rotate_session_keys(self, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public):
        # Perform X3DH key exchange to derive the session key

        # Perform Diffie-Hellman for Identity Key
        shared_identity_key = self.identity_private_key.exchange(
            recipient_identity_public_key
        )

        # Perform Diffie-Hellman for Signed PreKey
        shared_signed_prekey = self.signed_prekey_private.exchange(
            recipient_signed_prekey_public
        )

        # Perform Diffie-Hellman for One-Time PreKey
        shared_onetime_prekey = self.onetime_prekey_private.exchange(
            recipient_onetime_prekey_public
        )

        # Combine all shared keys to derive a session key using HKDF
        hkdf = HKDF(
        ...
        # Combine the shared keys in a specific order to ensure that each Diffie-Hellman result is used.
        combined_shared_keys = shared_identity_key + shared_signed_prekey + shared_onetime_prekey
        session_key = hkdf.derive(combined_shared_keys)

        # Store the session key for encryption/decryption
    ...
    def encrypt(self, content, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public, is_file=False):
        ...
        ephemeral_public_key = self.rotate_session_keys(
            recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public
        )
