try:
    ...
    print("请先安装pdf2image库: pip install pdf2image")

import os
from concurrent.futures import ThreadPoolExecutor
from pathlib import Path

def pdf2img(pdf_path, output_dir, format='png', dpi=400):
    ...
    print(f"[PDF to PNG] 当前核心数{cpu_count}, 采用{int(cpu_count / 4)}个核心")
    print("[PDF to PNG] 正在处理数据中: ")
    images = convert_from_path(pdf_path, dpi=dpi, thread_count=int(cpu_count / 4))
