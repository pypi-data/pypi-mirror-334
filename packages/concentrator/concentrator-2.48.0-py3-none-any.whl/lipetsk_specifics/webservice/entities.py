...
class WhoHaveBenefit(ComplexModelWithNamespace):
    Type = UnsignedInteger(values=list(PrivilegeOwnerEnum.values.keys()), min_occurs=1, nillable=False)
    FirstName = FirstName
    LastName = LastName
    MiddleName = MiddleName
    DocType = DocType
...
class UpdateApplicationRequest(ComplexModelWithNamespace):
    ...
    DeclaredPerson = LipetskDeclaredPersonData.customize(nillable=False, min_occurs=1)
...
class GetApplicationResponse(ComplexModelWithNamespace):
    ...
    Applicant = LipetskApplicantData.customize(nillable=False, min_occurs=1)
    DeclaredPerson = LipetskDeclaredPersonData.customize(nillable=False, min_occurs=1)
    EduOrganizationsData = LipetskEduOrganizationsData.customize(nillable=False, min_occurs=1)
...
class DeclarationStatus(CustomComplexModel):
    ...
    class Annotations(CustomComplexModel.Annotations):
        doc = 'Информация о статусе заявления и позиции в очереди'
