...
from pleach.custom import Component, CustomComponent
from pleach.custom.code_parser.code_parser import CodeParser, CodeSyntaxError
from pleach.custom.custom_component.base_component import BaseComponent, ComponentCodeNullError
from pleach.custom.utils import build_custom_component_template
...
from pleach.field_typing import BaseLanguageModel
...
def test_code_parser_parse_classes():
    ...
    parser = CodeParser("from pleach.custom import Component\n\nclass Test(Component): pass")
    tree = parser.get_tree()
    for node in ast.walk(tree):
