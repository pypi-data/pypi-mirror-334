...
from pleach.initial_setup.setup import get_or_create_default_folder
from pleach.logging.logger import configure, logger
from pleach.main import setup_app
from pleach.services.database.utils import session_getter
from pleach.services.deps import get_db_service, get_settings_service, session_scope
from pleach.services.settings.constants import DEFAULT_SUPERUSER
from pleach.services.utils import initialize_services
from pleach.utils.version import fetch_latest_version, get_version_info
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_pleach, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Pleach server on Windows."""
    print_banner(host, port)
    run_pleach(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "pleach")
        'A new version of pleach is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=pleach_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if pleach_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    # Use pleach.utils.version to get the version info
    version_info = get_version_info()
    pleach_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= pleach_is_pre_release(pleach_version)  # Update pre-release status

    notice = build_version_notice(pleach_version, package_name)
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/pleach-ai/pleach]GitHub Repo[/link][/bold] :star2:"
    )
    telemetry_text = (
...
def run_pleach(host, port, log_level, options, app) -> None:
    """Run Pleach server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from pleach.server import PleachApplication

        server = PleachApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="PLEACH_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
    ...
    async def _create_superuser():
        ...
        async with session_getter(db_service) as session:
            from pleach.services.auth.utils import create_super_user

            if await create_super_user(db=session, username=username, password=password):
                # Verify that the superuser was created
                from pleach.services.database.models.user.model import User
...
# command to copy the pleach database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'pleach.db' and 'pleach-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("pleach"))
    db_path = cache_dir / "pleach.db"
    pre_db_path = cache_dir / "pleach-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from pleach.services.database.models.api_key import ApiKey, ApiKeyCreate
            from pleach.services.database.models.api_key.crud import create_api_key, delete_api_key
