...
from pleach.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def create_class(code, class_name):
    ...
    # Replace from pleach import CustomComponent with from pleach.custom import CustomComponent
    code = code.replace("from pleach import CustomComponent", "from pleach.custom import CustomComponent")
    code = code.replace(
        "from pleach.interface.custom.custom_component import CustomComponent",
...
def get_default_imports(code_string):
    ...
    pleach_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, pleach_imports)
    pleach_module = importlib.import_module("pleach.field_typing")
    default_imports.update({name: getattr(pleach_module, name) for name in necessary_imports})
