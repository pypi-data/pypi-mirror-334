...
from pleach.inputs.inputs import MessageTextInput
from pleach.memory import aget_messages, astore_message
from pleach.schema.message import Message
from pleach.template import Output
from pleach.utils.constants import MESSAGE_SENDER_AI, MESSAGE_SENDER_NAME_AI


class MessageStoreComponent(Component):
    display_name = "Message Store"
    description = "Stores a chat message or text into Pleach tables or an external memory."
    ...
    inputs = [
        ...
        HandleInput(
            ...
            input_types=["Memory"],
            info="The external memory to store the message. If empty, it will use the Pleach tables.",
        ),
        MessageTextInput(
            name="sender",
            display_name="Sender",
