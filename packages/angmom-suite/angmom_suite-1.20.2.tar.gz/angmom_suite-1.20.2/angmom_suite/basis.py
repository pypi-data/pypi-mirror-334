...
def wigner_eckard_expand(red, j1, j2, k, q):
    return jnp.array([[(-1)**(j1-m1) * float(wigner_3j(j1, k, j2, -m1, q, m2))
                       for m2 in jnp.arange(-j2, j2 + 1, 1)]
                      for m1 in jnp.arange(-j1, j1 + 1, 1)], dtype="float64") * red
...
def project_irrep_basis(symbols, **chain):
    ...
    if symbols:
        ...
        return jnp.empty((chain[jlab].shape[1], 0))
...
class Symbol:
    ...
    def get_op(self, op):
        ...
        if op == "spin":
            return np.sum([self.get_op(op) for op in self.elementary_ops('spin')], axis=0)

        if op == "angm":
            return np.sum([self.get_op(op) for op in self.elementary_ops('angm')], axis=0)
        ...
        def get_red(cplg):
            ...
            if (red1 := get_red(j1)) is not None:
                return (-1)**(j1_qn + j2_qn + j_qn + k) * (2 * j_qn + 1) * \
                    float(wigner_6j(j_qn, k, j_qn, j1_qn, j2_qn, j1_qn)) * red1

            if (red2 := get_red(j2)) is not None:
                return (-1)**(j1_qn + j2_qn + j_qn + k) * (2 * j_qn + 1) * \
                    float(wigner_6j(j_qn, k, j_qn, j2_qn, j1_qn, j2_qn)) * red2
