...
global common_dict
common_dict = dict()
allowed_providers = {"azure", "aws", "windows_365"}
...
def stats(org: str, edge_ids: str, run_id: str, ca_label: str, verbose: bool):
    """edge rotate certs"""
    target_org_id = org
    ...
    logger.info(f"Starting Rotating Certs with ca_label: {ca_label} on Org: {target_org_id}, Edges: {edge_ids}")
    logger.info("\n")
    ...
    if target_org_id.upper() != "ALL":
        try:
            ...
            if not org:
                ...
                return
            rotate_certs_on_all_edges(org, edge_ids, verbose)
...
def get_all_org_all_edge_stats(verbose: bool):
    for org in org_service.details.items():
        rotate_certs_on_all_edges(org, None, verbose)


def rotate_certs_on_all_edges(org: dict, edge_ids: str, verbose: bool):
    ...
    try:
        ...
        for edge in edges:
            ...
            if edge["providerLabel"].lower() not in allowed_providers:
                ...
                continue
            rotate_certs_on_edge(org, edge, verbose)
    except httpx.HTTPStatusError as ex:
        # logger.error(f"Http status code: {ex.response.status_code}")
        # logger.error(f"Url: {ex.request.url}")
        # logger.error(f"Response content: {ex.response.text}")
...
def rotate_certs_on_edge(org: dict, edge: dict, verbose: bool):
    rotate_edge_certs = dict()
    ...
    try:
        ...
        if is_cert_rotated(edge_twin_id, edgehub_url, verbose):
            rotate_edge_certs["status"] = "su"
            rotate_edge_certs["info"] = "cert already rotated"
        ...
        if not is_cert_rotated(edge_twin_id, edgehub_url, verbose):
...
def is_cert_rotated(edge_twin_id: str, edgehub_url: str, verbose: bool):
    ...
    if (
        ...
        return ca_label and ca_label == common_dict["CA_LABEL"]
    return False
