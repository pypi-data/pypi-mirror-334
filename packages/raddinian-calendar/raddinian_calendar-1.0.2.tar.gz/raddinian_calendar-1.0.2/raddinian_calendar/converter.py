...
def get_weekday(day: int):
    return RADDINIAN_WEEKDAYS[(day - 1) % DAYS_PER_WEEK]
...
def greg_radd(day: int, month: int, year: int) -> str:
    ...
    for i, days in enumerate(DAYS_PER_MONTH):
        ...
        delta_days -= days
        r_month += 1
    return f"{get_weekday(r_day)}, {r_day}. {RADDINIAN_MONTHS[r_month - 1]}, {abs(r_year)} {era}"

__all__ = ["radd_greg", "greg_radd", "is_leap_year", "get_leap_days", "get_weekday"]
