...
** [[elisp:(org-cycle)][| *DocStr | ] Basic example command.
    ...
    cs.examples.menuChapter('=Get Parameter=')

    siteBpoId = platfSiteBootstrap.platfSiteBootstrap_primaryBpo().pyCmnd().results
    bpoFps = platfSitesActivate_fps.PlatfSitesActivate_FPs(bpoId=str(siteBpoId))
    ...
    if fileParsDict:
        ...
        for thisArg in argChoices:
            cmnd('parGet', args=thisArg, comment=" # Read Value of FileParam")
...
#+begin_src sh :results output :session shared
  platfSitesActivate-fps.cs  -i parGet acct
#+end_src
#+RESULTS:
        #+end_org """)
