...
_CG_ERR_PREFIX = "_cl() returned error "                    #: used by _cg() to return _cl() err code in first out line
...
def _cl(err_code: int, command_line: str, extra_args: Sequence = (), lines_output: Optional[list[str]] = None,
    ...
    :param exit_on_err:         pass False to **not** exit the app on error (:paramref:`~_cl.exit_msg` has then to be
                                empty).
    :param exit_msg:            additional text to print on stdout/console if app debug level is greater or equal to 1
                                or if an error occurred and :paramref:`~_cl.exit_on_err` is ``True``.
...
def add_file(ini_pdv: PdvType, file_name: str, rel_path: str) -> bool:
    ...
    if any(os_path_basename(file_name).startswith(_) for _ in TEMPLATES_FILE_NAME_PREFIXES):
        ...
        if os_path_isfile(dst_file_name):
            ...
            return False
        elif os_path_isdir(dst_file_name):
...
def build_gui_app(ini_pdv: PdvType, **build_flags):
    """ build gui app with buildozer, add LIBS to make clean/full build and EMBED to include APK to share. """
    extra_args = []
    ...
    with _in_prj_dir_venv(pdv_str(ini_pdv, 'project_path')):
        ...
        if success and build_flags['EMBED']:
            ...
            # mask/camoflage APK extension for buildozer/P4A to embed APK
            copy_file(os_path_join("bin", new_apk), os_path_join(MOVES_SRC_FOLDER_NAME, file_name + ".{apk_ext}"))
            ...
            cae.po(f"    = embedded {slim_apk=} into APK in {MOVES_SRC_FOLDER_NAME}/ at {datetime.datetime.now()}")
...
def delete_file(ini_pdv: PdvType, file_or_dir: str) -> bool:
    ...
    :param ini_pdv:             project dev vars.
    :param file_or_dir:         file/folder name to delete (optional with path, relative to working tree root folder).
