...
Reference: https://github.com/ppannuto/python-titlecase/blob/main/titlecase/__init__.py
"""
...
def titlecase(text, callback=None, small_first_last=True):
    ...
    for line in lines:
        ...
        for word in words:
            ...
            if "-" in word:
                hyphenated = map(
                    lambda t: titlecase(t, callback, False), word.split("-")
                )
            ...
            CONSONANTS = "".join(
                set(string.ascii_lowercase) - {"a", "e", "i", "o", "u", "y"}
            )
            is_all_consonants = regex.search(
                "\A[" + CONSONANTS + "]+\Z", word, flags=regex.IGNORECASE
            )
...
def capitalize_full(sentence: str) -> str:
    """
    This function will capitalize all the letters of a sentence or a word
    """
    if not sentence:
        return ""
    lower_to_upper = {lc: uc for lc, uc in zip(ascii_lowercase, ascii_uppercase)}
    return reduce(
        lambda x, y: lower_to_upper.get(x, x) + lower_to_upper.get(y, y), sentence
    )


def decapitalize(sentence: str) -> str:
    """
    This function will decapitalize all the letters of a sentence or a word
    """
    if not sentence:
        return ""
    upper_to_lower = {uc: lc for lc, uc in zip(ascii_lowercase, ascii_uppercase)}
    return reduce(
        lambda x, y: upper_to_lower.get(x, x) + upper_to_lower.get(y, y), sentence
    )
...
def main():
    ...
    parser.add_argument(
        "-t",
        "--type",
        choices=["first", "title", "all", "decap"],
        help="different types of capitalization conversion",
    )
    parser.add_argument("string", nargs="*", default=[], help="String to titlecase")

    args = parser.parse_args()
    input_string = " ".join(args.string)

    if args.type == "first":
        cprint(capitalize(input_string))
    elif args.type == "title":
        cprint(titlecase(input_string))
        # potential bug
        # cprint(titlecase('ashdjas sdhfjk sdfhj kls'))
    elif args.type == "all":
        cprint(capitalize_full(input_string))
    elif args.type == "decap":
        cprint(decapitalize(input_string))
    else:
        cprint(input_string)
