...
def main_cli():
    ...
    if args.get('checkResult') is True:
        passed_result = passed_result.replace('py_result', 'xray_result')
        JunitXml(args['result']).dump_xray_format_xml(passed_result)
    res_dict = uploader.import_execution(passed_result, args['summary'], args['projectId'], args['format'],
                                         args['testPlan'])
