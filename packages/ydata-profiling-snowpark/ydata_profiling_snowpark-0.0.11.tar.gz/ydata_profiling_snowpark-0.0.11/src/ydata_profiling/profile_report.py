...
from ydata_profiling.config import Config, Settings, SparkSettings, SnowparkSettings
...
#For both snowpark and pyspark DF
def is_empty(df):
...
class ProfileReport(SerializeReport, ExpectationsReport):
    ...
    def __init__(
        self,
        df: Optional[Union[pd.DataFrame, sparkDataFrame, snowparkDataFrame]] = None,
        minimal: bool = False,
        tsmode: bool = False,
        ...
        **kwargs,
        ...
        Args:
            df: a pandas or spark.sql DataFrame or snowflake.snowpark Dataframe
    ...
    def __validate_inputs(
        df: Optional[Union[pd.DataFrame, sparkDataFrame, snowparkDataFrame]],
        minimal: bool,
        tsmode: bool,
        ...
        if isinstance(df, pd.DataFrame):
            ...
            if (
                df is not None and is_empty(df)
    ...
    def __initialize_dataframe(
        df: Optional[Union[pd.DataFrame, sparkDataFrame, snowparkDataFrame]], report_config: Settings
    ) -> Optional[Union[pd.DataFrame, sparkDataFrame, snowparkDataFrame]]:
