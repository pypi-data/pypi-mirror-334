...
def describe_counts_spark(
    ...
    value_counts = value_counts.sort("count", ascending=False).cache_result()
    ...
    columns = series.limit(1).toPandas().columns

    # FIXME: reduce to top-n and bottom-n
    value_counts_index_sorted = (
        value_counts_index_sorted.limit(200)
        .toPandas()
        .set_index(columns[0], drop=True)
        .squeeze(axis="columns")
    )

    summary["n_missing"] = n_missing
    summary["value_counts"] = value_counts.cache_result()
