...
def spark_preprocess(config: Settings, df: DataFrame) -> DataFrame:
    ...
    def _check_column_map_type(df: DataFrame, column_name: str) -> bool:
        """
        In Snowpark, there's no direct MapType as in Spark. The closest concept
        is an OBJECT type (represented by VariantType in Snowpark). This function
        checks whether the given column is an OBJECT type.
        """
        # Attempt to find the column in the schema
        field = next((f for f in df.schema.fields if f.name.lower() == column_name.lower()), None)
        if field is None:
            raise ValueError(f"Column '{column_name}' not found in DataFrame.")

        # Return True if the column's data type is VariantType (OBJECT in Snowflake)
        return isinstance(field.datatype, VariantType)
