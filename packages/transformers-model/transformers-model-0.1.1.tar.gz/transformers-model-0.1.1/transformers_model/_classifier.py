...
class TokenClassifier(nn.Module):
    ...
    def __init__(self, backbone, classifier):
        ...
        该方法初始化了模型的主干网络和分类器，并设置了设备属性为None。
        """
    ...
    def __init__(self, pretrained_path, num_classes: int, num_train_layers=0, config = None, **kwargs):
        ...
        **kwargs: 其他可变关键字参数，传递给TextCNN分类器。
        ...
        if num_train_layers <= 0:
            ...
            # 否则，只冻结指定数量的层
            for param in list(backbone.parameters())[:num_train_layers]:
                param.requires_grad_(False)
...
class AutoRNNAttentionTokenClassifier(TokenClassifier):
    ...
    def __init__(self, pretrained_path, num_classes, num_train_layers=0, config = None, **kwargs):
        ...
        **kwargs: 其他传递给分类器的参数。
        ...
        # 根据num_train_layers的值决定是否冻结模型的某些层
        if num_train_layers <= 0:
            # 如果num_train_layers小于等于0，冻结所有层
            for param in backbone.parameters():
                param.requires_grad_(False)
        else:
            # 否则，只冻结指定数量的层
...
class AutoCNNTextClassifier(TextClassifier):
    ...
    def __init__(self, pretrained_path, num_classes,  max_length: int = 512,   
                 padding: Union[bool, str, PaddingStrategy] = True, truncation=True, 
                 return_tensors='pt', return_token_type_ids=False, is_split_into_words=False, 
                 num_train_layers=0, config = None, **kwargs):
        """
        初始化模型参数。
        ...
        **kwargs: 其他传递给分类器的参数。
        ...
        # 根据num_train_layers的值决定是否冻结主干模型的参数
        if num_train_layers <= 0:
            # 冻结所有主干模型参数，不允许训练
            for param in backbone.parameters():
                param.requires_grad_(False)
