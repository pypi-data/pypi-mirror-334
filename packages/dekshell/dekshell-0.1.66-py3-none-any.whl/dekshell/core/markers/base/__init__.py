...
class MarkerBase:
    ...
    def get_tag_match(cls):
        ...
        if cls.tag_head_re is not None:
            if isinstance(cls.tag_head_re_args, str):
                args = [cls.tag_head_re_args]
            else:
                args = cls.tag_head_re_args
            args = tuple(re.escape(x) for x in args)
            r = cls.tag_head_re if cls.tag_head_re_args is None else cls.tag_head_re % args
        else:
            r = None
        return s, r
    ...
    def translate(self, context, s):
        ...
        def handler_core(expression, ignore, marker):
            ...
            elif key.startswith(self.trans_marker_command_output):
    ...
    @classmethod
    def eval_mixin(cls, context, expression):
