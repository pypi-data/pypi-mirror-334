...
from dektools.file import sure_dir, write_file, read_text, remove_path, sure_parent_dir, normal_path, \
    ...
    split_file, combine_split_files, remove_split_files
...
default_methods = {
    'echo': lambda *x, **y: print(*x, **y, flush=True),
    'echox': lambda *x, **y: print(*x, **y, file=sys.stderr, flush=True),
    'pp': pprint,
    'Path': Path,
    'path': {
        ...
        'rf': read_file,
        'sf': split_file,
        'rsf': remove_split_files,
        'cf': combine_split_files,
    },
    'cd': os.chdir,
