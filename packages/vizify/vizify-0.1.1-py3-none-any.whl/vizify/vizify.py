...
class Vizify:
    ...
    def stacked_bar_chart(self, pdf):
        """Generate stacked bar charts for categorical columns."""
        self.add_page_title(pdf, "Stacked Bar Charts")
        for col in self.cat_cols:
            value_counts = self.data[col].value_counts()
            if value_counts.empty:
                continue
            fig, ax = plt.subplots(figsize=(8, 5))
            value_counts.plot(kind="bar", stacked=True, ax=ax, color=sns.color_palette("pastel"))
            ax.set_title(f"Stacked Bar Chart of {col}")
            pdf.savefig(fig)
            plt.close(fig)

    def line_plots(self, pdf):
        """Generate line charts for numerical columns."""
        self.add_page_title(pdf, "Line Charts")
        for col in self.num_cols:
            fig, ax = plt.subplots(figsize=(8, 5))
            sns.lineplot(data=self.data[col], ax=ax, marker="o")
    ...
    def pie_chart(self, pdf):
        ...
        for cat_col in self.cat_cols:
            ...
            try:
                ...
                print(f"Could not generate pie chart for {cat_col}: {e}")
