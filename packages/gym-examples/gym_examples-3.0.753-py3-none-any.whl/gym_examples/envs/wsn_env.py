...
class WSNRoutingEnv(gym.Env):
    ...
    def compute_attention_rewards(self, rewards):
        ...
        '''
        print(f"Before torch tensor operation\nRewards type: {type(rewards)}, Rewards shape: {np.array(rewards).shape}")
        rewards = torch.tensor(rewards, dtype=torch.double)
        print(f"After torch tensor operation\nRewards type: {type(rewards)}, Rewards shape: {rewards.shape}")
        rewards = rewards.unsqueeze(0)  # Add batch dimension
        print(f"After unsqueeze, rewards type: {type(rewards)}, rewards shape: {rewards.shape}")
