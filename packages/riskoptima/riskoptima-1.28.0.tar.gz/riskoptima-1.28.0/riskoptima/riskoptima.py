...
class RiskOptima:
    ...
    def run_spy_vix_strategy(start_date, end_date, symbol_spy, symbol_vix, ma_window):
        """
        1. Fetches SPY and VIX data
        2. Computes rolling MA, Bollinger bands
        3. Detects signals for "second lower low + higher VIX + within ±2σ"
        4. Returns DataFrame of signals and combined DataFrame of SPY/VIX
        """
        df_spy = yf.download(symbol_spy, start=start_date, end=end_date)[['Close']].rename(columns={'Close': 'SPY_Close'})
        df_vix = yf.download(symbol_vix, start=start_date, end=end_date)[['Close']].rename(columns={'Close': 'VIX_Close'})
