...
class Env():
    ...
    def init_height_loader(self,scenario:dict, dir_inputs) -> None:

        # if scenario['test_settings']['mode'] == 'replay':
        #     self.location_temp = scenario['data']['scene_name'].split("_")[0]  # jiangtong/dapai
        # else:
        #     return None
        # print("init_data_loader")
    ...
    def make(self,scenario:dict, read_only=False, save_img_path='',kinetics_mode='simple') -> Tuple:
        ...
        Returns:
            ...
            traj:全局的背景车辆轨迹数据;
        """
        observation,observation_all,traj = self.controller.init(scenario,kinetics_mode)
    ...
    def step(self,action:Tuple[float,float,int],traj_future:Dict,observation_last:Observation,traj=200, path_planned=None, vis_cir_open=False) -> Observation:
        ...
        observation = self.controller.step(self,action,self.client,traj_future,observation_last,traj, path_planned, last_slope, last_height, vis_cir_open=vis_cir_open)  # 使用车辆运动学模型单步更新场景;
        height = self.get_height(observation.format())
