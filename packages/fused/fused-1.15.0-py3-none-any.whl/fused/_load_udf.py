...
def load(url_or_udf: Union[str, Path], /, *, cache_key: Any = None) -> AnyBaseUdf:
    ...
    Args:
        url_or_udf: A string representing the location of the UDF. This can be
            a GitHub URL starting with "https://github.com", a Fused platform-specific
            identifier in the format "email/udf_name", or a local file path pointing to
            a Python file.
    ...
    if is_udf_token(url_or_udf):
        if url_or_udf.startswith("UDF_"):
            url_or_udf = f"public@fused.io/{url_or_udf.removeprefix('UDF_')}"
        else:
            try:
