...
# TODO: 极端值处理方法统一为一个函数或类或剥离出一个公共函数
def cut_mad(series: Union[np.ndarray, pd.Series],
            n: int = 3,
            up_cut_val: Union[None, bool, int, float] = None,
            low_cut_val: Union[None, bool, int, float] = None,
            func_filter: Callable = None):
    '''
    | MAD极端值处理方法
    | up_cut_val为超过上限的填充值，
    |    若为None，则按普通MAD方法填充；若为False，则不进行处理；若指定值，则填充为指定值
    | low_cut_val为超过下限的填充值，意义同up_cut_val
    ...
    upper = median + n * med
    lower = median - n * med
    if up_cut_val == False and isinstance(up_cut_val, bool):
        pass
    else:
        if isnull(up_cut_val):
            up_cut_val = upper
        values[values > upper] = up_cut_val
    if low_cut_val == False and isinstance(low_cut_val, bool):
        pass
    else:  
...
def cut_nstd(series: Union[np.ndarray, pd.Series],
             n: int = 3,
             up_cut_val: Union[None, bool, int, float] = None,
             low_cut_val: Union[None, bool, int, float] = None
             ):
...
# def winsorize(ser, t=None):
#     if t is None:
#         t = 0.05
#     counts = ser.shape[0]
#     ser_rank = ser.rank()
#     up = np.floor(counts * (1 - t / 2))
#     bottom = np.ceil(counts * t / 2)
#     if bottom >= up:
#         return ser
#     if up > ser.shape[0]:
#         up = ser.shape[0]
#     if bottom < 1:
#         bottom = 1
#     ser.loc[ser_rank > up] = ser.loc[(ser_rank == up)]
#     ser.loc[ser_rank < bottom] = ser.loc[(ser_rank == bottom)]
#     return ser
