...
def time_add(t, seconds=0, minutes=0, hours=0, days=0, months=0):
...
def get_month_start_end(start_date=None, end_date=None):
    ...
    dates = pd.date_range(start_date_, end_date_)
    yms = set([(x.year, x.month) for x in dates])
    starts = [str(y)+str(m).zfill(2)+'01' for y, m in yms]
    starts = [copy_format(x, start_date) for x in starts]
    starts.sort()
    ends = [str(y)+str(m).zfill(2)+str(calendar.monthrange(y, m)[-1]).zfill(2) for y, m in yms]
    ends = [copy_format(x, end_date) for x in ends]
    ends.sort()
