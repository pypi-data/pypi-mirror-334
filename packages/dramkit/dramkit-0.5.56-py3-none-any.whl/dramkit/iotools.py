...
def get_all_paths0(target_path='./'):
    paths = []
    for dirpath, dirname, filenames in os.walk(target_path):
        for filename in filenames:
            if _WINDOWS_SYSTEM:
                paths.append(os.path.join(target_path, dirpath, filename).replace(os.sep, '/'))
            else:
...
def get_all_paths(dir_path, ext=None, start=None,
    ...
    if _WINDOWS_SYSTEM:
        fpaths = [x.replace(os.sep, '/') for x in fpaths]
...
def _get_copy_abs_tgt_dir(src_dir, tgt_dir, keep_root_same):
    ...
    return abs_tgt_dir.replace(os.sep, '/') if _WINDOWS_SYSTEM else abs_tgt_dir
...
def copy_dir_structure(src_dir, tgt_dir,
    ...
    abs_dir_path = os.path.abspath(src_dir)
    if _WINDOWS_SYSTEM:
        abs_dir_path = abs_dir_path.replace(os.sep, '/')
...
def handle_win_fname(fname: str) -> str:
    ...
    for k, v in const.WIN_NOT_ALLOW_FILE_STR.items():
...
def del_root_dir_in_path(path, level=1):
    """删除文件路径的前缀文件夹"""
    splits = path_split(path)
    res = os.path.join(*splits[level:])
    if _WINDOWS_SYSTEM:
        res = res.replace(os.sep, '/')
    return res


def print_dirtree(rootdir, prefix=''):
    """打印目录树
    ...
    prefix = ''
    dirpath = rootdir
    print_dirtree(rootdir, prefix=prefix)
    """
    
    def _print_dirtree(dirpath, prefix=''):
        entries = sorted(sorted(os.listdir(dirpath)),
                         key=lambda x: os.path.isdir(os.path.join(dirpath, x)))
        for i, entry in enumerate(entries):
            ...
            if os.path.isdir(path):
                _print_dirtree(path, new_prefix)
                
    print(rootdir)
    _print_dirtree(rootdir, prefix)
