...
def get_browser_regs():
    cfg_dir = get_parent_path(os.path.dirname(__file__), 1)
    fcfg = os.path.join(cfg_dir, '_config/browser_regs.yml')
    res = load_yml(fcfg)
    return res['BROWSER_REGS']
    
    
BROWSER_REGS = get_browser_regs()
...
def open_html(path_html, browser=None, logger=None):
    ...
    '''
    if browser is None:
        _browers  = ['edge', 'chrome', '360chrome', 'ie', 'firefox']
