...
def letterglue_creator(word):
    ...
    for i, letter in enumerate(word):
        group_size = (counter // 26) + 1
        group_index = counter % 26
        var_name = alphabet[group_index] * group_size
        code += f'{var_name} = "{letter}"\n'
        counter += 1
    glue_order = ", ".join([alphabet[(i // (26 ** (i // 26))) % 26] * ((i // 26) + 1) for i in range(len(word))])
