...
async def main():
    ...
    if not groq_api_key:
        print(
            "GROQ_API_KEY environment variable not found. Please set it before running this example."
        )
    ...
    model = await MultiProviderModel.create(
        primary_model="llama2-70b-4096",  # Groq's Llama2 model
        fallback_models=["mixtral-8x7b-instruct"],  # Fallback model
    ...
    print(
        "Type 'exit' to quit, 'stats' to view provider stats, or 'clear' to reset memory\n"
    )
