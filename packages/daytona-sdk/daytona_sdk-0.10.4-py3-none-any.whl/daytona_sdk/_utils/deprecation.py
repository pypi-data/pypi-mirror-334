from typing import Type, TypeVar, Callable, ParamSpec, Any, cast
...
def deprecated_alias(old_name: str, new_name: str) -> Callable[[Type[T]], Type[T]]:
    ...
    def decorator(cls: Type[T]) -> Type[T]:
        ...
        warning_message = f"`{old_name}` is deprecated. Please use `{new_name}` instead. This will be removed in a future version."
        ...
        if isinstance(cls, type) and issubclass(cls, Enum):
            ...
            for attr in ('__module__', '__qualname__', '__name__', '__doc__'):
                setattr(DeprecatedEnum, attr, getattr(cls, attr)
                        if attr != '__name__' else old_name)
