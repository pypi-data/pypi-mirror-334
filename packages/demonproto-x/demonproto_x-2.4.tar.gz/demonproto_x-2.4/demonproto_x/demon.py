...
class demon:
    ...
    def generate_signed_prekey(self):
        # Generate Signed PreKey Pair (SPK) using X25519
    ...
    def generate_onetime_prekey(self):
        # Generate One-Time PreKey Pair (OPK) using X25519
        self.onetime_prekey_private = PrivateKey.generate()
    ...
    def generate_replay_protection_token(self, message, recipient_public_key):
        ...
        recipient_key = recipient_public_key.encode(HexEncoder).hex()
    ...
    def encrypt(self, content, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public, is_file=False):
        ...
        if is_file:
            ...
            with open(encrypted_file_path, 'wb') as file:
                file.write(recipient_identity_public_key.encode(HexEncoder))
                file.write(iv)
            ...
            encrypted_content = {
                'recipient_public_key': recipient_identity_public_key.encode(HexEncoder).hex(),  # Convert recipient public key to hex
    ...
    def decrypt(self, encrypted_content, private_key, is_file=False):
        ...
        if is_file:
            with open(encrypted_content, 'rb') as file:
                recipient_public_key_bytes = file.read(64)  # Length of X25519 public key
                iv = file.read(32)
                tag = file.read(16)
                ciphertext = file.read()

            recipient_public_key = PublicKey(recipient_public_key_bytes, encoder=HexEncoder)
        else:
            recipient_public_key = PublicKey(bytes.fromhex(encrypted_content['recipient_public_key']), encoder=HexEncoder)
    ...
    def initiate_key_exchange(self):
        ...
        return self.identity_public_key.encode(HexEncoder)
    ...
    def prepare_message(self, message, recipient_identity_public_key_bytes, recipient_signed_prekey_public_bytes, recipient_onetime_prekey_public_bytes):
        ...
        recipient_identity_public_key = PublicKey(recipient_identity_public_key_bytes, encoder=HexEncoder)
        recipient_signed_prekey_public = PublicKey(recipient_signed_prekey_public_bytes, encoder=HexEncoder)
        recipient_onetime_prekey_public = PublicKey(recipient_onetime_prekey_public_bytes, encoder=HexEncoder)
