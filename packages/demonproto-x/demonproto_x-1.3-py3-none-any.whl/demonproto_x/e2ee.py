...
class demon:
    def __init__(self):
        ...
        # Generate Identity Key Pair (IK) using ECDHE for key exchange
        self.identity_private_key = ec.generate_private_key(ec.SECP256R1(), self.backend)
    ...
    def generate_signed_prekey(self):
        # Generate Signed PreKey Pair (SPK) using ECDHE
        self.signed_prekey_private = ec.generate_private_key(ec.SECP256R1(), self.backend)
    ...
    def generate_onetime_prekey(self):
        # Generate One-Time PreKey Pair (OPK) using ECDHE
        self.onetime_prekey_private = ec.generate_private_key(ec.SECP256R1(), self.backend)
    ...
    def rotate_session_keys(self, recipient_identity_public_key, recipient_signed_prekey_public, recipient_onetime_prekey_public):
        ...
        shared_secret = self.identity_private_key.exchange(ec.ECDH(), recipient_identity_public_key)
        shared_secret += self.signed_prekey_private.exchange(ec.ECDH(), recipient_signed_prekey_public)
        shared_secret += self.onetime_prekey_private.exchange(ec.ECDH(), recipient_onetime_prekey_public)
