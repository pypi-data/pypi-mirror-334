...
def get_base_command(config: Dict[str, Any], test_config: Dict[str, Any]) -> str:
    ...
    """
    return test_config.get('base_command', config.get('base_command', '/bin/bash'))
...
def process_params(test_config: Dict[str, Any], is_shell_command: bool) -> List[str]:
    """
    Process parameters for a test command.
    Simply returns a list of parameters as provided in the config.
    
    Args:
        test_config: The test-specific configuration
        is_shell_command: Whether the command is a shell command (not used anymore)
