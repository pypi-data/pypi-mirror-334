...
def build_command(base_command, params):
    """
    Build a command string with proper escaping based on command type.
    """
    shell_operators = re.compile(r'(?:&&|\|\||;|\$\(|\`)')
    requires_shell = bool(shell_operators.search(base_command))
    ...
    if base_command == 'python' and len(params) >= 2 and params[0] == '-c':
        ...
        cmd_to_run = " ".join(
            shlex.quote(comp) if ' ' in comp or "'" in comp or '"' in comp else comp
            for comp in cmd_components
        )
        
        # Check if the combined command contains shell operators
        if not requires_shell:
            requires_shell = bool(shell_operators.search(cmd_to_run))
    
    # If the command requires shell operations, wrap it in /bin/bash
    if requires_shell and not base_command.endswith('bash') and not base_command.endswith('sh'):
        escaped_cmd = cmd_to_run.replace("'", "'\"'\"'")
        cmd_to_run = f"/bin/bash -c '{escaped_cmd}'"
