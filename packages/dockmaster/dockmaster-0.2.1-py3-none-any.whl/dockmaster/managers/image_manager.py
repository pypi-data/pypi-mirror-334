    ...
    def push_image(
        ...
        try:
            ...
            if ":" in original_image_name:
                repository, tag = original_image_name.split(":", 1)
            else:
                repository = original_image_name
                tag = "latest"
            ...
            if use_existing_tags:
                ...
                # 获取repository部分
                timestamp_image_name = original_image_name
                # 构造latest镜像名称
            ...
            # 如果使用时间戳标签但不使用已有标签，需要创建新标签
                ...
                try:
                    ...
                    logger.success(f"已为镜像 {original_image_name} 添加时间戳标签 {timestamp_image_name}")
            ...
            if timestamp_image_name:
                # 添加命名空间前缀
                if namespace and "/" not in timestamp_image_name:
                    timestamp_target = f"{namespace}/{timestamp_image_name}"
                    try:
                        image = self.docker_client.images.get(timestamp_image_name)
                        image.tag(timestamp_target)
                        logger.success(f"已为镜像 {timestamp_image_name} 添加标签 {timestamp_target}")
                    except Exception as e:
                ...
                # 添加仓库前缀
                if registry:
                    registry_timestamp_target = f"{registry}/{timestamp_target}"
                    try:
                        image = self.docker_client.images.get(timestamp_target)
                        image.tag(registry_timestamp_target)
                        logger.success(f"已为镜像 {timestamp_target} 添加标签 {registry_timestamp_target}")
                        timestamp_target = registry_timestamp_target
                    except Exception as e:
            ...
            if use_existing_tags or use_timestamp_tag:
                latest_image_name = f"{repository}:latest"
                
                # 添加命名空间前缀
                if namespace and "/" not in latest_image_name:
                    latest_target = f"{namespace}/{latest_image_name}"
                    try:
                        image = self.docker_client.images.get(latest_image_name)
                        image.tag(latest_target)
                        logger.success(f"已为镜像 {latest_image_name} 添加标签 {latest_target}")
                    except Exception as e:
                        logger.error(f"添加命名空间前缀标签失败: {e}")
                        # 继续处理，因为时间戳标签可能已经成功
                else:
                ...
                # 添加仓库前缀
                if registry:
                    registry_latest_target = f"{registry}/{latest_target}"
                    try:
                        image = self.docker_client.images.get(latest_target)
                        image.tag(registry_latest_target)
                        logger.success(f"已为镜像 {latest_target} 添加标签 {registry_latest_target}")
                        latest_target = registry_latest_target
                    except Exception as e:
                        logger.error(f"添加仓库前缀标签失败: {e}")
                        # 继续处理，因为时间戳标签可能已经成功
            ...
            # 如果不使用时间戳标签和已有标签，直接处理原始镜像名称
                ...
                if namespace and "/" not in self.image_name:
                    # 自动添加命名空间前缀
                    target_image = f"{namespace}/{self.image_name}"
                    try:
                        image = self.docker_client.images.get(self.image_name)
                        image.tag(target_image)
                        logger.success(f"已为镜像 {self.image_name} 添加标签 {target_image}")
                    except Exception as e:
                ...
                if registry:
                    # 重新标记镜像
                    registry_target = f"{registry}/{target_image}"
                    try:
                        image = self.docker_client.images.get(target_image)
                        image.tag(registry_target)
                        logger.success(f"已为镜像 {target_image} 添加标签 {registry_target}")
                        target_image = registry_target
                    except Exception as e:
                        logger.error(f"添加仓库前缀标签失败: {e}")
                        return False
