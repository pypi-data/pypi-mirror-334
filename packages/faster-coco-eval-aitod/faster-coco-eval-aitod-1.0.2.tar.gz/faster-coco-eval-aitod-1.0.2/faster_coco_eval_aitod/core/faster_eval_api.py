...
import faster_coco_eval_aitod.faster_eval_api_cpp as _C
from faster_coco_eval_aitod.core.cocoeval import COCOeval as COCOevalBase
...
class COCOeval_faster(COCOevalBase):
    ...
    def accumulate(self, p=None, with_lrp=True):
        ...
        if self.separate_eval:
            ...
            self.eval = _C.COCOevalAccumulate(self._paramsEval, self._evalImgs_cpp, with_lrp)
        
        # Get dimension parameters from params
        num_categories = len(self.params.catIds) if self.params.useCats else 1
        num_area_ranges = len(self.params.areaRng)
        num_max_detections = len(self.params.maxDets)
        
        # Need to adjust the shape of the LRP keys
        lrp_keys = ['olrp_loc', 'olrp_fp', 'olrp_fn', 'olrp', 'lrp_opt_thr']
        
        for key in lrp_keys:
            if key in self.eval:
                # Verify that the data length matches the dimension product
                expected_len = num_categories * num_area_ranges * num_max_detections
                if len(self.eval[key]) != expected_len:
                    raise ValueError(
                        f"The data length does not match: {key} "
                        f"Expected {expected_len} (C{num_categories}×A{num_area_ranges}×M{num_max_detections}), "
                        f"Actually {len(self.eval[key])}"
                    )
                
                # Reshape to (C, A, M) three-dimensional array
                self.eval[key] = np.array(self.eval[key]).reshape(
