...
from faster_coco_eval_aitod.core import mask as maskUtils
from faster_coco_eval_aitod.core.coco import COCO
...
class COCOeval:
    ...
    def _summarize(
        ...
        if ap == 1:
            ...
            if lrp_type == 'oLRP_thresholds':
                s = self.eval['lrp_opt_thr'][:, aind, mind]
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            _count = 22 if self.lvis_style else 19
            ...
            if self.lvis_style:
                stats[19] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=0)  # APr
                stats[20] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=1)  # APc
                stats[21] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=2)  # APf
