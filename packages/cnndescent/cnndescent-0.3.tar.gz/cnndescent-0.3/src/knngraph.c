...
KNNGraph KNNGraph_create(float *data, DistanceFunc dist, uint32_t k, 
	...
	graph->k = k;
	graph->dim = dim;
	graph->data = vector_create(float, VEC_MIN_CAP);
	graph->stride = (graph->dim + 1);
	graph->dist = dist;
	graph->points = points;
...
void KNNGraph_bruteforce(KNNGraph graph) {
	...
	for (size_t i = 0UL; i < graph->points; ++i) {
		for (size_t j = i + 1; j < graph->points; ++j) {
			float dist = graph->dist(&graph->data[i * graph->stride], &graph->data[j * graph->stride], graph->dim);
		...
		for (size_t v = 0UL; v < graph->points; ++v) {
			...
			for (size_t i = 0UL; i < vector_size(new[v]); ++i) {
				for (size_t j = 0UL; j < vector_size(pairs[i].neighbors); ++j)
					pairs[i].neighbors[j].dist = graph->dist(&graph->data[pairs[i].id * graph->stride],
															 &graph->data[pairs[i].neighbors[j].id * graph->stride],
															 graph->dim);
			}
...
Neighbor *KNNGraph_KNearest(KNNGraph graph, float *point) {
	...
	float dist = graph->dist(&graph->data[start_node * graph->stride], point, graph->dim);
	...
	while (vector_size(candidates) > 0) {
		...
		for (size_t i = 0UL; i < vector_size(graph_neighbors[cand]); ++i) {
			...
			float dist = graph->dist(&graph->data[neighbor * graph->stride], point, graph->dim);
