...
RPTree RPTree_create(float *data, uint32_t points, uint32_t stride,
...
static uint32_t tree_split_rec(RPTree tree, uint32_t *data, uint32_t depth) {
    ...
    for (size_t i = 0UL; i < tree->dimension; ++i) {
        ...
        hyperplane[i] = tree->data[ind * tree->stride + i] - tree->data[ind_ * tree->stride + i];
        offset += hyperplane[i] * (tree->data[ind * tree->stride * i] + tree->data[ind_ * tree->stride + i]) / 2;
    }

    for (size_t i = 0; i < vector_size(data); i++) {
        /* Split data according to their projection onto the hyperplane */
        float margin = dot_product(hyperplane, &(tree->data[data[i] * tree->stride]), tree->dimension) - offset;
        if (margin < -EPSILON) {
...
RPTree *RPTree_build_forest(uint32_t n_trees, float *data, uint32_t stride, uint32_t points,
                           uint32_t dimension, uint32_t leaf_size) {
    RPTree *forest = malloc(sizeof(RPTree) * n_trees);
    # pragma omp parallel for num_threads(n_threads)
    for (size_t i = 0UL; i < n_trees; ++i) {
        forest[i] = RPTree_create(data, stride, points, dimension, leaf_size);
