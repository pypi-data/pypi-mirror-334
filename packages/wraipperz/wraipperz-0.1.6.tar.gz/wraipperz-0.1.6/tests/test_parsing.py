...
class Address(BaseModel):
    street: str = Field(json_schema_extra={"example": "123 Main St", "comment": "Street address"})
    city: str = Field(json_schema_extra={"example": "New York", "comment": "City name"})
    country: str = Field(json_schema_extra={"example": "USA"})


class Actor(BaseModel):
    name: str = Field(json_schema_extra={"example": "John Doe", "comment": "Full name of the actor"})
    height: float = Field(json_schema_extra={"example": 1.85, "comment": "Height in meters"})
    gender: Optional[Gender] = Field(default=None, json_schema_extra={"example": "male", "comment": "Gender identity"})
    weight: Optional[float] = Field(default=None, json_schema_extra={"example": 75.5})
    birth_date: Optional[date] = Field(default=None, json_schema_extra={"example": "1990-01-15", "comment": "Date of birth"})
    languages: List[str] = Field(default_factory=list, json_schema_extra={"example": ["English", "French"], "comment": "Languages spoken"})
    address: Optional[Address] = Field(default=None, json_schema_extra={"example": {"street": "123 Broadway", "city": "Los Angeles", "country": "USA"}, "comment": "Current address"})
    filmography: Dict[int, str] = Field(default_factory=dict, json_schema_extra={"example": {2020: "Movie A", 2021: "Movie B"}, "comment": "Movies by year"})
...
def test_complex_nesting_yaml_generation():
    ...
    if "category1" in parsed_data["nested_dict"]:
        ...
        if isinstance(category1, dict) and "subcategory1" in category1:
            ...
            pytest.fail(f"subcategory1 not found in category1 or category1 is not a dict: {category1}")
    else:
...
def test_list_nested_object():
    ...
    class Line(BaseModel):
        ...
        text: str = Field(json_schema_extra={"example": "Hello, how are you?", "comment": "The text of the line"})
        # extra_info: str = Field(json_schema_extra={"example": "Hello, how are you?", "comment": "The text of the line"})

    class Lines(BaseModel):
        lines: List[Line] = Field(json_schema_extra={"example": [Line(name="Bob", text="Hello, how are you?")], "comment": "Lines of each entry in the script in order."})
