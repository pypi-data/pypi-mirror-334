...
def http_rest(errors: list[str],
    ...
    if jwt_data:
        ...
        if auth_scheme == "bearer":
            # yes, import the JWT implementation package
            from pypomes_jwt import jwt_issue_tokens
            # request the authentication token
            account_id: str = jwt_data.pop("account-id")
            token_data: dict[str, Any] = jwt_issue_tokens(errors=op_errors,
                                                          account_id=account_id,
                                                          account_claims=jwt_data,
                                                          logger=logger)
            if not op_errors:
                op_headers = op_headers or {}
                op_headers["Authorization"] = f"Bearer {token_data.get('access_token')}"
