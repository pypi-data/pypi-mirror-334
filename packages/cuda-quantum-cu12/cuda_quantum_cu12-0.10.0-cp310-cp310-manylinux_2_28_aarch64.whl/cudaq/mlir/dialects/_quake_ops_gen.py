...
@_ods_cext.register_operation(_Dialect)
@_ods_extend_opview_class(_ods_ext_module)
class GetStateOp(_ods_ir.OpView):
  OPERATION_NAME = "quake.get_state"

  _ODS_REGIONS = (0, True)

  def __init__(self, result, calleeName, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    _ods_context = _ods_get_default_loc_context(loc)
    attributes["calleeName"] = (calleeName if (
    issubclass(type(calleeName), _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('StrAttr')) else
      _ods_ir.AttrBuilder.get('StrAttr')(calleeName, context=_ods_context))
  ...
  @builtins.property
  def calleeName(self):
    return _ods_ir.StringAttr(self.operation.attributes["calleeName"])

  @calleeName.setter
  def calleeName(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["calleeName"] = value
