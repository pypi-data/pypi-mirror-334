...
#Редакт чего-то
    ...
    def edit_message_media(self, chat_id: Union[int, str] = None, message_id: int = None, media: Union[str, bytes, InputMediaPhoto, InputMediaAnimation, InputMediaVideo, InputMediaAudio, InputMediaDocument] = None, caption: str = None, mode: str = "Markdown", inline_message_id: str = None, reply_markup: Union[dict, Markup] = None) -> bool:
...
def get_models():
    ...
    return format_models(json.loads(requests.get('https://text.pollinations.ai/models').content))
def format_models(models):
    """Редакт списка моделей"""
    def format_model(model):
        name = model['name']
        description = model['description']
        type_model = model['type']
        capabilities = ', '.join([k for k in model if k not in ['name', 'description', 'censored', 'type']])
        censored = "Да" if model.get('censored', False) else "Нет"
        vision = "Да" if model.get('vision', False) else "Нет"
        return f"• `{name}`: {description} | тип: {type_model} | возможности: {capabilities} | цензура: {censored} | виденье: {vision}"
...
def generate_chat_otvet(messages: list = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False):
    ...
    • 'hypnosis-tracy' - Hypnosis Tracy 7B - Self-help AI assistant
